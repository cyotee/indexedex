...
abstract contract UniswapV4DetfQueryTarget is UniswapV4DetfTarget {
    ...
    /// @notice Registered static wrapper holding raw DETF.
    function rawSY() external view returns (address) { return DETFChildSYRepo._layoutStruct().rawSY; }

    /// @notice Registered static wrapper holding funded staking gons.
    function stakingSY() external view returns (address) { return DETFChildSYRepo._layoutStruct().stakingSY; }

    /// @notice Quote the funded index after due expansion and the proposed mint's seigniorage.
    /// @dev This hypothetical state is confined to previews; stored balances and rates remain funded only.
    function previewStakingGonsPerUnit(IERC20 tokenIn_, uint256 amountIn_) external view returns (uint256) {
        ...
        address staking_ = _entryRebasingClaimToken();
        if (amountIn_ != 0 && address(tokenIn_) != address(this) && address(tokenIn_) != staking_) {
            (uint256 gross_,) = _entryPreviewMint(tokenIn_, amountIn_);
            rewards_[1] = _splitMintedDetf(gross_).inventoryDetf;
        }
        return IStakedDETF(staking_).previewDistributions(rewards_).gonsPerUnit;
    }
    ...
    /// @notice Returns the actual immutable policy verified by the deployment package.
    function ownerOnlyLiquidity() external view returns (bool) {
        return IUniswapV4HookLiquidityPolicy(_entryHook()).ownerOnlyLiquidity();
    }
    ...
    /// @notice Quote purchased DETF, vested principal, funded rewards and new liquidity DETF.
    /// @dev Reserve-LP quotes retain the complete position and always return zero liquidity DETF.
    function previewBond(IERC20 tokenIn_, uint256 amountIn_, uint256 lockDuration_) external view
        returns (uint256 purchasedDetf_, uint256 principalDetf_, uint256 rewardsDetf_, uint256 liquidityDetf_)
    {
        uint256 duration_ = _effectiveLockDuration(lockDuration_);
        if (amountIn_ == 0) revert Repo.ZeroAmount();
        Repo.Storage storage s_ = Repo._layoutStruct();
        if (address(tokenIn_) == s_.hook) {
            purchasedDetf_ = _quoteReserveLpBond(amountIn_, duration_);
        } else {
            address pair_;
            IStandardExchange vault_;
            if (!s_.isReserveLive) {
                pair_ = _firstBondPair(address(tokenIn_));
                vault_ = IStandardExchange(_hook().standardExchangeOf(pair_));
            } else {
                if (!s_.bondTable.tokens._contains(address(tokenIn_))) {
                    revert Repo.InvalidRoute(address(tokenIn_), address(this));
                }
                vault_ = s_.bondTable.vaultOf[address(tokenIn_)];
                pair_ = _hookPairOfVault(vault_);
            }
            if (address(tokenIn_) == pair_ || address(tokenIn_) == address(vault_)) {
                uint256 pairEq_ = _pairEq(vault_, tokenIn_, amountIn_);
                purchasedDetf_ = _quoteBondPurchase(pair_, pairEq_, duration_);
                liquidityDetf_ = _quoteBondG(pair_, pairEq_);
            } else {
                (purchasedDetf_, liquidityDetf_) = _previewCustomBondAmounts(address(tokenIn_), pair_, amountIn_, duration_);
            }
        }
        MintSplit memory split_ = _splitBondDetf(purchasedDetf_, liquidityDetf_);
        principalDetf_ = split_.userDetf;
        rewardsDetf_ = split_.inventoryDetf;
    }

    function _previewCustomBondAmounts(address tokenIn, address pair, uint256 amount, uint256 duration)
        private view returns (uint256 purchased, uint256 liquidity)
    {
        uint256 multiplier = DETFBondNFTMathLib._bonusMultiplierOfVault(address(this), duration);
        uint256 pairValue;
        (pairValue, purchased, liquidity) = _quote().previewBondAfterDeposit(tokenIn, pair, address(this), amount, multiplier);
        if (!Repo._layoutStruct().isReserveLive) {
            purchased = _openingBondQuote(pair, Math.mulDiv(pairValue, multiplier, ONE_WAD));
            liquidity = _openingBondQuote(pair, pairValue);
        } else if (liquidity == 0) {
            liquidity = _openingBondQuote(pair, pairValue);
        }
    }
    ...
    function previewFirstBondPayments(IERC20 tokenIn_, uint256 amount_)
        external view returns (address[] memory tokens_, uint256[] memory amounts_)
    {
        return _entryPreviewFirstBondPayments(tokenIn_, amount_);
    }
}
