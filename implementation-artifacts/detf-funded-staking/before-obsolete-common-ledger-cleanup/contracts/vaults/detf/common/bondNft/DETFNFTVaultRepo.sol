...
library DETFNFTVaultRepo {
    ...
    function _initialize(IDetf detf_, IERC20 lpToken_, IERC20 rewardToken_, uint8 decimalOffset_) internal {
    ...
    function _addToPosition(Storage storage layoutStruct_, uint256 tokenId_, uint256 additionalShares_) internal {
        ...
        if (newEff_ > 0 && pending_ > 0) {
            ...
            layoutStruct_.userRewardPerSharePaid[tokenId_] = rps_ > pendingPerShare_ ? rps_ - pendingPerShare_ : 0;
        } else {
     ...
     */
    function _convertToShares(Storage storage layoutStruct_, uint256 lpAmount_)
        internal
        view
        returns (uint256 shares_)
    {
        ...
        if (totalShares_ == 0 || totalLpReserve == 0) {
            ...
            shares_ =
                BetterMath._convertToSharesDown(lpAmount_, totalLpReserve, totalShares_, layoutStruct_.decimalOffset);
        }
    }
    ...
    function _convertToSharesAtLpReserve(Storage storage layoutStruct_, uint256 lpAmount_, uint256 lpReserveBefore_)
        internal
        view
        returns (uint256 shares_)
    {
        ...
        shares_ =
            BetterMath._convertToSharesDown(lpAmount_, lpReserveBefore_, totalShares_, layoutStruct_.decimalOffset);
    }

    /**
     * @notice Converts shares back to LP token amount.
     */
    function _convertToAssets(Storage storage layoutStruct_, uint256 shares_)
        internal
        view
