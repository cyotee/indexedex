...
abstract contract UniswapV4Detf_PonsV2Se_ProductLaw_Decimals is UniswapV4Detf_PonsV2Se_Stage11Helpers_Decimals, V4FundedD25Behavior {
    ...
    function test_T7_10_laterBond_joinUnbalanced_unboostedG() public {
        ...
        IERC20 payment_ = IERC20(launchToken);
    ...
    function test_T7_14_commonNftUnused_claimHoldsNoHookLp() public {
        ...
        IStandardExchangeIn(address(detfInfo)).exchangeIn(IERC20(launchToken), 10 ether, IERC20(address(detfInfo)), 0, detfUser, false, _deadline());
        vm.stopPrank();
    ...
    function test_principalVestsLinearlyWhileStaked() public {
    ...
    function test_matureBondPaysFundedStakingAndRetainsLiquidity() public {
    ...
    function test_issuanceFundsStakingImmediatelyAndSettlementDoesNotJoinLp() public {
        _assert_compound_raises_protocolLp(_deployOpenLive());
    }

    function test_mandatoryPolicyWithoutFundedPremiumDoesNotExpand() public {
        address d_ = _deployOpenLive();
    ...
    function test_A0_donateBeforeFirstBond_cannotFreeMint() public {
        ...
        IStandardExchangeIn(address(info_)).exchangeIn(IERC20(launchToken), 1 ether, IERC20(address(info_)), 0, attacker, false, _deadline());
        vm.stopPrank();
    ...
    function test_T_LOCAL_I1_bookedDetf_trueWithoutPushReverts() public {
        ...
        if (U == 0) {
            vm.expectRevert();
            vm.prank(detfUser);
            IStandardExchangeIn(address(detfInfo)).exchangeIn(IERC20(launchToken), 1, IERC20(address(detfInfo)), 0, detfUser, true, _deadline());
        } else {
            vm.prank(detfUser);
            IStandardExchangeIn(address(detfInfo)).exchangeIn(IERC20(launchToken), U, IERC20(address(detfInfo)), 0, detfUser, true, _deadline());
            vm.expectRevert();
            vm.prank(detfUser);
            IStandardExchangeIn(address(detfInfo)).exchangeIn(IERC20(launchToken), 1, IERC20(address(detfInfo)), 0, detfUser, true, _deadline());
        }
