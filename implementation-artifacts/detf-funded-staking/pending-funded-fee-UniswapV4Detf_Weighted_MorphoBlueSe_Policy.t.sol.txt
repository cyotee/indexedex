    ...
    UniswapV4Detf_Stage11PolicySuite
{
    ...
    function test_T8_4_policy_pairA_not_pairB_via_trades() public {
        ...
        uint256 out_ = IStandardExchangeIn(address(info)).exchangeIn(pairA, LIVE_MINT_AMT, IERC20(address(info)), 0, detfUser, false, _deadline());
        vm.stopPrank();
        assertGt(out_, 0, "live mint A");
        vm.startPrank(detfUser);
        vm.expectRevert();
        IStandardExchangeIn(address(info)).exchangeIn(pairB, 1 ether, IERC20(address(info)), 0, detfUser, false, _deadline());
        vm.stopPrank();
    }
