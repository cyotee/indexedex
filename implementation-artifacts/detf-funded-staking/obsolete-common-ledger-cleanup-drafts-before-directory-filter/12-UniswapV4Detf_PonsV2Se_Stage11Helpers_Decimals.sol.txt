...
abstract contract UniswapV4Detf_PonsV2Se_Stage11Helpers_Decimals is TestBase_UniswapV4Detf_PonsV2Se_Decimals, V4FundedDonationAssertions, V4FundedFeeBehavior, V4FundedPolicyAssertions {
    ...
    function _firstBondOn(address d, uint256 amt) internal returns (uint256 tokenId, uint256 shares) {
        ...
        vm.stopPrank();
        if (_policyInitialBond[d] == 0) _policyInitialBond[d] = tokenId;
    }
    ...
    function _mintOn(address d, uint256 amt) internal returns (uint256 userDetf) {
        ...
        vm.startPrank(detfUser);
        IERC20(launchToken).approve(d, type(uint256).max);
        userDetf = IStandardExchangeIn(d).exchangeIn(IERC20(launchToken), amt, IERC20(d), 0, detfUser, false, _deadline());
        vm.stopPrank();
    }
    ...
    function _liveMintOn(address d, address who, uint256 amt) internal returns (uint256 userDetf) {
        ...
        vm.startPrank(who);
        IERC20(launchToken).approve(d, type(uint256).max);
        userDetf = IStandardExchangeIn(d).exchangeIn(IERC20(launchToken), amt, IERC20(d), 0, who, false, _deadline());
        vm.stopPrank();
    }
        ...
        internal returns (uint256 principal, uint256 claimMinted)
    {
        ...
        uint256 lp_ = IERC20(IUniswapV4Detf(d).hook()).balanceOf(address(nft_));
        vm.prank(seller);
        uint256 rewards_;
        ...
        assertEq(IERC20(IUniswapV4Detf(d).hook()).balanceOf(address(nft_)), lp_, "claim retains protocol LP");
    }
    ...
    function _donateToken(address d, address token, uint256 amount) internal {
        ...
        IUniswapV4Detf(d).donate(IERC20(token), amount, false);
        vm.stopPrank();
    }
    ...
    function _pushSyntheticUp(address d) internal {
        ...
        for (uint256 i; i < toks_.length; ++i) {
            ...
            _donateToken(d, toks_[i], 200 ether);
    ...
    function _expectedJoinDetf(uint256 pairAmount_, uint256 opening_) internal pure returns (uint256) {
        return pairAmount_ * ONE_WAD / opening_ / 1e9;
    }
