...
record_path = art / 'production-readiness/corrected-candidate-regressions.json'
...
contracts = '^(CanonicalRegistryAuthorizationTest|UniswapV4Detf_Burn_H6|(Cp|Weighted|Orbital|CurveQuad)(Public)?ReserveLiquidityTest)$'
tests = '^(test(Fuzz)?_canonical|test(Fuzz)?_small|test_T7_9_|test_standardFallbackOneNativeUnitThenFullExit)'
...
expected = {'CanonicalRegistryAuthorizationTest': 8, 'UniswapV4Detf_Burn_H6': 11}
for family in ('Cp', 'Weighted', 'Orbital', 'CurveQuad'):
    for policy in ('', 'Public'):
        expected[family + policy + 'ReserveLiquidityTest'] = 1
actual = {}
for line in lines:
...
record['validation_passed'] = (result.returncode == 0 and record['passed'] == 27
