...
The existing NFT reward detector uses a positive DETF balance delta as new rewards (`DETFNFTVaultRepo._updateGlobalRewards`). A funded staking reserve also receives DETF principal on deposits. Its accounting must distinguish those deposits from rewards; a principal deposit must not itself rebase existing holders. This is an accounting adaptation, not a new reward-rate formula.
