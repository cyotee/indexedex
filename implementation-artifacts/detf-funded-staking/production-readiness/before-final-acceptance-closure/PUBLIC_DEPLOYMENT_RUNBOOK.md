# Public deployment handoff — candidate pending validation

This is a future operator procedure, not authorization to broadcast. This execution is limited to hermetic tests, read-only upstream forks and an isolated local deployment. No public deployment or fund migration has occurred. Use this procedure only after the final release manifest reports that all required acceptance gates passed for the exact source and artifact hashes.
...
## Evidence required before the hold can be removed

The final report must link the matching complete build (including maintained scripts), unfiltered hermetic result, 27 existing security/boundary regressions and seven new lifecycle regressions, all eight retained V3 cases, matching provider fork evidence, strict local package receipts and all 39 lifecycle cases. The 35 retained provider/V3 cases have an explicit verified dependency-closure reuse record; that record never substitutes for a renewed full hermetic result after PR-09/10. Include source/dependency and artifact manifests, code-size/link checks, actual package/proxy cuts and configuration, storage/caller reconciliation, A1–A42 dispositions and unresolved limitations. The internal review must not be represented as an independent external audit.
