...
## Execution checkpoint — 2026-09-11 UTC

The prior candidate passed 31,305 hermetic tests, 27 overlapping security regressions, 35 pinned forks and 410 frontend tests, and deployed all 19 local onchain stages (156 verified receipts). That evidence remains archived.

The strict 39-case local lifecycle then produced **33 passed / 6 failed / 0 skipped**. Traces confirmed PR-09 (zero-LP residual retries exhaust CP/V3 transaction gas) and PR-10 (Orbital NAV intermediate arithmetic overflows for high-precision V2 shares). Scoped production fixes and seven direct/constructive-fuzz regressions are implemented; validation is pending. Four Quad fixture cases now establish actual funded inventory growth required for growth-based LP fees, retaining all assertions and gas bounds.

The next complete build passed in 9,253.252 seconds. Its seven new regressions returned **5 passed / 2 failed / 0 skipped**: the four Orbital cases passed, while nested V3 zero-amount errors still exhausted the CP/V3 sweep budget. The exact native V3/V4 errors now terminate those retries. The unchanged seven cases and complete validation are being renewed; the failed trace is retained under `production-readiness/before-nested-zero-error-fix/`.

The current complete build passed in **8,552.311 seconds**, including all maintained scripts. All **seven new regressions passed / zero failed / zero skipped**; CP/V3 fuzz executed 17 cases including the retained replay, and Orbital fuzz executed 16. The original failing residual now completes within the unchanged 30M call budget. The existing 27 regressions and full unfiltered suite are running next; local deployment/lifecycle and funding remain pending.

Current source/config fingerprint: `187b03242b92380b8696be84a5d3f55a74ba4a140012b67820a4bee70cd7b2fe`. P1, affected P2/P3 checks and current-package P5 deployment verification are reopened. Older tests and deployed immutable bytecode are not evidence for changed production paths. Preserve local nodes, corrected core and historical receipts; deploy updated packages locally after the complete build. Funding simulation and final release acceptance remain pending. **The exact candidate is not ready.**
...
| Current-package local deployment and 39 rehearsal cases | Earlier candidate: deployment/receipts/runtime passed; 33/39 lifecycle cases passed. Two confirmed production defects and one fixture prerequisite were corrected; the exact final candidate now requires renewed build/tests and fresh deployment on node 18665. |
...
- [ ] Recheck the changed V4 residual-capital sweep and dust parking against real SE quotes: joinable residuals are processed, repeated unchanged balances terminate, and parked assets do not create an unfunded user entitlement.
...
- [ ] Run relevant caller checks when sources, ABIs or bindings changed. The active application declares `npm --prefix frontend/apps/dtf run check`; add its applicable live transaction tests against the local rehearsal when deployment bindings change. Reuse matching earlier lint/typecheck/unit/SVG evidence.
- [ ] Reconcile retained/merged/retired tests with requirement coverage and compiled method counts. Report measured build/runtime effects without adding overlapping test totals or claiming unmeasured speedups.
...
- [ ] If production code changed in P1/P2, run `forge build` before targeted tests; include maintained scripts in the complete build. After corrections stabilize, obtain one matching full hermetic result. If only evidence is missing, collect or run only the missing check.
- [ ] Verify deployed runtime and creation artifacts against current sources and dependencies, especially components loaded indirectly by FactoryServices. Check every release component against the 24,576-byte runtime limit and verify library linking.
- [ ] Record exact commands, exit codes, source/configuration fingerprints, test counts, skipped cases and timings. No required test may silently disappear through a changed filter.
