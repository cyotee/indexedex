    ...
    TestBase_AaveV3StataStandardExchange_Decimals
{
    ...
    function test_E1_mintRedeemRoundTrip_bounded() public {
        ...
        vm.startPrank(attacker);
        IERC20(realStata).approve(realVault, stataShares_);
        uint256 shares_ = IStandardExchangeIn(realVault).exchangeIn(
            IERC20(realStata), stataShares_, IERC20(realVault), 0, attacker, false, _deadline()
        );
        vm.stopPrank();
        assertEq(shares_, stataShares_, "1:1 first mint");

        uint256 stataBefore_ = IERC20(realStata).balanceOf(attacker);
        vm.startPrank(attacker);
        IERC20(realVault).approve(realVault, shares_);
        uint256 burned_ = IStandardExchangeOut(realVault).exchangeOut(
            IERC20(realVault), shares_, IERC20(realStata), shares_, attacker, false, _deadline()
        );
        vm.stopPrank();

        assertEq(burned_, shares_, "E1 burned SE");
        uint256 received_ = IERC20(realStata).balanceOf(attacker) - stataBefore_;
        assertLe(received_, stataShares_, "E1: no free lunch on SE mint/redeem round-trip");
        assertEq(received_, stataShares_, "E1 1:1 redeem stata");
    }
