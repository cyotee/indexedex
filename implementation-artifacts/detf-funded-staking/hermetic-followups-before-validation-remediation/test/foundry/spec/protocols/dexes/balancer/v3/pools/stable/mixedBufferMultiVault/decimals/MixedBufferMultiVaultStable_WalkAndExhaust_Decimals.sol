...
abstract contract MixedBufferMultiVaultStable_WalkAndExhaust_Decimals is TestBase_MixedBufferMultiVaultStablePool_Decimals {
    ...
    function test_M11_walk_deposit_secondVaultSucceeds() public {
        ...
        rateAsset.mint(address(h0), MBMVS_INIT_BUFFER * 5);
        rateAsset.mint(alice, MBMVS_INIT_BUFFER * 5);
        mintSharesForVault(0, alice, MBMVS_INIT_SHARES * 5);
        pairToken.mint(alice, MBMVS_INIT_UNPAIRED * 5);
    ...
    function test_M11_allVaultsExhausted_selector() public {
        ...
        rateAsset.mint(address(h0), MBMVS_INIT_BUFFER * 3);
        rateAsset.mint(address(h1), MBMVS_INIT_BUFFER * 3);
        pairToken.mint(alice, MBMVS_INIT_UNPAIRED * 3);
    ...
    function test_M11_preSeat_allVaultsExhausted_selector() public {
        ...
        rateAsset.mint(address(h0), MBMVS_INIT_BUFFER * 5);
        rateAsset.mint(address(h1), MBMVS_INIT_BUFFER * 5);
        pairToken.mint(alice, MBMVS_INIT_UNPAIRED * 5);
    ...
    function _initTwoVaultPool(address pool, address share0, address share1) internal {
        ...
        for (uint256 i; i < toks.length; ++i) {
            if (address(toks[i]) == address(rateAsset)) amounts[i] = MBMVS_INIT_BUFFER;
            else if (address(toks[i]) == address(pairToken)) amounts[i] = MBMVS_INIT_UNPAIRED;
