...
abstract contract MixedLegWeightedBufferPoolSpec_Decimals is TestBase_MixedLegWeightedBufferPool_Decimals {
    ...
    function test_deploy_mixed_U2_P1() public view {
        ...
        assertEq(address(ml().bufferToken(0)), address(rateAsset));
        assertEq(address(ml().shareToken(0)), address(seVault));
        assertEq(address(ml().unpairedToken(0)), address(pairToken));
    ...
    function test_reject_unpairedEqualsBuffer() public {
        ...
        unpaired[0] = IERC20(address(rateAsset)); // same as buffer
        ...
        buffers[0] = IERC20(address(rateAsset));
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IMixedLegWeightedBufferPool.DuplicatePoolToken.selector, address(rateAsset))
        );
    ...
    function test_swap_unpairedToUnpaired_exactIn() public {
        ...
        uint256 out = swapExactIn(alice, IERC20(address(pairToken)), IERC20(address(weth)), amountIn);
        assertGt(out, 0);
    ...
    function test_swap_unpairedToBuffer_exactIn() public {
        ...
        uint256 daiBefore = rateAsset.balanceOf(alice);
        ...
        assertGt(out, 0);
        assertEq(rateAsset.balanceOf(alice), daiBefore + out);
    ...
    function test_swap_bufferToUnpaired_exactIn() public {
        ...
        uint256 usdcBefore = pairToken.balanceOf(alice);
        uint256 vtBefore = ml().virtualBuffer(0);
        uint256 out = swapExactIn(alice, IERC20(address(rateAsset)), IERC20(address(pairToken)), amountIn);
        assertGt(out, 0);
        assertEq(pairToken.balanceOf(alice), usdcBefore + out);
    ...
    function test_swap_bufferToShares_exactIn() public {
        ...
        uint256 out = swapExactIn(alice, IERC20(address(rateAsset)), IERC20(address(seVault)), amountIn);
        assertGt(out, 0);
    ...
    function test_swap_sharesToBuffer_exactIn() public {
        ...
        uint256 daiBefore = rateAsset.balanceOf(alice);
        uint256 out = swapExactIn(alice, IERC20(address(seVault)), IERC20(address(rateAsset)), amountIn);
        assertGt(out, 0);
        assertEq(rateAsset.balanceOf(alice), daiBefore + out);
    ...
    function test_lp_addProportional_scalesVirtual() public {
        ...
        rateAsset.mint(alice, ML_INIT_BUFFER * 2);
        pairToken.mint(alice, ML_INIT_UNPAIRED * 2);
