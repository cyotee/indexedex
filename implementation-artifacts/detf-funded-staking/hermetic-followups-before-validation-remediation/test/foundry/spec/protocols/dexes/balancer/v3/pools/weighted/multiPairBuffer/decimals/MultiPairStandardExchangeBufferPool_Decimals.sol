...
abstract contract MultiPairStandardExchangeBufferPoolSpec_Decimals is TestBase_MultiPairStandardExchangeBufferPool_Decimals {
    ...
    function test_init_virtualBuffer_fromBufferSeed() public view {
        ...
        assertEq(mp().virtualBuffer(0), ttaScaled18(MP_INIT_BUFFER));
    ...
    function test_swap_bufferToShares_exactIn() public {
        ...
        // permit2 already approved for router in base setUp for rateAsset
        ...
        uint256 poolDai = rateAsset.balanceOf(address(bv3Vault)); // vault holds pooled tokens
    ...
    function test_lp_addProportional_scalesVirtual() public {
        ...
        rateAsset.mint(alice, MP_INIT_BUFFER * 2);
        ...
        vm.startPrank(alice);
        IERC20(address(rateAsset)).approve(address(router), type(uint256).max);
