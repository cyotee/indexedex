...
p=Path('test/foundry/spec/vaults/detf/common/claimToken/V4ReserveLiquidity.t.sol');old=p.read_text();s=old
imports='''import {SimpleMintableERC20} from "contracts/test/stubs/SimpleMintableERC20.sol";
...
anchor='contract CpReserveLiquidityTest is TestBase_UniswapV4Detf, V4ReserveLiquidityBehavior, DETFFundedStakingArtifacts {'
code='''
...
'''
assert anchor in s;s=s.replace(anchor,anchor+code)
art=Path('implementation-artifacts/detf-funded-staking');(art/'v4-lido-buffer-regression.patch').write_text(''.join(difflib.unified_diff(old.splitlines(True),s.splitlines(True),fromfile=str(p),tofile=str(p))))
print('Prepared V4 Lido regression in existing two-policy CP suite; not applied')
