...
import hashlib,json
base=Path('contracts/protocols/dexes/uniswap/v3')
changed=[]
def save(p,s):
...
p=base/'UniswapV3StandardExchangeInQueryTarget.sol';s=p.read_text()
...
save(p,s)
p=base/'UniswapV3StandardExchangeInMultiQueryTarget.sol';s=p.read_text();mark='    function previewExchangeInManyToOne(';assert mark in s;save(p,s.replace(mark,preview+'\n'+mark,1))
p=base/'UniswapV3StandardExchangeInQueryFacet.sol';s=p.read_text().replace('Preview-only facet to keep mutate InFacet under EIP-170.','Inventory-transition queries; split from standard previews to remain under EIP-170.')
...
save(p,s)
p=base/'UniswapV3StandardExchangeInMultiQueryFacet.sol';s=p.read_text().replace('import {IFacet}', 'import {IStandardExchangeIn} from "@crane/contracts/interfaces/IStandardExchangeIn.sol";\n\nimport {IFacet}',1)
...
save(p,s)
Path('implementation-artifacts/detf-funded-staking/v3-query-size-sources.json').write_text(json.dumps(changed,indent=2)+'\n')
