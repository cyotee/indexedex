...
/// @notice Minimal harness exposing BasicVaultCommon._secureTokenTransfer.
contract BasicVaultCommonHarness is BasicVaultCommon {
    /// @notice Initialize Permit2AwareRepo so tests can exercise the Permit2 path.
 ...
 * @notice Regression tests for IDXEX-035: balance-delta accounting in BasicVaultCommon._secureTokenTransfer.
...
contract BasicVaultCommon_TokenTransfer_Test is Test {
    ...
    function setUp() public {
        // Not a fork test file: we set the production Permit2 address, but these
        // spec tests only exercise the ERC20 allowance path.
        harness = new BasicVaultCommonHarness(PERMIT2_PRODUCTION);
    ...
    /* ---------------------------------------------------------------------- */
    /*  US-IDXEX-035.1: Balance-delta accounting                              */
    /* ---------------------------------------------------------------------- */
    ...
    /* ---------------------------------------------------------------------- */
    /*  US-IDXEX-035.3: Fee-on-transfer token scenarios                       */
    /* ---------------------------------------------------------------------- */
