...
contract SingleStandardExchangeDETF_NaturalExpansion_Test is TestBase_SingleStandardExchangeDETF {
    ...
    ISingleStandardExchangeDETFInfo internal expInfo;
    ISingleStandardExchangeDETFBonding internal expBonding;
    ...
    function setUp() public virtual override {
        ...
        expInfo = ISingleStandardExchangeDETFInfo(expDetf);
        expBonding = ISingleStandardExchangeDETFBonding(expDetf);
    }
    ...
    function test_E3_multiBondRewardRatioMatchesShares() public {
        // Use the setUp instance (already live) + a second smaller bond from bob for ratio proof.
        // Avoid a second large bootstrap that can hit MaxInRatio after setUp drained SE liquidity.
        IDETFNFTVault vault_ = _bondNftVault(expDetf);
        uint256 bondA_ = userBondId;
        uint256 seSharesB_ = _fundSeShares(bob, 80e18);
        vm.startPrank(bob);
        seShare.approve(expDetf, seSharesB_);
        (uint256 bondB_,) = expBonding.bond(
            seShare, seSharesB_, DEFAULT_MIN_LOCK, bob, false, block.timestamp + 1 hours
        );
        vm.stopPrank();
