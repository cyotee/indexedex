...
abstract contract SingleStandardExchangeDETF_NestedPush_Decimals is TestBase_SingleStandardExchangeDETF_Decimals {
    ...
    function test_T_NEST_2_nestedShort_hostRevertsTransferDeltaInsufficient() public {
        ...
        // Use rateAsset (rateAsset) so the secure-pull gate runs before deep SE swap math.
        uint256 dust_ = 10e18;
        rateAsset.mint(bob, dust_ * 2);
        vm.prank(bob);
        IERC20(address(rateAsset)).transfer(address(seVault), dust_);

        uint256 Rh = seBook.reserveOfToken(address(rateAsset));
        uint256 Bh = IERC20(address(rateAsset)).balanceOf(address(seVault));
    ...
    function test_T_NEST_3_nestedI1_bookedHost_trueWithoutPushReverts() public {
        ...
        // Use rate asset rateAsset if it is a vault token on SE with R==B.
        IERC20 tokenIn_ = IERC20(address(rateAsset));
    ...
    function test_T_NEST_4_noNestedApproveOnFundPath() public {
        ...
        // Nested asset path (allowlisted rate asset → mint) if SE accepts rateAsset.
        // Fund bob with rateAsset and mint via DETF nested SE push.
        uint256 amt_ = 50e18;
        rateAsset.mint(bob, amt_);
        vm.startPrank(bob);
        IERC20(address(rateAsset)).approve(openDetf, amt_);
        // May revert UnsupportedRoute if rateAsset not allowlisted on this SE surface — then share path above is DoD.
        try openExchangeIn.exchangeIn(
            IERC20(address(rateAsset)), amt_, IERC20(openDetf), 0, bob, false, block.timestamp + 1 hours
        ) returns (uint256) {
            assertEq(
                IERC20(address(rateAsset)).allowance(openDetf, address(seVault)),
                0,
                "T-NEST-4: no rateAsset fund approve"
            );
        } catch {
