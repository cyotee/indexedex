...
/**
 * @title MultiVaultWeightedDetf_Alignment_FeeCreatorClaim
 * @notice D28 FC1–FC12 on the production MVW proxy (Open mode).
 */
contract MultiVaultWeightedDetf_Alignment_FeeCreatorClaim is TestBase_MultiVaultWeightedDetf {
    function setUp() public override {
        super.setUp();
        detf = _deployOpenModeDetf("FC MultiVault Weighted", "fcMVW");
    ...
    function test_FC1_mvw_feeToAndCreatorCanClaim() public {
    ...
    function test_FC2_mvw_claimEqualsPendingAndBalance() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        _liveMint(alice, 200e18);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC3_mvw_dueAmountsFloor() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC4_mvw_newSharesDoNotClaimOldPot() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        _liveMint(alice, 200e18);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC5_mvw_newPotAtNewWeights() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC6_mvw_secondClaimZero() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        _liveMint(alice, 200e18);
    ...
    function test_FC7_mvw_nonOwnerCannotClaim() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        _liveMint(alice, 200e18);
        IDETFNFTVault nft_ = _nft();
        vm.prank(alice);
    ...
    function test_FC8_mvw_ids1and2CannotSellOrClose() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
    ...
    function test_FC9_mvw_d2NoOriginalShares() public {
        _bootstrapViaFirstBond(alice, 1_500e18);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC10_mvw_feeToChangeDoesNotMoveId1() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        _liveMint(alice, 200e18);
    ...
    function test_FC11_mvw_creatorZeroFeeToOwnsBoth() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        _liveMint(alice, 200e18);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC12_mvw_conservationTwoWaves() public {
        _bootstrapViaFirstBond(alice, 2_000e18);
        _liveMint(alice, 200e18);
        uint256 bobShares_ = _fundSeShares0(bob, 400e18);
        vm.startPrank(bob);
        seShare0.approve(detf, bobShares_);
        detfBonding.bond(seShare0, bobShares_, DEFAULT_MIN_LOCK, bob, false, block.timestamp + 1 hours);
        vm.stopPrank();
        _liveMint(bob, 100e18);
        IDETFNFTVault nft_ = _nft();
        uint256 claimed_ = _claim(DETF_FEE_TO_BOND_NFT_ID, _feeTo())
    ...
    function _liveMint(address who_, uint256 seLp_) internal {
        uint256 shares_ = _fundSeShares0(who_, seLp_);
        vm.startPrank(who_);
        seShare0.approve(detf, shares_);
        IStandardExchangeIn(detf).exchangeIn(
            seShare0, shares_, IERC20(detf), 0, who_, false, block.timestamp + 1 hours
        );
        vm.stopPrank();
    }
}
