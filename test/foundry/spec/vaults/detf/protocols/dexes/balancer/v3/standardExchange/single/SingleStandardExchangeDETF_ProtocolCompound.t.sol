...
contract SingleStandardExchangeDETF_ProtocolCompound_Test is TestBase_SingleStandardExchangeDETF {
    ...
    ISingleStandardExchangeDETFBonding internal compoundBonding;
    ...
    function setUp() public virtual override {
        ...
        compoundInfo = ISingleStandardExchangeDETFInfo(compoundDetf);
        compoundBonding = ISingleStandardExchangeDETFBonding(compoundDetf);
    ...
    function test_C3_userClaimFreeDetfWhileLocked() public {
        ...
        if (lockedUserBondId == 0 || vault_.effectiveSharesOf(lockedUserBondId) == 0) {
            uint256 seShares_ = _fundSeShares(alice, 80e18);
            vm.startPrank(alice);
            seShare.approve(compoundDetf, seShares_);
            (lockedUserBondId,) = compoundBonding.bond(
                seShare, seShares_, DEFAULT_MIN_LOCK, alice, false, block.timestamp + 1 hours
            );
            vm.stopPrank();
        }
