...
import {
    UniswapV4StandardExchangeOrbitalDETFRepo
...
/// @notice D29 donate DN1–DN13 / DN15–DN21 on production Uni V4 Orbital DETF proxy.
contract UniswapV4StandardExchangeOrbitalDETF_ReserveDonation is
    TestBase_UniswapV4StandardExchangeOrbitalDETF
{
    ...
    function setUp() public override {
        ...
        (userBondId,) = _firstBondBothPairs(200 ether, 200 ether);
    ...
    function _mintToken() internal view returns (IERC20) {
        return IERC20(pair0);
    }
    ...
    function _donateMintToken(address from_, uint256 amount_) internal returns (uint256 lpOut_) {
        ...
        vm.startPrank(from_);
        _mintToken().approve(address(nft_), amount_);
        lpOut_ = IDetfNftReserveDonation(address(nft_)).donate(_mintToken(), amount_, 0, false, _dl());
        vm.stopPrank();
    }
    ...
    function test_N2_donate_vaultShare_credits_id0() public {
        address shareAddr_ = detfInfo.vaultShare0();
        if (shareAddr_ == address(0)) shareAddr_ = detfInfo.standardExchange0();
    ...
    function test_N3_donate_lpToken_thisCallInboundOnly() public {
        ...
        assertEq(lp_.balanceOf(address(nft_)), booked_, "N3 booked unchanged");
        vm.startPrank(donor);
    ...
    function test_N5_inert_reverts() public {
        ...
        SimpleMintableERC20(pair0).mint(donor, 1 ether);
        vm.startPrank(donor);
        IERC20(pair0).approve(address(nft_), 1 ether);
        vm.expectRevert(abi.encodeWithSignature("ReserveNotLive()"));
        IDetfNftReserveDonation(address(nft_)).donate(IERC20(pair0), 1 ether, 0, false, _dl());
        vm.stopPrank();
        assertEq(IDetfNftReserveDonation(address(nft_)).previewDonate(IERC20(pair0), 1 ether), 0, "N5 preview");
    }

    function test_N6_twoBonders_navUnchanged() public {
        address bob = makeAddr("dn6bob");
        SimpleMintableERC20(pair0).mint(bob, 80 ether);
        vm.startPrank(bob);
        IERC20(pair0).approve(detf, 80 ether);
        (uint256 bobId,) =
            detfInfo.bond(IERC20(pair0), 40 ether, DEFAULT_MIN_LOCK, bob, false, _dl());
        vm.stopPrank();
        IDETFNFTVault nft_ = _nft();
        ...
        _donateMintToken(donor, 15 ether);
    ...
    function test_N8_joinDonatedCapital_eoaReverts() public {
        ...
        vm.prank(attacker);
        vm.expectRevert(
            abi.encodeWithSelector(UniswapV4StandardExchangeOrbitalDETFRepo.NotAuthorized.selector, attacker)
        );
    ...
    function test_N9_pretransferred_noSurplus_reverts() public {
        ...
        assertEq(nft_.originalSharesOf(DETF_PROTOCOL_BOND_NFT_ID), id0Before_, "N9 lpToken");
    }
    ...
    function test_N10_previewEqualsExecute() public {
        ...
        assertApproxEqRel(lpOut_, preview_, 0.02e18, "N10 preview ~ execute");
    }
    ...
    function test_N11_ownerOnlyLiquidity_donateStillWorks() public {
        ...
        vm.stopPrank();
        assertGt(_donateMintToken(donor, 5 ether), 0, "N11 donate");
    }
    ...
    function test_N15_n10_userConvertUnchanged() public {
        ...
        _donateMintToken(donor, 11 ether);
    ...
    function test_N16_lastClose_thenDonate_nextBondDoesNotCapture() public {
        ...
        vm.prank(bob);
        detfInfo.closeBondMature(bobId, _minOut3(), bob, _dl());
        vm.prank(detfUser);
        detfInfo.closeBondMature(userBondId, _minOut3(), detfUser, _dl());
        IDETFNFTVault nft_ = _nft();
        // After last-exit the orbital book is DETF-heavy at MIN; pair0 single-sided join reverts ZeroAmount.
        // Donate the self-leg (same join last-close just used) then next-bond both pairs.
        uint256 userDetf_ = IERC20(detf).balanceOf(detfUser);
        uint256 donateAmt_ = userDetf_ / 4;
        if (donateAmt_ == 0) donateAmt_ = userDetf_;
        vm.startPrank(detfUser);
        IERC20(detf).approve(address(nft_), donateAmt_);
        IDetfNftReserveDonation(address(nft_)).donate(IERC20(detf), donateAmt_, 0, false, _dl());
        vm.stopPrank();
        ...
        SimpleMintableERC20(pair0).mint(carol, 40 ether);
        SimpleMintableERC20(pair1).mint(carol, 40 ether);
        vm.startPrank(carol);
        IERC20(pair0).approve(detf, 40 ether);
        IERC20(pair1).approve(detf, 40 ether);
        (uint256 carolId,) = detfInfo.bond(
            IERC20(pair0), 20 ether, IERC20(pair1), 20 ether, DEFAULT_MIN_LOCK, carol, false, _dl()
        );
        vm.stopPrank();
        assertGe(nft_.originalSharesOf(DETF_PROTOCOL_BOND_NFT_ID), id0AfterDonate_, "N16 id0 kept");
        uint256 carolAssets_ = nft_.convertToAssets(nft_.originalSharesOf(carolId));
        assertLt(carolAssets_, IERC20(nft_.lpToken()).balanceOf(address(nft_)), "N16 no swallow");
    }
    ...
    function test_N18_disabled_donateReverts_closeWorks() public {
        ...
        vm.warp(block.timestamp + DEFAULT_MIN_LOCK + 1);
        vm.prank(bob);
        uint256[] memory out_ = detfInfo.closeBondMature(bobId, _minOut3(), bob, _dl());
        assertGt(out_[1] + out_[2], 0, "N18 close");
    }
    ...
    function test_N19_permit2_allowance() public {
        ...
        vm.stopPrank();
        assertApproxEqRel(fromPermit_, preview_, 0.02e18, "N19 permit2 ~ preview");
    ...
    function test_N21_d2_afterDonate() public {
        _donateMintToken(donor, 13 ether);
        _assertD2();
    }
