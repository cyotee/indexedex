...
///      Same family CP mint-only harness as CP gold. Not a new hostile token type.
contract QuadHostilePairToken is SimpleMintableERC20 {
...
/**
 * @title UniswapV4Detf_Quad_Adversarial_Reentrancy
 * @notice Mint reentrancy during pair transferFrom hits IsLocked. Quad.setUp then hostile CP extra.
 * @dev reentrancy burn/bond N/A family had mint only
 */
contract UniswapV4Detf_Quad_Adversarial_Reentrancy is TestBase_UniswapV4Detf_Quad_Adversarial {
    QuadHostilePairToken internal hostilePair;
    ...
    function setUp() public override {
        ...
        hostilePair = new QuadHostilePairToken();
        ...
        pair2.approve(hostileDetf, type(uint256).max);
        IERC20(se0).approve(hostileDetf, type(uint256).max);
        IERC20(se1).approve(hostileDetf, type(uint256).max);
        IERC20(se2).approve(hostileDetf, type(uint256).max);
        hostileInfo.bond(
