...
/**
 * @title UniswapV4StandardExchangeCurveQuadStableDETF_Alignment_FeeCreatorClaim
 * @notice D28 FC1–FC12 on the production Uni V4 Curve Quad Stable DETF proxy.
 */
contract UniswapV4StandardExchangeCurveQuadStableDETF_Alignment_FeeCreatorClaim is
    TestBase_UniswapV4StandardExchangeCurveQuadStableDETF
{
    ...
    function _liveMint(address who_, uint256 pairAmt_) internal {
        _fundPair(detf, pair0, who_, pairAmt_ * 2);
        vm.startPrank(who_);
        IERC20(pair0).approve(detf, pairAmt_);
        IStandardExchangeIn(detf).exchangeIn(
            IERC20(pair0), pairAmt_, IERC20(detf), 0, who_, false, _dl()
        );
        vm.stopPrank();
    }

    function test_FC1_quad_feeToAndCreatorCanClaim() public {
        _bootstrapViaFirstBond(alice, 200 ether);
    ...
    function test_FC2_quad_claimEqualsPendingAndBalance() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        _liveMint(alice, 20 ether);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC3_quad_dueAmountsFloor() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC4_quad_newSharesDoNotClaimOldPot() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        _liveMint(alice, 20 ether);
        IDETFNFTVault nft_ = _nft();
        ...
        IUniswapV4StandardExchangeCurveQuadStableDETF(detf).depositClaim(
            IERC20(detf), freeDetf_, 0, alice, false, _dl()
        );
        vm.stopPrank();
    ...
    function test_FC5_quad_newPotAtNewWeights() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC6_quad_secondClaimZero() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        _liveMint(alice, 20 ether);
    ...
    function test_FC7_quad_nonOwnerCannotClaim() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        _liveMint(alice, 20 ether);
        IDETFNFTVault nft_ = _nft();
        vm.prank(alice);
    ...
    function test_FC8_quad_ids1and2CannotSellOrClose() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        ...
        vm.prank(feeTo_);
        vm.expectRevert();
        detfInfo.closeBondMature(DETF_FEE_TO_BOND_NFT_ID, minOut_, feeTo_, _dl());
        vm.prank(feeTo_);
        vm.expectRevert();
        detfInfo.closeBondMature(DETF_CREATOR_BOND_NFT_ID, minOut_, feeTo_, _dl());
        vm.prank(feeTo_);
        vm.expectRevert();
        detfInfo.sellPositionToDetfNft(DETF_FEE_TO_BOND_NFT_ID, feeTo_);
    }

    function test_FC9_quad_d2NoOriginalShares() public {
        _bootstrapViaFirstBond(alice, 150 ether);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC10_quad_feeToChangeDoesNotMoveId1() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        _liveMint(alice, 20 ether);
    ...
    function test_FC11_quad_creatorZeroFeeToOwnsBoth() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        _liveMint(alice, 20 ether);
        IDETFNFTVault nft_ = _nft();
    ...
    function test_FC12_quad_conservationTwoWaves() public {
        _bootstrapViaFirstBond(alice, 200 ether);
        _liveMint(alice, 20 ether);
        _laterBond(bob, 40 ether);
        _liveMint(bob, 10 ether);
        IDETFNFTVault nft_ = _nft();
        uint256 claimed_ = _claim(DETF_FEE_TO_BOND_NFT_ID, _feeTo())
