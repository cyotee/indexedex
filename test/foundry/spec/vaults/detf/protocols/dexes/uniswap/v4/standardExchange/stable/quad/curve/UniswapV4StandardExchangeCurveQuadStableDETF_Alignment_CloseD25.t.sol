...
/// @notice D25 close on Uni V4 Curve Quad.
contract UniswapV4StandardExchangeCurveQuadStableDETF_Alignment_CloseD25 is
    TestBase_UniswapV4StandardExchangeCurveQuadStableDETF
{
    ...
    function test_D25_7_onlyUserBondClose_minRejoinLpOutGt0() public {
        (uint256 tokenId,) = _firstBondOn(detf, _amts(50 ether), pair0);
        vm.warp(block.timestamp + DEFAULT_MIN_LOCK + 1);
        IDETFNFTVault nft_ = _bondNftVault(detf);
        uint256 id0Before_ = nft_.originalSharesOf(nft_.detfNFTId());
        vm.prank(detfUser);
