...
/// @notice H-CP-GV3 money paths: firstBond / mint / burn / close against Uni V3 SE.
contract UniswapV4Detf_Cp_Univ3Se is TestBase_UniswapV4Detf_Cp_Univ3Se {
    ...
    function test_fundedSe_claimPurchaseMatchesBalanceQuote() public {
        ...
        for (uint256 i; i < 2; ++i) {
            ...
            uint256 quoted = IStandardExchangeIn(address(claim)).previewExchangeIn(IERC20(detf), amount, claim);
            assertGt(quoted, 0);
            vm.startPrank(detfUser);
            IERC20(detf).approve(address(claim), amount);
            uint256 minted = IStandardExchangeIn(address(claim)).exchangeIn(
    ...
    function test_H_CP_GV3_firstBond() public {
    ...
    function test_H_CP_GV3_mint() public {
    ...
    function test_H_CP_GV3_burn() public {
        _firstBond(100 ether);
        vm.startPrank(detfUser);
    ...
    function test_H_CP_GV3_close() public {
