...
import {
    UniswapV4StandardExchangeWeightedDETFRepo
...
/// @notice D29 donate DN1–DN13 / DN15–DN21 on production Uni V4 Weighted DETF proxy.
contract UniswapV4StandardExchangeWeightedDETF_ReserveDonation is
    TestBase_UniswapV4StandardExchangeWeightedDETF
{
    ...
    function setUp() public override {
        ...
        (userBondId,) = _firstBondOn(detf, _one(200 ether), pair0);
    ...
    function _laterBond(address who_, uint256 amt_) internal returns (uint256 tokenId_) {
        SimpleMintableERC20(pair0).mint(who_, amt_ * 2);
        vm.startPrank(who_);
        IERC20(pair0).approve(detf, type(uint256).max);
        (tokenId_,) = detfInfo.bond(IERC20(pair0), amt_, DEFAULT_MIN_LOCK, who_, false, _dl());
        vm.stopPrank();
    }
    ...
    function test_N5_inert_reverts() public {
        ...
        address p0_ = IUniswapV4StandardExchangeWeightedDETF(inert_).pairToken(0);
    ...
    function test_N6_twoBonders_navUnchanged() public {
        ...
        uint256 bobId_ = _laterBond(bob, 20 ether);
        IDETFNFTVault nft_ = _nft();
        ...
        assertEq(nft_.originalSharesOf(DETF_FEE_TO_BOND_NFT_ID), 0, "N6 id1");
        assertEq(nft_.originalSharesOf(DETF_CREATOR_BOND_NFT_ID), 0, "N6 id2");
    }
    ...
    function test_N8_joinDonatedCapital_eoaReverts() public {
        ...
        vm.prank(attacker);
        vm.expectRevert(
            abi.encodeWithSelector(UniswapV4StandardExchangeWeightedDETFRepo.NotAuthorized.selector, attacker)
        );
    ...
    function test_N16_lastClose_thenDonate_nextBondDoesNotCapture() public {
        uint256 bobId_ = _laterBond(makeAddr("dn16bob"), 20 ether);
    ...
    function test_N18_disabled_donateReverts_closeWorks() public {
        uint256 bobId_ = _laterBond(makeAddr("dn18bob"), 20 ether);
        vm.prank(owner);
        ...
        assertGt(out_[detfInfo.detfBindingIndex() == 0 ? 1 : 0], 0, "N18 close");
    }
