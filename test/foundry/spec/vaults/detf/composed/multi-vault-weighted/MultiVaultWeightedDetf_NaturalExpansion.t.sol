...
contract MultiVaultWeightedDetf_NaturalExpansion_Test is TestBase_MultiVaultWeightedDetf {
    ...
    IMultiVaultWeightedDetfInfo internal expInfo;
    IMultiVaultWeightedDetfBonding internal expBonding;
    ...
    function setUp() public virtual override {
        ...
        expInfo = IMultiVaultWeightedDetfInfo(expDetf);
        expBonding = IMultiVaultWeightedDetfBonding(expDetf);
    }
    ...
    function test_E2_openModeNeverExpands() public {
        ...
        vm.startPrank(alice);
        seShare0.approve(open_, seShares_);
        IMultiVaultWeightedDetfBonding(open_).bond(
            seShare0, seShares_, DEFAULT_MIN_LOCK, alice, false, block.timestamp + 1 hours
        );
        vm.stopPrank();

        IMultiVaultWeightedDetfInfo openInfo_ = IMultiVaultWeightedDetfInfo(open_);
    ...
    function test_E3_multiBondRewardRatioMatchesShares() public {
        ...
        uint256 seSharesB_ = _fundSeShares0(bob, 80e18);
        vm.startPrank(bob);
        seShare0.approve(expDetf, seSharesB_);
        (uint256 bondB_,) = expBonding.bond(
            seShare0, seSharesB_, DEFAULT_MIN_LOCK, bob, false, block.timestamp + 1 hours
        );
        vm.stopPrank();
