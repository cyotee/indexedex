 ...
 */
contract ProtocolDETFSellNFTTest is DualSelfCommonDETFIntegrationBase {
    function test_sellNFT_transfersPrincipalShares_burnsNFT_mintsRICHIRShares() public {
        ...
        (uint256 tokenId, uint256 shares) = IBaseDualSelfCommonDETFBonding(address(detf))
        ...
        vm.prank(detfAlice);
        uint256 richirMinted = IBaseDualSelfCommonDETFBonding(address(detf)).sellNFT(tokenId, detfAlice);
