...
contract DETFNFTVaultTest is Test {
    ...
    function test_sellNFT_transferToDETFNFT() public pure {
        ...
        uint256 detfNFTSharesBefore = 500e18;

        uint256 detfNFTSharesAfter = detfNFTSharesBefore + userNFTShares;

        assertEq(detfNFTSharesAfter, 600e18, "Protocol NFT should accumulate shares");
    }
    ...
    function test_detfNFT_alwaysUnlocked() public view {
        // Protocol NFT has unlockTime = 0 (always unlocked)
        uint256 detfNFTUnlockTime = 0;

        assertTrue(block.timestamp >= detfNFTUnlockTime, "Protocol NFT should always be unlocked");
    }

    function test_detfNFT_cannotBeRedeemed() public pure {
        // Protocol NFT token ID is reserved and cannot be redeemed normally
        uint256 detfNFTId = 1; // Token ID 1 is reserved for protocol

        assertTrue(detfNFTId == 1, "Protocol NFT ID should be 1");
    }

    function test_detfNFT_accumulatesWithoutNewBond() public pure {
