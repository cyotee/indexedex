...
contract CurveQuadBufferHookNativeSYTest is TestBase_UniswapV4StandardExchangeCurveQuadStableBufferHook {
    ...
    function test_curveQuadSYMetadataNativeRateAndAllFacetSelectors() public view {
        ...
        uint256 left_ = Math.sqrt(IERC20(se0).balanceOf(hook) * token1.balanceOf(hook));
        uint256 right_ = Math.sqrt(token2.balanceOf(hook) * token3.balanceOf(hook));
        assertEq(sy_.exchangeRate(), Math.mulDiv(Math.sqrt(left_ * right_), 1e18, sy_.totalSupply()));
    ...
    function test_curveQuadSYAllPairAndShareInputsAndOutputs() public {
    ...
    function test_curveQuadStandardExactOutputLiquidityUsesRequiredAmountOnly() public {
    ...
    function test_curveQuadSYInternalSharesBurnOnlyRequestedAmountAndPaySeShares() public {
    ...
    function test_curveQuadSYLimitsAndPrepaidShareInventoryCannotCreditCaller() public {
        uint256 before_ = IERC20(se0).balanceOf(user);
    ...
    function test_curveQuadStandardSwapsRemainSupplyNeutral() public {
...
contract CurveQuadBufferHookRestrictedSYTest is TestBase_UniswapV4StandardExchangeCurveQuadStableBufferHook {
    ...
    function test_curveQuadSYKeepsExistingLiquidityOwnerRestriction() public {
        _seedFullBook(1_000 ether);
        address outsider_ = makeAddr("curve quad restricted outsider");
