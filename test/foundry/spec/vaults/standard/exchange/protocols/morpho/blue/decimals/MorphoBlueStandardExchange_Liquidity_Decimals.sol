...
/// @notice U1–U5 on a non-18 loan token.
abstract contract MorphoBlueStandardExchange_Liquidity_Decimals is
    TestBase_MorphoBlueStandardExchange_Decimals
{
    ...
    function test_U1_maxWithdraw_shrinksTowardIdle() public {
        ...
        assertLe(
            maxAfter,
            _idleOf(se) + 10 + morpho.expectedTotalSupplyAssets(marketParams)
                - morpho.expectedTotalBorrowAssets(marketParams),
            "U1 cap is cash"
        );
    }
    ...
    function test_U4_deposit_stillSucceedsWhileUtilized() public {
        _utilize();
        uint256 preview = seIn.previewExchangeIn(IERC20(address(loanToken)), _u(50), IERC20(se));
        uint256 out = _wrapExactIn(user, _u(50));
