...
abstract contract AerodromeSE_Adversarial_Decimals is TestBase_AerodromeStandardExchange_Decimals {
    ...
    function test_A1_donateToken_cannotMintFreeShares() public {
        ...
        uint256 amount_ = _testAmt(tokenA);
    ...
    function test_A2_donateSeShares_noFreeMintOrTheft() public {
        IStandardExchangeProxy vault_ = _vault();
        uint256 lpAmt_ = IERC20(address(_getPool(PoolConfig.Balanced))).balanceOf(address(this)) / 50;
    ...
    function test_A3_donateLp_cannotMintFreeShares() public {
        ...
        uint256 lpAmt_ = IERC20(address(_getPool(PoolConfig.Balanced))).balanceOf(address(this)) / 100;
    ...
    function test_E4_holderBalance_notDilutedByOthersSwap() public {
        IStandardExchangeProxy vault_ = _vault();
        (MintableERC20Decimals tokenA, MintableERC20Decimals tokenB) = _getTokens(PoolConfig.Balanced);
        uint256 victimShares_ = _mintSeShares(victim, IERC20(address(_getPool(PoolConfig.Balanced))).balanceOf(address(this)) / 80);
        assertEq(vault_.balanceOf(victim), victimShares_, "victim seeded");

        uint256 amount_ = _testAmt(tokenA) / 8;
        tokenA.mint(attacker, amount_);
        vm.startPrank(attacker);
        tokenA.approve(address(vault_), amount_);
    ...
    function test_E5_invalidRoute_unsupportedToken_reverts() public {
        ...
        uint256 amount_ = _testAmt(tokenA) / 10;
        tokenA.mint(attacker, amount_);
        vm.startPrank(attacker);
        tokenA.approve(address(vault_), amount_);
        vm.expectRevert(
    ...
    function test_H3_exchangeOut_maxInTooLow_noFreeShares() public {
        IStandardExchangeProxy vault_ = _vault();
        (MintableERC20Decimals tokenA, MintableERC20Decimals tokenB) = _getTokens(PoolConfig.Balanced);
        uint256 amountOut_ = _from18(address(tokenB), 2 ether);
        uint256 requiredIn_ =
            vault_.previewExchangeOut(IERC20(address(tokenA)), IERC20(address(tokenB)), amountOut_);
    ...
    function test_I1_pretransferred_claimedLeInventory_stillReverts() public {
        IStandardExchangeProxy vault_ = _vault();
        (MintableERC20Decimals tokenA, MintableERC20Decimals tokenB) = _getTokens(PoolConfig.Balanced);
        uint256 inventory_ = _testAmt(tokenA) / 2;
        uint256 claimed_ = _testAmt(tokenA) / 8;
        uint256 pull_ = _testAmt(tokenA) / 32;
    ...
    function test_I2_transferBeforeCall_pretransferred_revertsDelta0() public {
        IStandardExchangeProxy vault_ = _vault();
        (MintableERC20Decimals tokenA, MintableERC20Decimals tokenB) = _getTokens(PoolConfig.Balanced);
        uint256 claimed_ = _testAmt(tokenA) / 10;
    ...
    function test_I3_residualInventory_cannotFundSecondFreePretransfer() public {
        ...
        (MintableERC20Decimals tokenA, MintableERC20Decimals tokenB) = _getTokens(PoolConfig.Balanced);
        uint256 amount_ = _testAmt(tokenA) / 8;
        uint256 residualSeed_ = _testAmt(tokenA) / 16;
    ...
    function test_I_positive_honestPullSwap_succeeds() public {
        IStandardExchangeProxy vault_ = _vault();
        (MintableERC20Decimals tokenA, MintableERC20Decimals tokenB) = _getTokens(PoolConfig.Balanced);
        uint256 amountIn_ = _testAmt(tokenA) / 8;
    ...
    /* ---------------------------------------------------------------------- */
    /*  E6: fat max + transfer-only-used must not skim booked R (SEC-SE-AC-001) */
    /* ---------------------------------------------------------------------- */
    ...
    /// @notice A0: donated reserve LP blocks zap-in deposit; attacker mints nothing (SEC-SE-AC-002).
    /// @dev Donator != attacker. Must not only assert no immediate share mint (A3).
    ...
    /* ---------------------------------------------------------------------- */
    /*  I1 LP-deposit: lastTotal exact-gap must not free-mint (SEC-SHARP-004) */
    /* ---------------------------------------------------------------------- */
