...
import {
    TestBase_UniswapV4BalancerQuadStableSwapHook_Decimals
} from "contracts/hooks/uniswap/v4/stable/quad/balancer/TestBase_UniswapV4BalancerQuadStableSwapHook_Decimals.sol";
import {
...
import {RateProviderHarness} from
    "contracts/hooks/uniswap/v4/stable/quad/balancer/TestBase_UniswapV4BalancerQuadStableSwapHook.sol";

/**
 * @title UniswapV4BalancerQuadStableSwapHook_Decimals
 ...
 */
abstract contract UniswapV4BalancerQuadStableSwapHook_Decimals is
    TestBase_UniswapV4BalancerQuadStableSwapHook_Decimals
{
    ...
    function test_S0_inertBook_swapReverts() public {
        ...
        IUniswapV4BalancerQuadStableSwapHook inert = IUniswapV4BalancerQuadStableSwapHook(h);
    ...
    function test_R1_rateProvider_scales() public {
        ...
        rp.setRate(2e18);
        (IUniswapV4BalancerQuadStableSwapHook h,) = _deployWithProviders(address(rp), address(0), address(0), address(0));
    ...
    function _deployWithProviders(address p0, address p1, address p2, address p3)
        internal
        returns (IUniswapV4BalancerQuadStableSwapHook h, address hookAddr)
    {
        ...
        h = IUniswapV4BalancerQuadStableSwapHook(hookAddr);
    }
    ...
    function test_A1_reentrancy_addLiquidity() public {
        BalancerQuadHostilePull hostile = new BalancerQuadHostilePull("H", "H", _dec0());
        (address h, IUniswapV4BalancerQuadStableSwapHook q) = _deployWithHostilePull(hostile);
    ...
    function test_A1_reentrancy_removeLiquidity() public {
        BalancerQuadHostilePush hostile = new BalancerQuadHostilePush("H", "H", _dec0());
        (address h, IUniswapV4BalancerQuadStableSwapHook q) = _deployWithHostilePush(hostile);
    ...
    function _hostileBalanced(address h, uint256 human) internal view returns (uint256[4] memory amounts) {
        address[4] memory toks = IUniswapV4BalancerQuadStableSwapHook(h).tokens();
        for (uint256 i; i < 4; ++i) {
            amounts[i] = _swapAmt(toks[i], human);
        }
    }

    function _deployWithHostilePull(BalancerQuadHostilePull hostile)
        internal
    ...
    function _deployWithHostilePush(BalancerQuadHostilePush hostile)
        internal
        returns (address h, IUniswapV4BalancerQuadStableSwapHook q)
    {
        MintableERC20Decimals a = new MintableERC20Decimals("A", "A", _dec1());
...
contract BalancerQuadHostilePull {
    string public name;
...
contract BalancerQuadHostilePush {
    string public name;
    string public symbol;
    uint8 public immutable decimals;
    uint256 public totalSupply;
