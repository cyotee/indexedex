...
/**
 * @title UniswapV4SingleSEBufferHook_StagedInit_Test
 * @notice S43 bootstrap-only deploy plus one wrap-aware door / finalize matrix.
 */
contract UniswapV4SingleSEBufferHook_StagedInit_Test is TestBase {
    ...
    function test_productionFacetCuts_oneProductFacetAdd() public view {
        IDiamond.FacetCut[] memory cuts = hookPkg.productionFacetCuts();
        assertEq(cuts.length, 1);
        assertEq(cuts[0].facetAddress, address(hookPkg.PRODUCT_FACET()));
        assertEq(uint8(cuts[0].action), uint8(IDiamond.FacetCutAction.Add));
        _assertSelectorEq(cuts[0].functionSelectors, hookPkg.PRODUCT_FACET().facetFuncs());
    }
    ...
    function test_facetName_isInitFacetType() public view {
        assertEq(
            ...
            type(UniswapV4SingleStandardExchangeBufferHookInitFacet).name
        );
    }
    ...
    function test_packageName_unchanged() public view {
        assertEq(
            hookPkg.packageName(),
            type(UniswapV4SingleStandardExchangeBufferHookDFPkg).name
        );
    }
    ...
    function test_S43_deployAlone_productionSelectorsUnmatched() public {
        ...
        assertEq(loupe.facetAddress(IHook.previewWrap.selector), address(0));
        assertEq(loupe.facetAddress(IHook.currency0.selector), address(0));
        assertEq(loupe.facetAddress(IHook.pairToken.selector), address(0));
        assertEq(loupe.facetAddress(IHook.getHookPermissions.selector), address(0));
    }

    function test_S43_deployAlone_erc165_fiveIds_noIERC20() public {
        (address h,,,) = _freshBootstrap();
        bytes4 hookVaultType = bytes4(keccak256("UniswapV4SingleStandardExchangeBufferHook"));
        assertTrue(IERC165(h).supportsInterface(type(IHooks).interfaceId));
        assertTrue(IERC165(h).supportsInterface(type(IHook).interfaceId));
        assertTrue(IERC165(h).supportsInterface(type(IBasicVault).interfaceId));
        assertTrue(IERC165(h).supportsInterface(type(IStandardVault).interfaceId));
        assertTrue(IERC165(h).supportsInterface(hookVaultType));
        assertFalse(IERC165(h).supportsInterface(type(IERC20).interfaceId));
    }
    ...
    function test_deployPair_invalidPairReverts() public {
        ...
        vm.expectRevert(Common.InvalidPoolToken.selector);
        ...
        vm.expectRevert(Common.InvalidPoolToken.selector);
        init.deployPair(address(pairToken), address(0xB0B0));
    }
    ...
    function test_finalize_removesInit_addsProduction() public {
        ...
        assertEq(loupe.facetAddress(IHooks.beforeInitialize.selector), address(hookPkg.PRODUCT_FACET()));
        assertEq(loupe.facetAddress(IHooks.beforeSwap.selector), address(hookPkg.PRODUCT_FACET()));
        assertEq(loupe.facetAddress(IHook.previewWrap.selector), address(hookPkg.PRODUCT_FACET()));
        assertEq(loupe.facetAddress(IHook.currency0.selector), address(hookPkg.PRODUCT_FACET()));
        assertEq(
    ...
    function test_firstDeployerWins_unfinalized() public {
        (address tPair, address tSe) = _freshLegs();
        IUniswapV4SingleStandardExchangeBufferHookPackage.PkgArgs memory args = _pkgArgs(tSe, tPair);
    ...
    function test_J_previewWrapBeforeFinalize_unmatched() public {
        (address h,,,) = _freshBootstrap();
        assertEq(IDiamondLoupe(h).facetAddress(IHook.previewWrap.selector), address(0));
        vm.expectRevert(abi.encodeWithSelector(Proxy.NoTargetFor.selector, IHook.previewWrap.selector));
        IHook(h).previewWrap(1 ether);
    }
    ...
    function test_beforeInitialize_viewOnly_afterFinalize() public {
        (address h, IUniswapV4HookStagedPairInit init, address t0, address t1) = _freshBootstrap();
        init.deployPair(t0, t1);
        assertTrue(init.finalizeInitialization());
        PoolKey memory key = PairPoolLib.pairKey(t0, t1, 60, IHooks(h));
        ...
        vm.prank(address(pm));
        bytes4 sel = IHooks(h).beforeInitialize(address(this), key, 0);
    ...
    function test_beforeInitialize_feeZero_onBootstrapLib() public {
        ...
        badFee.fee = 3000;
        vm.prank(address(pm));
        vm.expectRevert(Common.InvalidPoolFee.selector);
    ...
    {
        (t0, t1) = _freshLegs();
        h = _deployBootstrapOnly(_pkgArgs(t1, t0));
        init = IUniswapV4HookStagedPairInit(h);
    }

    function _freshLegs() internal returns (address tPair, address tSe) {
        SimpleMintableERC20 a = new SimpleMintableERC20("PairB", "PRB");
        tPair = address(a);
        address pv = address(_deployCraneErc4626(tPair));
        tSe = _deployERC4626SE(pv);
    }

    function _pkgArgs(address se_, address pair_)
        internal
        view
