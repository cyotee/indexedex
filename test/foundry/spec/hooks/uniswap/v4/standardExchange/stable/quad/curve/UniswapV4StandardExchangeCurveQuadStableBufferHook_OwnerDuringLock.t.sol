    ...
    TestBase_UniswapV4StandardExchangeCurveQuadStableBufferHook
{
    ...
    function test_D30_ownerSwapExactIn_matchesPreview() public {
        _ownerSeedBook();
        uint256 pred = quad.previewSwapExactIn(address(token1), address(token2), 1 ether);
        assertGt(pred, 0);
        uint256 before = token2.balanceOf(owner);
        vm.prank(owner);
        uint256 out_ = quad.ownerSwapExactIn(
            address(token1), address(token2), 1 ether, 0, block.timestamp + 1 days
        );
    ...
    function test_D30_ownerSwapExactOut_matchesPreview() public {
        ...
        uint256 predIn = quad.previewSwapExactOut(address(token1), address(token2), want);
        vm.prank(owner);
        uint256 in_ = quad.ownerSwapExactOut(
            address(token1), address(token2), want, predIn, block.timestamp + 1 days
        );
        assertEq(in_, predIn);
    }
    ...
    function test_D30_thirdPartyCannotOwnerSwap() public {
        ...
        vm.prank(user);
        quad.ownerSwapExactIn(address(token1), address(token2), 1 ether, 0, block.timestamp + 1 days);
    }
    ...
    function test_D30_publicSwapStillFills() public {
        ...
        assertEq(token2.balanceOf(user) - before, pred);
    }
    ...
    function test_D89_ownerDepositSingle_atMin_lpOutGt0() public {
        ...
        vm.prank(owner);
        quad.exitProportional(lp, owner, mins, block.timestamp + 1 days);
