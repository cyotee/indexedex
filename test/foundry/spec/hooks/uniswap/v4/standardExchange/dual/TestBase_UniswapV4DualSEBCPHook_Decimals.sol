 ...
 * @notice Dual SE buffer CP hook with combo-decimal ERC-4626 underlyings. Cell ID D_U{left}_U{right}.
 * @dev Does not call gold Dual setUp (18-dec SimpleMintableERC20). vaultShare / hook LP stay 18.
 */
abstract contract TestBase_UniswapV4DualSEBCPHook_Decimals is TestBase_ERC4626StandardExchange {
    ...
    function _leftDecimals() internal pure virtual returns (uint8);
    function _rightDecimals() internal pure virtual returns (uint8);

    function _uA(uint256 human) internal pure returns (uint256) {
        return human * (10 ** uint256(_leftDecimals()));
    }

    function _uB(uint256 human) internal pure returns (uint256) {
        return human * (10 ** uint256(_rightDecimals()));
    }
    ...
    function setUp() public virtual override {
        ...
        tokenA = new MintableERC20Decimals("TokenA", "TKA", _leftDecimals());
        tokenB = new MintableERC20Decimals("TokenB", "TKB", _rightDecimals());
        ...
        hookPkg = DualFactory.deployPackage(
            ...
            abi.encode(
                type(IUniswapV4DualStandardExchangeBufferConstantProductHookPackage).name,
                "v1",
                _leftDecimals(),
                _rightDecimals()
            )._hash()
        ...
        // Fund user
        tokenA.mint(user, _uA(1_000_000));
        tokenB.mint(user, _uB(1_000_000));
        vm.startPrank(user);
    ...
    function _ensureProductDoorsAndFinalize(address hook_) internal {
        _ensureProductDoorsAndFinalize(hook_, address(tokenA), address(tokenB));
    }
    ...
    function _deployBootstrapOnly(
        ...
        uint256 mineNonce = DualFactory.findMineNonce(hookFactory, hookPkg, args);
        return DualFactory.deployHook(hookPkg, args, mineNonce);
    }
    ...
    /// @dev Kept for existing Dual specs. Constructs the live product key; does not initialize.
    ...
    /// @dev Human units of left/right underlyings, mapped onto a pool currency after address sort.
    function _humanFor(address currency, uint256 human) internal view returns (uint256) {
        return _amountForCurrency(currency, _uA(human), _uB(human));
    }
    ...
    function _userAcquireSeShares(address se, MintableERC20Decimals pairToken, uint256 pairAmt)
        internal
