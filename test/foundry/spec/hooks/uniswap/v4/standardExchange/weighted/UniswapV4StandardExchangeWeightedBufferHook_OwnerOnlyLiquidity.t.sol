...
contract UniswapV4StandardExchangeWeightedBufferHook_OwnerOnlyLiquidity_Test is
    TestBase_UniswapV4StandardExchangeWeightedBufferHook
{
    ...
    function test_ownerOnlyLiquidity_thirdPartyAddReverts() public {
        ...
        weighted.joinProportional(amounts, user, 0, block.timestamp + 1 days);
    }
    ...
    function test_ownerOnlyLiquidity_publicSwapStillWorks() public {
        ...
        amounts[1] = 500 ether;
        vm.prank(owner);
        weighted.joinProportional(amounts, owner, 0, block.timestamp + 1 days);
        uint256 preview = weighted.previewSwapExactIn(address(token0), address(token1), 1 ether);
        assertGt(preview, 0);
