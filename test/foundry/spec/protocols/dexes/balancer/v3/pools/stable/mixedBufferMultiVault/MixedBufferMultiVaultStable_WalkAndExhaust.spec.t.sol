 ...
 * @dev HostileSE is a non-SUT harness. SUT remains production pool/pkg/registry. Layout U=1 free + buffer + N=2.
 */
contract MixedBufferMultiVaultStable_WalkAndExhaust is TestBase_MixedBufferMultiVaultStablePool {
    ...
    function test_M11_walk_deposit_secondVaultSucceeds() public {
        ...
        dai.mint(address(h0), MBMVS_INIT_BUFFER * 5);
        dai.mint(alice, MBMVS_INIT_BUFFER * 5);
        mintSharesForVault(0, alice, MBMVS_INIT_SHARES * 5);
        usdc.mint(alice, MBMVS_INIT_UNPAIRED * 5);
    ...
    function test_M11_allVaultsExhausted_selector() public {
        ...
        dai.mint(address(h0), MBMVS_INIT_BUFFER * 3);
        dai.mint(address(h1), MBMVS_INIT_BUFFER * 3);
        usdc.mint(alice, MBMVS_INIT_UNPAIRED * 3);
    ...
    function test_M11_preSeat_allVaultsExhausted_selector() public {
        ...
        dai.mint(address(h0), MBMVS_INIT_BUFFER * 5);
        dai.mint(address(h1), MBMVS_INIT_BUFFER * 5);
        usdc.mint(alice, MBMVS_INIT_UNPAIRED * 5);
    ...
    function _initTwoVaultPool(address pool, address share0, address share1) internal {
        ...
        for (uint256 i; i < toks.length; ++i) {
            if (address(toks[i]) == address(dai)) amounts[i] = MBMVS_INIT_BUFFER;
            else if (address(toks[i]) == address(usdc)) amounts[i] = MBMVS_INIT_UNPAIRED;
