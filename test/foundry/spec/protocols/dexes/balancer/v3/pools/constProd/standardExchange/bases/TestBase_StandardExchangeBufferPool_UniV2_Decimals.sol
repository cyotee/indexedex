...
import {
    TestBase_StandardExchangeBufferPool_UniswapV2_Decimals
} from "test/foundry/spec/protocols/dexes/balancer/v3/pools/constProd/standardExchange/uniswapV2/bases/TestBase_StandardExchangeBufferPool_UniswapV2_Decimals.sol";

/**
 * @title TestBase_StandardExchangeBufferPool_UniV2
 * @notice Thin extension of `TestBase_StandardExchangeBufferPool_UniswapV2_Decimals` that adds the
 ...
 *      unchanged from `TestBase_StandardExchangeBufferPool_UniswapV2_Decimals` (which itself extends
 ...
 */
abstract contract TestBase_StandardExchangeBufferPool_UniV2_Decimals is TestBase_StandardExchangeBufferPool_UniswapV2_Decimals {
    /* ---------------------------------------------------------------------- */
    ...
    function _shiftRateUp() internal returns (uint256 newRate) {
        ...
        uint256 amountIn = tta.balanceOf(address(uniV2DaiUsdcPair)) / 20;
        if (amountIn == 0) {
            mintTTA(address(this), _from18(address(tta), 1_000_000e18));
    ...
    function _mintTestToken(IERC20 token, address to, uint256 amount) internal {
        if (address(token) == address(pairToken)) {
            pairToken.mint(to, amount);
        } else if (address(token) == address(rateAsset)) {
            rateAsset.mint(to, amount);
        } else {
        ...
        returns (PoolSwapParams memory params, uint256 idxOut)
    {
        ...
        uint256 givenScaled18 = address(tokenIn) == address(tta) ? ttaScaled18(amountIn) : amountIn;
        uint256[] memory bal = bv3Vault.getCurrentLiveBalances(bufferPool);
        params = PoolSwapParams({
            kind: SwapKind.EXACT_IN,
            amountGivenScaled18: givenScaled18,
    ...
    function _sharesOutAtNav(uint256 dx, uint256 rate) internal view returns (uint256) {
        (uint256[] memory scalingFactors,) = bv3Vault.getPoolTokenRates(bufferPool);
        uint256 scalingFactor = scalingFactors[_sharesIdx()];
        // `dx` is native TTA. onSwap quotes in Vault scaled18 = raw * 10^(18-decimals).
        return Math.mulDiv(ttaScaled18(dx), 1e18, scalingFactor * rate);
    }
