...
import {TestBase_StandardExchangeBufferPool_UniV2_Decimals} from "test/foundry/spec/protocols/dexes/balancer/v3/pools/constProd/standardExchange/bases/TestBase_StandardExchangeBufferPool_UniV2_Decimals.sol";
 ...
 *      fixture's timeframe).
 */
abstract contract StandardExchangeBufferPool_RateTrackingTest_Decimals is TestBase_StandardExchangeBufferPool_UniV2_Decimals {
    ...
    function test_initialQuote_isNav() public {
        ...
        uint256 dx = _from18(address(tta), vt / 100);
        if (dx == 0) dx = 1;
    ...
    function test_extremeRateDrift_revertsEffectiveWeightOutOfBounds() public {
        ...
        uint256 vtGuard = IStandardExchangeBufferPool(bufferPool).virtualTTA();
        uint256 dxGuard = _from18(address(tta), vtGuard / 100);
        if (dxGuard == 0) dxGuard = 1;
        (PoolSwapParams memory params,) = _buildQuoteParams(tta, shareToken(), dxGuard);
    ...
    function test_ttaToShares_afterUpwardNavShift_succeeds() public {
        uint256 v2In = tta.balanceOf(address(uniV2DaiUsdcPair)) / 10;
        if (v2In == 0) v2In = _from18(address(tta), 1_000e18);
        _swapThroughV2Pair(tta, ttb, v2In);
        uint256 dx = _payableTtaSwapRaw(0.29e18);
    ...
    function test_ttaToSharesExactOut_nearMaxOutRatio_succeeds() public {
        uint256 sharesOut = _exactOutSharesNearMax();
        ...
        uint256 maxIn = _exactOutMaxTtaIn();
        uint256 amountIn = _swapThroughBalancerVaultExactOut(tta, shareToken(), sharesOut, maxIn);
    ...
    function _exactOutMaxTtaIn() internal view returns (uint256 maxIn) {
        (,, uint256[] memory raw,) = bv3Vault.getPoolTokenInfo(bufferPool);
        uint256 ttaRaw = raw[_ttaIdx()];
        maxIn = ttaRaw;
        if (maxIn == 0) maxIn = 1;
    }

    /// @dev Exact-out shares near WeightedMath MaxOutRatio, capped so required TTA in
    ///      cannot exceed Vault live TTA (virtualTTA is scaled18 and can dwarf raw TTA).
    function _exactOutSharesNearMax() internal returns (uint256 sharesOut) {
        ...
        sharesOut = (maxOutRatioRaw * 95) / 100;
        uint256 affordable = _quoteSwapExactIn(tta, shareToken(), _smallTtaSwapRaw());
        if (affordable > 0 && sharesOut > affordable) sharesOut = affordable;
        if (sharesOut == 0) sharesOut = 1;
    }

    function _smallTtaSwapRaw() internal view returns (uint256 dx) {
        dx = _payableTtaSwapRaw(0.01e18);
    }

    /// @dev `virtualFracWad` of virtualTTA, converted to native TTA and capped at 25% of
    ///      Vault live raw TTA so `sendTo` cannot underflow after a rate shift.
    function _payableTtaSwapRaw(uint256 virtualFracWad) internal view returns (uint256 dx) {
        uint256 vt = IStandardExchangeBufferPool(bufferPool).virtualTTA();
        dx = _from18(address(tta), Math.mulDiv(vt, virtualFracWad, 1e18));
        (,, uint256[] memory raw,) = bv3Vault.getPoolTokenInfo(bufferPool);
        uint256 ttaRaw = raw[_ttaIdx()];
        uint256 liveCap = ttaRaw / 4;
        if (liveCap == 0) liveCap = ttaRaw;
        if (ttaRaw > 0 && dx > liveCap) dx = liveCap;
        if (dx == 0) dx = 1;
    }
