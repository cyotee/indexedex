 ...
 */
abstract contract TestBase_MixedBufferMultiVaultStablePool_Decimals is TestBase_StandardExchangeBufferPool_Decimals {
    ...
    function _deployExtraDaiSeVault(uint8 idx) internal {
        ...
        uint256 amtA = _from18(address(pairToken), AERODROME_INIT_AMOUNT);
        uint256 amtB = _from18(tokenB, AERODROME_INIT_AMOUNT);
        pairToken.mint(lp, amtA);
        _mintToken(tokenB, lp, amtB);
        vm.startPrank(lp);
        pairToken.approve(address(aeroRouter), amtA);
    ...
    function _initPool() internal virtual override {
        ...
        uint256 bufAmt = _from18(address(buffer), MBMVS_INIT_BUFFER);
        uint256 shareSeed = MBMVS_INIT_SHARES;

        for (uint8 i; i < n; ++i) {
            mintSharesForVault(i, alice, shareSeed * 3);
        }
        _mintToken(address(buffer), alice, bufAmt * 4);
        for (uint8 i; i < u; ++i) {
            address ut = address(_unpairedTokenAt(i));
            _mintToken(ut, alice, _from18(ut, MBMVS_INIT_UNPAIRED) * 2);
        }
        ...
        for (uint256 t; t < poolTokens.length; ++t) {
            ...
            if (kind == IMixedBufferMultiVaultStablePool.TokenKind.Unpaired) {
                amounts[t] = _from18(address(poolTokens[t]), MBMVS_INIT_UNPAIRED);
