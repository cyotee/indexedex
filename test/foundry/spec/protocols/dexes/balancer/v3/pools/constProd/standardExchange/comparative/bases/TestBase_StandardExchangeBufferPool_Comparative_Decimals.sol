 ...
 */
abstract contract TestBase_StandardExchangeBufferPool_Comparative_Decimals is
    TestBase_StandardExchangeBufferPool_UniswapV2_Decimals
{
    ...
    function _referenceInitAmounts() internal view returns (uint256 rawTTA, uint256 rawShares) {
        ...
        // virtualTTA is Vault scaled18 (raw * 10^(18-decimals)). Reference initialize takes raw.
        rawTTA = _from18(address(tta), IStandardExchangeBufferPool(bufferPool).virtualTTA());
    ...
    function _initReferencePool() internal virtual {
        ...
        vm.startPrank(alice);
        IERC20(address(tta)).approve(address(router), type(uint256).max);
    ...
    function tradeUnderlyingV2(uint256 daiIn) public returns (uint256 rateBefore, uint256 rateAfter) {
        ...
        uint256 pairReserve = pairToken.balanceOf(address(uniV2DaiUsdcPair));
        // ~0.5% of the V2 book: large enough that the rate moves, small enough that
        // buffer vs reference stay inside REL_TOL after the weight shift.
        uint256 minTrade = pairReserve / 200;
        if (daiIn < minTrade) daiIn = minTrade;
        if (daiIn > pairReserve / 2) daiIn = pairReserve / 3;
        if (daiIn == 0) daiIn = _from18(address(pairToken), 1_000e18);
        pairToken.mint(trader, daiIn);

        address[] memory path = new address[](2);
        path[0] = address(pairToken);
        path[1] = address(rateAsset);

        vm.startPrank(trader);
        pairToken.approve(address(uniV2Router), daiIn);
