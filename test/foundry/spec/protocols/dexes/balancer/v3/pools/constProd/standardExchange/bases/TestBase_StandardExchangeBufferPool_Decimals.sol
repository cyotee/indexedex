...
abstract contract TestBase_StandardExchangeBufferPool_Decimals is TestBase_BalancerV3Vault, IndexedexTest {
    ...
    IFacet internal aeroExchangeOutQueryFacet;
    ...
    function _from18(address token, uint256 wad) internal view returns (uint256 raw) {
        if (wad == 0) return 0;
        uint8 d = 18;
        try MintableERC20Decimals(token).decimals() returns (uint8 got) {
            d = got;
        } catch {}
        if (d == 18) return wad;
    ...
    /// @dev Newly mined combo tokens are not in Crane `tokens[]`, so Balancer setUp never
    ///      Permit2-approves them. Router.initialize / swaps then revert AllowanceExpired(0).
    function _approvePermit2ForToken(address token) internal {
        for (uint256 i = 0; i < users.length; ++i) {
            vm.startPrank(users[i]);
            IERC20(token).approve(address(permit2), type(uint256).max);
            permit2.approve(token, address(router), type(uint160).max, type(uint48).max);
            if (address(bufferRouter) != address(0)) {
                permit2.approve(token, address(bufferRouter), type(uint160).max, type(uint48).max);
            }
            if (address(batchRouter) != address(0)) {
                permit2.approve(token, address(batchRouter), type(uint160).max, type(uint48).max);
            }
            if (address(compositeLiquidityRouter) != address(0)) {
    ...
    function _deploySeVaultFacets() internal virtual {
        ...
        aeroExchangeOutQueryFacet = create3Factory.deployAerodromeStandardExchangeOutQueryFacet();
    }
    ...
    function _deploySeVaultPkg() internal virtual {
        vm.startPrank(owner);
        {
            ...
            pkgInit_.aerodromeStandardExchangeInFacet = aeroExchangeInFacet;

            pkgInit_.aerodromeStandardExchangeOutFacet = aeroExchangeOutFacet;

            pkgInit_.aerodromeStandardExchangeOutQueryFacet = aeroExchangeOutQueryFacet;
            ...
            pkgInit_.aerodromeRouter = IRouter(address(aeroRouter));

            pkgInit_.aerodromePoolFactory = aeroPoolFactory;

            aeroStdExDFPkg = indexedexManager.deployAerodromeStandardExchangeDFPkg(pkgInit_);
    ...
    function _seedAerodromeLiquidity() internal virtual {
        // Raw WAD on both legs, matching `_initPool`. Human-scaled seed + WAD-raw init
        // skewed mixed-decimal prices and minted too few SE shares for a 10e18 swap
        // (WeightedMath MaxInRatio). Raw 1:1 keeps LP/shares WAD-sized for every combo.
        uint256 initPair = AERODROME_INIT_AMOUNT;
        uint256 initRate = AERODROME_INIT_AMOUNT;
    ...
    function mintTTA(address recipient, uint256 amount) public {
        pairToken.mint(recipient, amount);
    }

    /// @dev Vault scaled18 for STANDARD TTA: raw * 10^(18-decimals). virtualTTA uses this unit.
    function ttaScaled18(uint256 raw) public view returns (uint256) {
        uint8 d = MintableERC20Decimals(address(tta)).decimals();
        if (d >= 18) return raw;
        return raw * (10 ** (18 - uint256(d)));
    }
    ...
    function mintShares(address recipient, uint256 daiAmount) public virtual returns (uint256 sharesOut) {
        ...
        pairToken.approve(address(aeroRouter), daiAmount);
        rateAsset.approve(address(aeroRouter), daiAmount);
    ...
    function _initPool() internal virtual {
        ...
        // Use WAD-sized native deposits so SE shares (always 18) match Balancer's scaled TTA.
        ...
        vm.startPrank(alice);
        pairToken.approve(address(aeroRouter), lpPair);
        rateAsset.approve(address(aeroRouter), lpRate);
        (,, uint256 lpOut) = aeroRouter.addLiquidity(
            address(pairToken),
            address(rateAsset),
            false,
