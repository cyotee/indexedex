...
abstract contract TestBase_CommonBufferMultiVaultWeightedPool is TestBase_StandardExchangeBufferPool {
    ...
    function _initPool() internal virtual override {
        ...
        _mintToken(address(buffer), alice, CBMV_INIT_BUFFER * 4);
        for (uint8 i; i < u; ++i) {
            _mintToken(address(_unpairedTokenAt(i)), alice, CBMV_INIT_UNPAIRED * 2);
        }
    ...
    function _lpAndDepositInner(
        ...
        (,, uint256 lpOut) =
            aeroRouter.addLiquidity(tokenA, tokenB, false, amt, amt, 1, 1, recipient, block.timestamp + 1 hours);
