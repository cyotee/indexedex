...
import {TestBase_StandardExchangeBufferPool_UniswapV2_Decimals} from
    "test/foundry/spec/protocols/dexes/balancer/v3/pools/constProd/standardExchange/uniswapV2/bases/TestBase_StandardExchangeBufferPool_UniswapV2_Decimals.sol";
 ...
 */
abstract contract StandardExchangeBufferPool_UniswapV2_Spec_Decimals is TestBase_StandardExchangeBufferPool_UniswapV2_Decimals, Behavior_StandardExchangeBufferPool_Registration, Behavior_StandardExchangeBufferPool_Initialization, Behavior_StandardExchangeBufferPool_Swap_TTAtoShares, Behavior_StandardExchangeBufferPool_Swap_SharesToTTA, Behavior_StandardExchangeBufferPool_LP_AddProportional, Behavior_StandardExchangeBufferPool_LP_RemoveProportional, Behavior_StandardExchangeBufferPool_LP_AddUnbalanced, Behavior_StandardExchangeBufferPool_Clamping, Behavior_StandardExchangeBufferPool_Errors, Behavior_StandardExchangeBufferPool_Adversarial {
    /* ---------------------------------------------------------------------- */
    ...
    /// @dev Cap at 10% of virtualTTA (scaled18 → native) so mixed-decimal Uni V2 fixtures
    ///      stay inside WeightedMath MaxInRatio (30%). P18_R6/R9 seed 1:1 raw reserves and
    ///      the 6/9-dec quote leg makes TTA-equivalent depth smaller than 10e18.
    function test_swap_TTAtoShares_basic() public {
        uint256 want = _from18(address(tta), 10e18);
        uint256 cap = _from18(address(tta), IStandardExchangeBufferPool(bufferPool).virtualTTA() / 10);
        if (cap == 0) cap = 1;
        if (want > cap) want = cap;
        behavior_swap_TTAtoShares_endToEnd(want);
    }
    ...
    function test_swap_sharesToTTA_basic() public {
        mintShares(this.getAlice(), 10e18);
    ...
    function test_lpAdd_unbalanced_bothTokens_basic() public {
        behavior_lpAdd_unbalanced_bothTokensUnequal(_from18(address(tta), 5e18), 10e18);
    }
