...
/// @notice Stage 3 univ2-se proofs on combo decimals. pairToken = tokenA; other = tokenB.
abstract contract UniswapV2StandardExchange_SecRemediation_Decimals is TestBase_UniswapV2StandardExchange_Decimals {
    ...
    /// @dev pairToken inventory/syncPull are `_uA`; amountOut is `_uB(1)` of the other token.
    function test_E6_exchangeOut_swap_inflatedMax_pretransferred_noExtraTransfer_noInventorySkim() public {
        ...
        uint256 syncPull_ = _uA(100);
        uniswapBalancedTokenA.mint(address(vault_), inventory_);
        uniswapBalancedTokenA.mint(attacker, syncPull_);
        vm.startPrank(attacker);
        uniswapBalancedTokenA.approve(address(vault_), syncPull_);
        vault_.exchangeIn(
            IERC20(address(uniswapBalancedTokenA)),
            syncPull_,
            IERC20(address(uniswapBalancedTokenB)),
            0,
            attacker,
            false,
            _deadline()
        ...
        uint256 bookedR_ = uniswapBalancedTokenA.balanceOf(address(vault_));
        ...
        uint256 used_ = vault_.previewExchangeOut(
            IERC20(address(uniswapBalancedTokenA)), IERC20(address(uniswapBalancedTokenB)), amountOut_
        );
    ...
    function test_A0_donateLp_thenZapInDeposit_cannotRedeemDonation() public {
        ...
        uint256 donate_ = lp_.balanceOf(address(this)) / 500;
        require(lp_.balanceOf(address(this)) >= donate_ && donate_ > 0, "A0 LP inventory");
    ...
    function test_A0_emptyVault_residualLp_firstMinter_noDrain() public {
        ...
        address donator_ = makeAddr("lpDonator");
        uint256 donate_ = lp_.balanceOf(address(this)) / 500;
        uint256 attackerLp_ = lp_.balanceOf(address(this)) / 500;
        require(lp_.balanceOf(address(this)) >= donate_ + attackerLp_ && donate_ > 0, "A0 LP inventory");
        ...
        uint256 lpOut_ = vault_.exchangeIn(IERC20(address(vault_)), shares_, lp_, 0, attacker, false, _deadline());
        vm.stopPrank();
    ...
    function test_I1_lpDeposit_pretransferredFalse_existingLpGap_doesNotMint() public {
        ...
        uint256 gap_ = lp_.balanceOf(address(this)) / 500;
        require(lp_.balanceOf(address(this)) >= gap_ && gap_ > 0, "I1 LP inventory");
    ...
    function test_I1_lpDeposit_pretransferredTrue_bookedInventory_noTransfer_reverts() public {
        ...
        uint256 seed_ = lp_.balanceOf(address(this)) / 500;
        require(lp_.balanceOf(address(this)) >= seed_ && seed_ > 0, "I1 LP seed");
    ...
    function test_CROPS_disabled_still_allows_exchangeOut() public {
        ...
        uint256 usedExec_ = vault_.exchangeOut(tokenIn, used_, tokenOut, amountOut_, attacker, false, _deadline());
        vm.stopPrank();
