...
/**
 * @title AerodromeStandardExchangeOut_Swap_Decimals
 ...
 */
abstract contract AerodromeStandardExchangeOut_Swap_Decimals is TestBase_AerodromeStandardExchange_Decimals {
    address caller;
    ...
    function testFuzz_exchangeOut_swap_refundInvariant(uint256 surplus) public {
        IStandardExchangeProxy vault = _getVault(PoolConfig.Balanced);
        (MintableERC20Decimals tokenA, MintableERC20Decimals tokenB) = _getTokens(PoolConfig.Balanced);
        surplus = bound(surplus, 1, _testAmt(tokenA));
