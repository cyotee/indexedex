...
/**
 * @title AerodromeStandardExchange_FeeCompound_Decimals
 * @notice Tests for Task 10: Aerodrome Fee Compounding
 * @dev Tests all user stories from US-10.1 through US-10.8
 */
abstract contract AerodromeStandardExchange_FeeCompound_Decimals is TestBase_AerodromeStandardExchange_Decimals {
    // Vault instance for testing
    IStandardExchange vault;
    IPool pool;

    // Test amounts
    uint256 constant DEPOSIT_AMOUNT = 0; // unused; amounts via _uA/_uB
    ...
    function setUp() public virtual override {
        ...
        aeroBalancedTokenA.mint(address(this), _uA(1000));
        aeroBalancedTokenB.mint(address(this), _uB(1000));
    }

    function _getOwnLPTokens() internal {
        // Provide liquidity to get LP tokens for this test contract
        aeroBalancedTokenA.approve(address(aerodromeRouter), _uA(100));
        aeroBalancedTokenB.approve(address(aerodromeRouter), _uB(100));
    ...
    function _generateFees() internal {
        ...
        aeroBalancedTokenA.approve(address(aerodromeRouter), _uA(1));
        ...
        aerodromeRouter.swapExactTokensForTokens(_uA(1), 0, routes, address(this), block.timestamp + 1);
    ...
    function test_US10_1_VaultDepositsAccumulateFees() public {
        ...
        aeroBalancedTokenA.approve(address(vault), _uA(1) / 10);
        vault.exchangeIn(
    ...
    function test_US10_1_FeesClaimedOnDeposit() public {
        ...
        aeroBalancedTokenA.approve(address(vault), _uA(1));
        vault.exchangeIn(
            IERC20(address(aeroBalancedTokenA)),
            _uA(1),
            IERC20(address(vault)),
    ...
    function test_US10_2_FeesDepositedProportionally() public {
        ...
        // The increase should be more than just the 1 human pairToken deposit (due to compound)
    ...
    function test_US10_7_PreviewAccountsForPendingCompound() public {
        ...
        uint256 previewBefore =
            vault.previewExchangeIn(IERC20(address(aeroBalancedTokenA)), _uA(1), IERC20(address(vault)));
    ...
    function test_US10_8_SmallAmountsHeldAsDust() public {
        ...
        aeroBalancedTokenA.approve(address(aerodromeRouter), _uA(1) / 100);
        ...
        aerodromeRouter.swapExactTokensForTokens(_uA(1) / 100, 0, routes, address(this), block.timestamp + 1);
