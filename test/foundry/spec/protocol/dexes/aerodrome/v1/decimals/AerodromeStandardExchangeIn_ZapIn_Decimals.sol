...
/**
 * @title AerodromeStandardExchangeIn_ZapIn_Decimals
 * @notice Tests for Route 2: Token to LP token zap.
 * @dev Route 2 handles zaps where tokenIn is a pool constituent and tokenOut is the LP token.
 *      Swaps half of tokenIn for the other pool token, then adds liquidity.
 */
abstract contract AerodromeStandardExchangeIn_ZapIn_Decimals is TestBase_AerodromeStandardExchange_Decimals {
    /* ---------------------------------------------------------------------- */
    ...
    function _test_execVsPreview(PoolConfig config, bool useTokenA) internal {
        ...
        MintableERC20Decimals tokenInStub = useTokenA ? tokenA : tokenB;
        IERC20 tokenIn = IERC20(address(tokenInStub));
        IERC20 lpToken = IERC20(address(pool));

        uint256 amountIn = _testAmt(tokenInStub);
    ...
    function test_Route2ZapIn_slippageProtection_exactMinimum() public {
        ...
        uint256 amountIn = _testAmt(tokenA);
        address recipient = makeAddr("recipient");
    ...
    function test_Route2ZapIn_reserveImpact_balanced() public {
        ...
        // - Input token reserve (token1/tokenA) should increase (net deposit)
        // - Opposing token reserve (token0/tokenB) stays approximately the same
        //   because it's swapped out then deposited back
        // tokenA = token1, tokenB = token0
        bool tokenAIs0 = address(tokenA) == pool.token0();
        if (tokenAIs0) {
            assertTrue(reserve0After > reserve0Before, "input token reserve increases");
            assertTrue(reserve1After >= reserve1Before, "other reserve should not decrease");
        } else {
            assertTrue(reserve1After > reserve1Before, "input token reserve increases");
            assertTrue(reserve0After >= reserve0Before, "other reserve should not decrease");
        }
    }
    ...
    function _boundZapAmount(IPool pool, MintableERC20Decimals tokenIn, uint256 amountIn)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 minIn = 10 ** uint256(tokenIn.decimals());
        if (minIn < MIN_TEST_AMOUNT) minIn = MIN_TEST_AMOUNT;
        uint256 maxIn = reserveIn / 10;
        if (maxIn <= minIn) {
            if (reserveIn <= minIn + 1) return 0;
            maxIn = reserveIn / 2;
            if (maxIn <= minIn) maxIn = reserveIn - 1;
        }
        if (maxIn <= minIn) return 0;
        return bound(amountIn, minIn, maxIn);
    }
