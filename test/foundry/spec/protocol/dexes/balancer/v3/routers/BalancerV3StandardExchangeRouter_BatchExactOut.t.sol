...
/**
 * @title BalancerV3StandardExchangeRouter_BatchExactOut_Test
 * @notice Tests for the BatchExactOut facet functions.
 * @dev Batch router allows multi-step swaps specifying exact output amounts.
 */
contract BalancerV3StandardExchangeRouter_BatchExactOut_Test is TestBase_BalancerV3StandardExchangeRouter {
    ...
    function test_swapExactOut_singlePath_directSwap() public {
        ...
        dai.transfer(address(vault), MAX_AMOUNT_IN);
        ...
        assertGe(usdcBalAfter - usdcBalBefore, SWAP_AMOUNT, "Should receive at least exact USDC");

        // Verify results
        assertEq(pathAmountsIn.length, 1, "Should have 1 path amount in");
        assertGt(pathAmountsIn[0], 0, "Path should have input");
        assertLe(pathAmountsIn[0], MAX_AMOUNT_IN, "Should not exceed max amount in");
    }

    function test_swapExactOut_singlePath_queryVsExec() public {
        // Mint DAI to alice
        dai.mint(alice, MAX_AMOUNT_IN);
        ...
        // `querySwapExactOut` executes a real path during `vault.quote`, including token movement.
        ...
        assertApproxEqAbs(actualAmountsIn[0], expectedAmountsIn[0], 1, "Query vs exec mismatch");
    }

    function test_swapExactOut_twoSteps_withStrategyVaultWithdrawal_thenSwap_queryVsExec() public {
        uint256 shareBalance = _depositToVault(alice, MAX_AMOUNT_IN);
        ...
        paths[0] = IBalancerV3StandardExchangeBatchRouterExactOut.SESwapPathExactAmountOut({
            tokenIn: IERC20(address(daiUsdcVault)), steps: steps, maxAmountIn: maxSharesIn, exactAmountOut: SWAP_AMOUNT
        });

        // `vault.quote` executes real operations; snapshot to avoid contaminating state for exec comparison.
    ...
    function test_swapExactOut_multiplePaths() public {
        ...
        dai.transfer(address(vault), MAX_AMOUNT_IN);
        usdc.transfer(address(vault), MAX_AMOUNT_IN);
        ...
        assertGt(pathAmountsIn[0], 0, "Path 1 should have input");
        assertGt(pathAmountsIn[1], 0, "Path 2 should have input");
    }
    ...
    function test_swapExactOut_slippage_reverts() public {
        // Mint DAI to alice
        dai.mint(alice, MAX_AMOUNT_IN);
    ...
    function test_swapExactOut_deadline_reverts() public {
        // Mint DAI to alice
        dai.mint(alice, MAX_AMOUNT_IN);
    ...
    function test_swapExactOut_routerNoRetention() public {
        // Mint DAI to alice
        dai.mint(alice, MAX_AMOUNT_IN);
