...
contract BaseDualSelfCommonDETFDFPkg_Deploy_Test is TestBase_BalancerV3StandardExchangeRouter {
    ...
    function _deployProtocolPkgs() internal {
        ...
        IBaseDualSelfCommonDETFDFPkg.PkgInit memory detfPkgInit =
            BaseDualSelfCommonDETF_Component_FactoryService.buildProtocolDETFPkgInit(facets, infra, pkgs, bridgeConfig);

        protocolDETFDFPkg = IVaultRegistryDeployment(address(indexedexManager)).deployBaseDualSelfCommonDETFDFPkg(detfPkgInit);
        vm.stopPrank();

        assertGt(address(protocolDETFDFPkg).code.length, 0, "BaseDualSelfCommonDETFDFPkg not deployed");
    }
    ...
    function test_processArgs_reverts_whenNotRegistry() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IBaseDualSelfCommonDETFDFPkg.NotCalledByRegistry.selector, address(this)));
