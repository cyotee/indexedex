...
/// @notice Registry-deployed loop E2E on each two-token combo. pairToken = tokenA.
abstract contract AaveCrossVersionLoopE2E_Decimals is TestBase_AaveCrossVersionLoopV3Market_Decimals {
    ...
    function test_deploy_through_registry_and_operate() public {
        ...
        IStandardExchangeOut(vault).exchangeOut(
            IERC20(vault), type(uint256).max, tokenA, want, address(this), false, block.timestamp
        );
        assertEq(tokenA.balanceOf(address(this)) - balBefore, want, "withdrew tokenA via diamond");
    }

    function test_deployVault_marker_exposes_pair_and_sources() public {
        _deployVaultThroughRegistry();
        assertEq(address(IMarkerDecimals(vault).tokenA()), address(tokenA), "marker tokenA");
        assertEq(address(IMarkerDecimals(vault).tokenB()), address(tokenB), "marker tokenB");
        assertEq(IMarkerDecimals(vault).aaveV36Pool(), address(v36Pool), "marker V3 pool");
        assertEq(IMarkerDecimals(vault).aaveV4Spoke(), address(v4Spoke), "marker V4 spoke");
    }
}
