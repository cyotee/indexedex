...
contract _LoopVaultDecimals is AaveCrossVersionLoopExchangeInTarget, AaveCrossVersionLoopExchangeOutTarget {
...
/// @notice exchangeOut money path on each two-token combo. pairToken = tokenA.
abstract contract AaveCrossVersionLoopExchangeOut_Decimals is TestBase_AaveCrossVersionLoopV3Market_Decimals {
    _LoopVaultDecimals internal vault;
    address internal v3lp = address(0x3133);
    address internal v4lp = address(0x4144);

    function _initVault() internal {
        vault = new _LoopVaultDecimals();
    ...
    function test_exchangeOut_partial_withdraw_burns_shares_and_delivers() public {
        ...
        uint256 shares = _deposit(_uA(100));
        ...
        assertGt(hf, 1e18, "vault V3 HF > 1 after withdraw");
    }
    ...
    function test_exchangeOut_exceeding_freeable_reverts() public {
        ...
        _deposit(_uA(100));
        vm.expectRevert();
        vault.previewExchangeOut(IERC20(address(vault)), tokenA, _uA(10_000));
    }
}
