...
/**
 * @title AerodromeFork_ZapOutWithdraw_Test
 * @notice Fork tests for Route 7: Vault shares to single token via withdraw + zap.
 * @dev Tests the complete flow: vault shares -> LP -> token against live Aerodrome.
 */
contract AerodromeFork_ZapOutWithdraw_Test is TestBase_AerodromeFork {
    /* ---------------------------------------------------------------------- */
    /*                              Setup Helper                              */
    /* ---------------------------------------------------------------------- */
    ...
    function test_Route7ZapOutWithdraw_pretransferred_true() public {
        ...
        uint256 tokenOut = vault.exchangeIn(
            IERC20(address(vault)), sharesToWithdraw, IERC20(address(tokenA)), 0, recipient, true, _deadline()
        );
