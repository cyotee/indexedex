...
/**
 * @title AerodromeFork_ZapInDeposit_Test
 * @notice Fork tests for Route 6: Token to vault shares via zap + deposit.
 * @dev Tests the complete flow: token -> LP -> vault shares against live Aerodrome.
 */
contract AerodromeFork_ZapInDeposit_Test is TestBase_AerodromeFork {
    /* ---------------------------------------------------------------------- */
    ...
    function test_Route6ZapInDeposit_slippageProtection_exactMinimum() public {
        ...
        uint256 sharesOut = vault.exchangeIn(
            IERC20(address(tokenA)), amountIn, IERC20(address(vault)), preview, recipient, false, _deadline()
        );
    ...
    function test_Route6ZapInDeposit_slippageProtection_reverts_whenMinimumTooHigh() public {
        ...
        vm.expectRevert();
        vault.exchangeIn(
            IERC20(address(tokenA)), amountIn, IERC20(address(vault)), preview + 1, recipient, false, _deadline()
        );
    }
    ...
    function test_Route6ZapInDeposit_pretransferred_true() public {
        ...
        uint256 sharesOut = vault.exchangeIn(
            IERC20(address(tokenA)), amountIn, IERC20(address(vault)), 0, recipient, true, _deadline()
        );
