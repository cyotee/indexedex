...
`[profile.olympus_port]` in repo `foundry.toml`:

| Setting | Value / why |
|---------|-------------|
| `src` | `contracts/protocols/tokens/stable/olympus` |
| `test` | `test/foundry/spec/protocols/tokens/stable/olympus` |
| `out` / `cache_path` | `out_olympus_port` / `cache_olympus_port` |
...
export FOUNDRY_PROFILE=olympus_port

forge build
forge test
forge test --match-path 'test/foundry/spec/protocols/tokens/stable/olympus/Kernel.t.sol' -vv
...
1. Shells to `./src/test/lib/quabi/path.sh <Contract>.json` to find the forge artifact under `out_olympus_port`.
...
| Issue | Fix |
|-------|-----|
| Suite not found / wrong tree | Export `FOUNDRY_PROFILE=olympus_port` |
