...
| Single SE DETF | `contracts/vaults/detf/standardExchange/single/` | **No** (opt-in via non-zero `rateTarget`) | on if `rateTarget` set | Yes (Common guards zeros) | Low (docs/default only) |
| Multi-vault weighted DETF | `contracts/vaults/detf/composed/multi-vault-weighted/` | **No** (per-leg optional; auto-deploy only when `rateAsset` set) | unrated if both zero | Yes | Low (clarify auto-deploy policy) |
