...
This document describes a DeFi system integrating an external Constant Product DEX liquidity pool into a Strategy Vault, wrapped in an ERC4626 vault token, and managed by a Balancer V3 SV Conversion Pool, two Constant Product pools, new Strategy Vaults, and two Stable Pools.
...
Two Stable Pools operate within the Balancer V3 Vault, designed for tokens valued as the same underlying asset:
...
The Balancer V3 Vault manages tokens for all pools and SVs. Users interact through Balancer Routers, which call the Vault to execute swaps, deposits, or withdrawals.
...
graph TD
    ...
    Vault --> StablePoolA[Stable_Pool_A: SV_A_Pool_1/SV_A_Pool_2]
    Vault --> StablePoolB[Stable_Pool_B: SV_B_Pool_1/SV_B_Pool_2]
...
### Diagram Description
- **User**: Interacts with the Balancer Router.
- **Balancer Router**: Calls the Balancer V3 Vault.
- **Balancer V3 Vault**: Manages all pools, Rate Provider, and SVs.
...
- **Rate_Provider**: Original Rate Provider adjusts `4626_SV_A_B` in Constant Product pools.
...
This diagram shows the full system, including the SV Conversion Pool, original SV, LP token, Token A/B, new SVs, and Stable Pools:
