...
IMPLEMENTED — the family is built, tested, and green under `test/foundry/fork/base_main/vaults/detf/protocols/dexes/balancer/v3/uniswap/v4/crossVersion/v2/`. **Solidity home:** `contracts/vaults/detf/protocols/dexes/balancer/v3/uniswap/v4/crossVersion/v2/` (pro-rata BPT share vault — **not** a true DETF; layout co-location only). **Product docs home:** this file under `docs/detf/balancer/v3/uniswap/v4/crossVersion/v2/` (not co-located with Solidity). This PRD reflects the as-built design, including the direct Balancer V3 reserve integration (no external proportional-exit adapter or generic-router abstraction — see Reserve Integration) and **optional** reserve-leg rate providers (default **off**; opt-in **on**).
...
- Contracts implemented in `contracts/vaults/detf/protocols/dexes/balancer/v3/uniswap/v4/crossVersion/v2/`
...
Suite under `test/foundry/fork/base_main/vaults/detf/protocols/dexes/balancer/v3/uniswap/v4/crossVersion/v2/`, following Crane `TestBase_` / `Behavior_` conventions:
