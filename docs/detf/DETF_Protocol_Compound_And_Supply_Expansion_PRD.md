...
3. **Lazy realization** on the same class of touch points as Phase 1 reward updates (and any path that updates global rewards before compound). **Alignment D31:** live **mint**, live **burn**, **redeemClaim**, and **closeBondMature** realize pending expansion **before** Policy mint/burn gates (and before claim/close payout). If post-realize synthetic fails the mint/burn gate, the **whole tx reverts**. **Donate does not realize.** Open: realize is a no-op.  
