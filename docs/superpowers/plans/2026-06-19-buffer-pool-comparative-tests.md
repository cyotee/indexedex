...
- [x] Task 6 — Full build + spec run green + final commit
...
- COMPLETE: all 6 comparative tests pass; sibling UniswapV2 buffer-pool spec still green (22/22, no regression). Branch `test/buffer-pool-comparative`. `forge build` lint gate still fails on the unrelated pre-existing crane launchpad/solady issue (not caused by this work); test compilation + execution are clean.
