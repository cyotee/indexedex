# DualLiquidity (removed)DETF Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the DualLiquidity (removed)DETF family per `contracts/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)Detf_PRD.md`: a share-based DETF whose Balancer V3 Weighted Pool reserve composes two commonToken/linked-token Uniswap V4 Standard Exchange Vaults and one tokenA/tokenB Uniswap V2 vault, with quote-and-pick-best routing, usage-fee share inflation, and an immutable diamond deployment.
...
- **`IDualLiquidity (removed)Detf.sol` deleted** — the bespoke family interface is gone. Errors moved into `DualLiquidity (removed)DetfRepo`; config is read from the standard vault surface (`IBasicVault.vaultTokens()` + BPT balance); USAGE fee tagged by `type(IStandardExchangeIn).interfaceId`.
...
`DualLiquidity (removed)Detf_ProductionDeploy`, `..._BootstrapDeposit`, `..._Deposits`, `..._DFPkg_Registry`, `..._MathLib` — all pass over real Uniswap V4/V2 + Balancer legs.

### Base API & gotchas for rebuilding suites (READ before writing a suite)

The abstract base is `DualLiquidity (removed)DetfProductionBase` (in `DualLiquidity (removed)Detf_ProductionDeploy.t.sol`). A route suite: `contract X is DualLiquidity (removed)DetfProductionBase`, `setUp() public override { super.setUp(); _bootstrapReserve(); }`, then real deposits. Use `DualLiquidity (removed)Detf_Deposits.t.sol` as the copy-paste template.
...
- **Errors** are on the `DualLiquidity (removed)DetfRepo` library: `DualLiquidity (removed)DetfRepo.ZeroAmount.selector`, `.DeadlineExpired.selector`, `.ReservePoolNotInitialized.selector`, `.UnsupportedRoute`; slippage errors are `IStandardExchangeErrors.MinAmountNotMet` / `MaxAmountExceeded`.
...
- **Residual dust → `feeTo()`.** Every route now sweeps grown intermediate balances to the fee oracle's `feeTo()` (`DualLiquidity (removed)DetfCommon._sweepResidual`) instead of reverting `ResidualInventory` (not refunded to the caller, which may be a contract that can't handle a partial refund). Linked/common deposit routes work on real pricing.
- **Abstract `DualLiquidity (removed)DetfProductionBase`** extracted (in `DualLiquidity (removed)Detf_ProductionDeploy.t.sol`): setup + bootstrap/config/deposit helpers, no tests. `ProductionDeploy` (2 assertion tests), `BootstrapDeposit`, and route suites extend it.
- **Deposits suite** rebuilt on real code (`DualLiquidity (removed)Detf_Deposits.t.sol`, 7/7 green): vault-share / linked-token / common-token deposits with exact preview==execution over real pricing, pretransferred path, and guards.

### REMAINING WORK (start a new session here)
1. **Rebuild the remaining deleted suites on the real base** (each must fully compile before it can run — the family tree is all-or-nothing). Follow `DualLiquidity (removed)Detf_Deposits.t.sol` as the template (extend `DualLiquidity (removed)DetfProductionBase`, `_bootstrapReserve()` in `setUp`, real deposits + `_acquireLegShare`, relational magnitude asserts + exact preview==execution):
...
contracts/interfaces/
  IDualLiquidity (removed)Detf.sol                      # family interface + config getters
contracts/vaults/detf/composed/dual-liquidity-linked/
  DualLiquidity (removed)Detf_PRD.md                    # (exists)
  DualLiquidity (removed)DetfMathLib.sol                # pure share<->BPT proportion math
  DualLiquidity (removed)DetfRepo.sol                   # diamond storage
  DualLiquidity (removed)DetfCommon.sol                 # storage-bound helpers (mint w/ fee, join, exit, classify)
  ...
  DualLiquidity (removed)DetfExchangeInTarget.sol       # exchangeIn routes (deposits, swaps, exact-in redemptions)
  DualLiquidity (removed)DetfExchangeInFacet.sol
  DualLiquidity (removed)DetfExchangeInQueryTarget.sol  # previewExchangeIn
  DualLiquidity (removed)DetfExchangeInQueryFacet.sol
  DualLiquidity (removed)DetfExchangeOutTarget.sol      # exchangeOut (exact-out redemptions + exact-out swaps)
  DualLiquidity (removed)DetfExchangeOutFacet.sol
  DualLiquidity (removed)DetfExchangeOutQueryTarget.sol # previewExchangeOut
  DualLiquidity (removed)DetfExchangeOutQueryFacet.sol
  DualLiquidity (removed)DetfDFPkg.sol                  # package: facetCuts + initAccount + dust init
  DualLiquidity (removed)Detf_Facet_FactoryService.sol
  DualLiquidity (removed)Detf_Pkg_FactoryService.sol
  DualLiquidity (removed)Detf_Component_FactoryService.sol
  TestBase_DualLiquidity (removed)Detf.sol              # family test base (mock-leg harness + real-component harness)
...
test/foundry/spec/vaults/detf/composed/dual-liquidity-linked/
  DualLiquidity (removed)DetfMathLib.t.sol
  DualLiquidity (removed)DetfExchangeIn_Deposits.t.sol
  DualLiquidity (removed)DetfExchangeIn_Swaps.t.sol
  DualLiquidity (removed)DetfExchangeIn_Redemptions.t.sol
  DualLiquidity (removed)DetfExchangeOut_ExactOut.t.sol
  DualLiquidity (removed)DetfDFPkg_Deploy.t.sol
  DualLiquidity (removed)Detf_Invariants.t.sol
```
...
**Files:**
- Create: `contracts/interfaces/IDualLiquidity (removed)Detf.sol`
- Create: `contracts/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfMathLib.sol`
- Test: `test/foundry/spec/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfMathLib.t.sol`

**Interfaces:**
- Consumes: `IStandardExchangeIn`, `IStandardExchangeOut`, `IERC20` (crane).
- Produces: `IDualLiquidity (removed)Detf` (extends both exchange interfaces + config getters below); `DualLiquidity (removed)DetfMathLib._sharesForBpt(uint256 bptIn, uint256 totalShares, uint256 totalBpt) → uint256`; `._bptForShares(uint256 sharesIn, uint256 totalShares, uint256 totalBpt) → uint256`.
...
```solidity
// test/foundry/spec/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfMathLib.t.sol
// SPDX-License-Identifier: BSL-1.1
pragma solidity ^0.8.0;

import {Test} from "forge-std/Test.sol";
import {DualLiquidity (removed)DetfMathLib} from
    "contracts/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfMathLib.sol";

contract DualLiquidity (removed)DetfMathLibTest is Test {
    function test_sharesForBpt_proportional() public pure {
        // 100 BPT into a reserve of 1000 BPT backing 2000 shares -> 200 shares
        assertEq(DualLiquidity (removed)DetfMathLib._sharesForBpt(100e18, 2000e18, 1000e18), 200e18);
    }

    function test_bptForShares_proportional() public pure {
        assertEq(DualLiquidity (removed)DetfMathLib._bptForShares(200e18, 2000e18, 1000e18), 100e18);
    }

    function test_sharesForBpt_roundsDown() public pure {
        assertEq(DualLiquidity (removed)DetfMathLib._sharesForBpt(1, 3, 2), 1); // 1*3/2 = 1.5 -> 1
    }

    function test_bptForShares_roundsDown() public pure {
        assertEq(DualLiquidity (removed)DetfMathLib._bptForShares(1, 3, 2), 0); // 1*2/3 = 0.66 -> 0
    }
    ...
    function testFuzz_roundTrip_neverProfits(uint128 bptIn, uint128 totalShares, uint128 totalBpt) public pure {
        ...
        uint256 shares = DualLiquidity (removed)DetfMathLib._sharesForBpt(bptIn, totalShares, totalBpt);
        uint256 back = DualLiquidity (removed)DetfMathLib._bptForShares(shares, totalShares, totalBpt);
...
Run: `forge test --match-path "test/foundry/spec/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfMathLib.t.sol" -vv`
Expected: compilation failure — `DualLiquidity (removed)DetfMathLib` not found.
...
library DualLiquidity (removed)DetfMathLib {
...
```solidity
// contracts/interfaces/IDualLiquidity (removed)Detf.sol
// SPDX-License-Identifier: BSL-1.1
pragma solidity ^0.8.0;
...
interface IDualLiquidity (removed)Detf is IStandardExchangeIn, IStandardExchangeOut {
...
```bash
git add contracts/interfaces/IDualLiquidity (removed)Detf.sol \
  contracts/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfMathLib.sol \
  test/foundry/spec/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfMathLib.t.sol
git commit -m "feat(detf): add DualLiquidity (removed)Detf interface and share math lib
...
- Create: `contracts/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfRepo.sol`
- Create: `contracts/vaults/detf/composed/dual-liquidity-linked/DualLiquidity (removed)DetfCommon.sol`
...
- [ ] **Step 1: Write failing Common tests** in `DualLiquidity (removed)DetfExchangeIn_Deposits.t.sol` (started here, extended in Task 3): deploy a `CommonHarness is DualLiquidity (removed)DetfCommon` test contract exposing `initialize/mintSharesForBpt/burnSharesForBpt/classify`; assert: fee split mints `gross * fee / 1e18` shares to `feeTo` and remainder to recipient (use a `MockVaultFeeOracle` stub with settable `usageFeeOfVault=5e16` (5%) and `feeTo`); classification returns correct kinds for all nine token identities; `_requireLive` reverts on zero amount, past deadline, uninitialized reserve.
...
- [ ] **Step 5: Commit** — `feat(detf): add DualLiquidity (removed)Detf storage, common helpers, and test mocks` (+ trailer).
...
**Files:**
- Create: `DualLiquidity (removed)DetfExchangeInTarget.sol`, `DualLiquidity (removed)DetfExchangeInFacet.sol`, `DualLiquidity (removed)DetfExchangeInQueryTarget.sol`, `DualLiquidity (removed)DetfExchangeInQueryFacet.sol` (family dir)
- Create: `TestBase_DualLiquidity (removed)Detf.sol` (family dir) — mock-leg harness: deploys the Common harness diamond-less (plain contract inheriting the Targets), three `MockStandardExchange` legs, `MockReservePool`, `MockVaultFeeOracle`, nine ERC20 stubs, and wires `Repo._initialize`.
- Test: `DualLiquidity (removed)DetfExchangeIn_Deposits.t.sol` (extend)

**Interfaces:**
- Consumes: Task 2 helpers.
- Produces: `exchangeIn`/`previewExchangeIn` handling the four deposit routes. Route dispatch in `DualLiquidity (removed)DetfExchangeInTarget._exchangeIn` by `(kindIn, kindOut)`:
...
- [ ] **Step 3: Implement Targets + Facets.** Facets are thin: inherit Target, implement `IFacet` (`facetName() = "DualLiquidity (removed)DetfExchangeInFacet"`, `facetInterfaces() = [type(IStandardExchangeIn).interfaceId]`, `facetFuncs() = [IStandardExchangeIn.exchangeIn.selector]`; query facet exposes `previewExchangeIn`). Keep per-route logic in private helpers (`_depositBpt`, `_depositVaultShares`, `_depositLinkedToken`, `_depositCommonToken`) to control stack depth.
- [ ] **Step 4: Run tests to verify pass.**
- [ ] **Step 5: Commit** — `feat(detf): add DualLiquidity (removed)Detf deposit routes with best-of quoting` (+ trailer).
...
**Files:**
- Modify: `DualLiquidity (removed)DetfExchangeInTarget.sol`, `DualLiquidity (removed)DetfExchangeInQueryTarget.sol`
- Test: `DualLiquidity (removed)DetfExchangeIn_Swaps.t.sol`
...
**Files:**
- Modify: `DualLiquidity (removed)DetfExchangeInTarget.sol`, `DualLiquidity (removed)DetfExchangeInQueryTarget.sol`
- Test: `DualLiquidity (removed)DetfExchangeIn_Redemptions.t.sol`
...
**Files:**
- Create: `DualLiquidity (removed)DetfExchangeOutTarget.sol`, `DualLiquidity (removed)DetfExchangeOutFacet.sol`, `DualLiquidity (removed)DetfExchangeOutQueryTarget.sol`, `DualLiquidity (removed)DetfExchangeOutQueryFacet.sol`
- Test: `DualLiquidity (removed)DetfExchangeOut_ExactOut.t.sol`
...
**Files:**
- Create: `DualLiquidity (removed)DetfDFPkg.sol`, `DualLiquidity (removed)Detf_Facet_FactoryService.sol`, `DualLiquidity (removed)Detf_Pkg_FactoryService.sol`, `DualLiquidity (removed)Detf_Component_FactoryService.sol` (family dir)
- Modify: `TestBase_DualLiquidity (removed)Detf.sol` — add real-component harness path
- Test: `DualLiquidity (removed)DetfDFPkg_Deploy.t.sol`
...
- [ ] **Step 6: Commit** — `feat(detf): add DualLiquidity (removed)Detf package with immutable deployment` (+ trailer).
...
**Files:**
- Test: `DualLiquidity (removed)Detf_Invariants.t.sol`

**Interfaces:**
- Consumes: mock-leg harness from `TestBase_DualLiquidity (removed)Detf`.
...
- [ ] **Step 2: Run** — `forge test --match-contract DualLiquidity (removed)Detf_Invariants -vv` with default depth; fix any violations found (violations are implementation bugs — apply superpowers:systematic-debugging, do not weaken invariants).
- [ ] **Step 3: Commit** — `test(detf): add DualLiquidity (removed)Detf invariant suite` (+ trailer).
...
**Files:**
- Modify: all family contracts (NatSpec passes), `DualLiquidity (removed)Detf_PRD.md` (checklist), `docs/CODEBASE_MAP.md` (add family entry if the map's vault section exists)
...
- [ ] **Step 4: Commit** — `docs(detf): NatSpec and PRD closeout for DualLiquidity (removed)Detf` (+ trailer).
