...
- **Reserve donation (D29):** permissionless `donate` on the **Bond NFT** (not leftover `UniV4DetfBondNft`). Accepts `pairToken` / `vaultShare` / family mint-bond tokens / **DETF** / reserve LP. No DETF mint. **New `originalShares` to id 0 only.** DETF diamond executes the host join (`onlyBondNft`; Uni V4 D9 / D30). `IDetf.donate` forwards. Law: [`contracts/vaults/detf/DETF_RESERVE_DONATION_PRD.md`](../../contracts/vaults/detf/DETF_RESERVE_DONATION_PRD.md).
