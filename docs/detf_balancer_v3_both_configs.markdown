# Decentralized ETF (DETF) with Balancer V3 Configurations

This document describes the **Decentralized ETF (DETF)** protocol, a DeFi system integrating external Constant Product DEX liquidity pools into Strategy Vaults, wrapped in ERC4626 vault tokens, and managed by Balancer V3 pools. Two DETF configurations are presented: Configuration 1 uses one external pool (A/B) with a final Constant Product pool, while Configuration 2 uses three external pools (A/B, A/C, B/C) with a final Weighted Pool. The diagrams focus on the pools, vaults, and tokens, excluding user interaction and vault management components.

## Explanation

The **Decentralized ETF (DETF)** protocol aggregates liquidity from external DEX pools into a structured system of Balancer V3 pools, using Strategy Vaults to standardize interactions and enable stable, constant product, and weighted swaps. It provides a scalable, flexible framework for managing multiple token pairs, akin to a traditional ETF but in a decentralized manner. Two configurations demonstrate the DETF’s versatility:

### Configuration 1: DETF with Single External Pool and Constant Product Consolidation
...
### Configuration 2: DETF with Three External Pools and Weighted Consolidation
...
- **Flexibility**: Enables stable, constant product, and weighted swaps with advanced pricing, mimicking ETF-like diversification in DeFi.

## Diagrams

### Diagram 1: Configuration 1 (DETF with Single External Pool and Constant Product Consolidation)
This Mermaid diagram illustrates the DETF with one external DEX pool and a final Constant Product pool:
...
### Diagram 2: Configuration 2 (DETF with Three External Pools and Weighted Consolidation)
This Mermaid diagram illustrates the DETF with three external DEX pools and a final Weighted Pool:
...
- Define specific DETF features (e.g., governance, fees, use cases).
