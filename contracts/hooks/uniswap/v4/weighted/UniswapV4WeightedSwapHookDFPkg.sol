...
/**
 * @title UniswapV4WeightedSwapHookDFPkg
 * @notice Hook diamond package: ERC20Permit LP + MultiAsset vault + weighted product facets.
 * @dev deployVault → registry.deployHookVault → hook CREATE2 factory. Salt excludes package address.
 *      postDeploy ensures all binom(n,2) pair doors. Unbuffered (H3); buffering is H8.
 */
contract UniswapV4WeightedSwapHookDFPkg is IUniswapV4WeightedSwapHookPackage {
    using BetterEfficientHashLib for bytes;

    bytes32 public constant PRODUCT_ID = keccak256("uv4-weighted-swap-hook");
    bytes4 public constant HOOK_VAULT_TYPE = bytes4(keccak256("UniswapV4WeightedSwapHook"));
    ...
    IUniswapV4WeightedSwapHookPackage private immutable SELF;
    ...
    function packageName() public pure returns (string memory) {
        return type(UniswapV4WeightedSwapHookDFPkg).name;
    }
    ...
    function calcSalt(bytes memory pkgArgs) public pure returns (bytes32 salt) {
        PkgArgs memory a = abi.decode(pkgArgs, (PkgArgs));
        // PRODUCT_ID + poolManager + feeOracle + tokens + weights + rateProviders.
        // Excludes package address, facets, msg.sender, tickSpacing, sqrtPriceX96.
        return keccak256(
            abi.encode(PRODUCT_ID, a.poolManager, a.feeOracle, a.tokens, a.weights, a.rateProviders)
        );
    }
    ...
    function _initLpAndVault(PkgArgs memory a) private {
        (string memory name_, string memory symbol_) = _buildLpMetadata(a.tokens);
        ...
        StandardVaultRepo._initialize(
            ...
            vaultFeeTypeIds(),
            vaultTypes(),
            abi.encode(a.tokens, a.weights, a.rateProviders)._hash()
        );
    }
    ...
    function _initProductBindings(PkgArgs memory a) private {
        ...
        for (uint256 i; i < n; ++i) {
            uint8 d = _readDecimalsFailClosed(a.tokens[i]);
    ...
    {
        string memory body = _safeSymbol(toks[0]);
        for (uint256 i = 1; i < toks.length; ++i) {
            body = string.concat(body, "-", _safeSymbol(toks[i]));
        }
    ...
    /// @notice Ensure all pair doors after diamond deploy (idempotent skip-if-live).
    ...
    function postDeploy(address proxy) public returns (bool) {
        IUniswapV4WeightedSwapHook h = IUniswapV4WeightedSwapHook(proxy);
        PairPoolLib.ensureAllPairPools(
            h.poolManager(),
            proxy,
            h.tokens(),
    ...
    function name() public pure returns (string memory) {
        return "UniswapV4WeightedSwapHook";
    }
    ...
    function _validateArgs(PkgArgs memory a) private pure {
        ...
        if (a.weights.length != n || a.rateProviders.length != n) revert InvalidN();
        uint256 sum;
        for (uint256 i; i < n; ++i) {
            if (a.tokens[i] == address(0)) revert ZeroAddress();
            if (i > 0 && !(a.tokens[i - 1] < a.tokens[i])) revert InvalidTokenOrder();
    ...
    function _safeSymbol(address token_) private view returns (string memory) {
        try IERC20Metadata(token_).symbol() returns (string memory s) {
            if (bytes(s).length > 0) return s;
        } catch {}
        return _addressFragment(token_);
    }

    function _addressFragment(address token_) private pure returns (string memory) {
    ...
    function _readDecimalsFailClosed(address token_) private view returns (uint8 d) {
