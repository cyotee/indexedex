...
contracts/hooks/uniswap/v4/stable/quad/
  interfaces/
    ...
    IUniswapV4QuadStableSwapHookPackage.sol
  facets/
    UniswapV4QuadStableSwapHookHooksFacet.sol
    UniswapV4QuadStableSwapHookLiquidityFacet.sol
  UniswapV4QuadStableSwapHookDFPkg.sol
  UniswapV4QuadStableSwapHookTarget.sol
  UniswapV4QuadStableSwapHookRepo.sol
  UniswapV4QuadStableSwapHookMath.sol
  UniswapV4QuadStableSwapHookPairPoolLib.sol
  UniswapV4QuadStableSwapHook_FactoryService.sol
  TestBase_UniswapV4QuadStableSwapHook.sol
```

**Retired:** monomorph `UniswapV4QuadStableSwapHook.sol`, `…Factory.sol`, `…Deployer.sol`, `…Common.sol`, `IUniswapV4QuadStableSwapHookFactory.sol`.
...
| Field | Salt / binding |
|-------|----------------|
| `PRODUCT_ID` | `keccak256("uv4-quad-stable-swap-hook")` |
...
- Bindings + product reserves: `UniswapV4QuadStableSwapHookRepo` via `initAccount`
- LP: shared `ERC20Repo` + ERC20Permit facets
- Vault views: `MultiAssetBasicVaultRepo` / `StandardVaultRepo` (synced on reserve updates)

## Test ladder

`CraneTest` → `IndexedexTest` → `TestBase_VaultComponents` → `TestBase_UniswapV4QuadStableSwapHook`
