 ...
 * @notice Product logic: IHooks + LP execute + Balancer StableMath beforeSwap (fee-on-output).
 * @dev Bindings in Repo (initAccount). LP via ERC20Repo; vault reserves via MultiAssetBasicVaultRepo.
 *      Swap identity = Crane StableMath (AMP_PRECISION 1e3). No SE buffer legs.
 *      No constructor immutables; no monomorph Common inheritance. No BaseHook / DeltaResolver.
 */
abstract contract UniswapV4BalancerQuadStableSwapHookTarget is IHooks, IUniswapV4BalancerQuadStableSwapHook {
        ...
        returns (bool)
    {
        try this.zapQuoteExactInExternal(z.workingR, z.rates, i, j, swapIn, z.amp, z.fee) returns (
            ...
            // Reject swaps that leave the shared book outside Balancer-safe imbalance.
            if (!Math.imbalanceOk(newR, z.rates)) return false;
            z.workingR = newR;
            z.W[i] -= swapIn;
            z.W[j] += userOut;
            return true;
    ...
    {
        if (rawSurplus == 0) return 0;
        // Prefer closed-form inverse capped by leave-one on out leg.
        uint256 outScaled = Math.scaleTo(z.workingR[j], z.rates[j]);
        uint256 leaveAmt = outScaled > 1e12 ? 1e12 : (outScaled > 1 ? 1 : 0);
        if (leaveAmt != 0 && outScaled > leaveAmt) {
            ...
            if (maxRawOut > 0) {
                ...
                if (maxUserOut > 0) {
                    ...
                    if (ideal > 0) {
                        ...
                        if (_isQuoteViable(z, i, j, maxViableIn)) return maxViableIn;
                    }
                }
            }
        }
        // Balancer Newton can reject large exact-in under thin books; binary-search a viable in.
        return _binarySearchViableIn(z, i, j, rawSurplus);
    }

    function _isQuoteViable(ZapWork memory z, uint256 i, uint256 j, uint256 swapIn)
        private
        view
        returns (bool)
    {
        if (swapIn == 0 || swapIn > z.W[i]) return false;
        try this.zapQuoteExactInExternal(z.workingR, z.rates, i, j, swapIn, z.amp, z.fee) returns (
            uint256 userOut, uint256[4] memory
        ) {
            return userOut > 0;
        } catch {
            return false;
    ...
    function _binarySearchViableIn(ZapWork memory z, uint256 i, uint256 j, uint256 rawSurplus)
        private
        view
        returns (uint256 best)
    {
        if (rawSurplus == 0) return 0;
        // Geometric shrink then expand-binary: find any viable, then maximize under cap.
        uint256 probe = rawSurplus;
        for (uint256 k; k < 64; ++k) {
            if (_isQuoteViable(z, i, j, probe)) {
                best = probe;
                break;
            }
            probe = probe / 2;
            if (probe == 0) {
                // try unit size once
                if (_isQuoteViable(z, i, j, 1)) return 1;
                return 0;
            }
        }
        if (best == 0) return 0;
        // Binary search upward toward rawSurplus for a larger viable in.
        uint256 lo = best;
        uint256 hi = rawSurplus;
        while (lo < hi) {
            uint256 mid = lo + (hi - lo + 1) / 2;
            if (_isQuoteViable(z, i, j, mid)) {
                lo = mid;
            } else {
                hi = mid - 1;
            }
    ...
    function _requirePriceable() internal view {
        ...
        // Soft gate: Balancer Newton may not converge under extreme imbalance; try/catch so
        // successful LP ops are not rolled back solely for reconvergence noise.
        try this.tryGetDExternal(xp, getCurrentAmp()) {} catch {
            // all scaled legs non-zero already — allow
        }
    }

    /// @dev External boundary for optional invariant reconvergence (must be cut on HooksFacet).
    function tryGetDExternal(uint256[4] memory xp, uint256 amp) external pure returns (uint256) {
        return Math.getD(xp, amp);
    }

    /* ---------------------------------------------------------------------- */
    /*              IStandardExchangeMultiAssetLiquidity (v1 surface)         */
    /* ---------------------------------------------------------------------- */
    ...
    function _requireAmountsLen4(uint256[] memory amounts) internal pure {
        if (amounts.length != 4) revert InvalidRoute();
    }
        ...
        returns (uint256 shares, uint256[] memory usedAmounts)
    {
        ...
        uint256[4] memory fixedIn = Math.toFixed4(amounts);
        uint256[4] memory actual;
        (shares, actual) = _previewAddLiquidity(fixedIn);
        usedAmounts = Math.toDynamic(actual);
    }
    ...
    function joinProportional(
        ...
        uint256[4] memory actual;
        (shares, actual) = _addLiquidity(fixedIn, mins, to, sharesMin);
        usedAmounts = Math.toDynamic(actual);
    }

    function previewExitProportional(uint256 shares) external view returns (uint256[] memory amounts) {
        amounts = Math.toDynamic(_previewRemoveLiquidity(shares));
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        uint256[4] memory mins = Math.toFixed4(amountsMin);
        amounts = Math.toDynamic(_removeLiquidity(shares, to, mins));
    }

    /// @dev Unbalanced join = zap-backed multipath (requires live full book).
    function previewJoinUnbalanced(uint256[] calldata amounts) external view returns (uint256 shares) {
        _requireAmountsLen4(amounts);
        (shares,) = _previewZapIn(Math.toFixed4(amounts));
    }
        ...
        returns (uint256 shares)
    {
        _requireDeadline(deadline);
        _requireAmountsLen4(amounts);
        (shares,) = _zapIn(Math.toFixed4(amounts), to, sharesMin);
    }
    ...
    {
        uint256[4] memory amounts;
        amounts[_tokenIndex(tokenIn)] = amountIn;
        (shares,) = _previewZapIn(amounts);
    }
    ...
    function joinSingleAssetExactIn(
        ...
        uint256[4] memory amounts;
        amounts[_tokenIndex(tokenIn)] = amountIn;
        (shares,) = _zapIn(amounts, to, sharesMin);
    }

    function previewJoinSingleAssetExactOut(address, uint256) external pure returns (uint256) {
        revert InvalidRoute();
    }
    ...
    /// @dev v1: true single-asset exit needs StableMath computeBalance join/exit; use exitProportional.
    function previewExitSingleAssetExactBptIn(address, uint256) external pure returns (uint256) {
        revert InvalidRoute();
    }

    function exitSingleAssetExactBptIn(address, uint256, address, uint256, uint256)
        external
        pure
        returns (uint256)
    {
    ...
    function previewExitSingleAssetExactTokenOut(address, uint256) external pure returns (uint256) {
        revert InvalidRoute();
    }
    ...
    function withdrawSingle(address, uint256, address, uint256, uint256)
        external
        pure
        returns (uint256)
    {
    ...
    function previewWithdrawSingle(address, uint256) external pure returns (uint256) {
        revert InvalidRoute();
    }
    ...
    function previewWithdrawSingleExactOut(address, uint256) external pure returns (uint256) {
        revert InvalidRoute();
    }
}
