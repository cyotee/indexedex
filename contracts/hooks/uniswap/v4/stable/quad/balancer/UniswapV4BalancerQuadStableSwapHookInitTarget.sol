...
/**
 * @title UniswapV4BalancerQuadStableSwapHookInitTarget
 * @notice Package-as-init doors, door views, and bootstrap beforeInitialize (S58).
 * @dev Does not inherit UniswapV4BalancerQuadStableSwapHookTarget.
 *      finalizeInitialization body lives on DFPkg.
 */
abstract contract UniswapV4BalancerQuadStableSwapHookInitTarget is IUniswapV4BalancerQuadStableSwapHookInit {
    using PoolIdLibrary for PoolKey;

    modifier nonReentrant() {
        Repo.Layout storage l = Repo._layout();
        if (l.reentrancyStatus == Repo.ENTERED) {
            revert UniswapV4BalancerQuadStableSwapHookTarget.Reentrancy();
        }
        ...
        returns (address c0, address c1)
    {
        if (tokenA == address(0) || tokenB == address(0) || tokenA == tokenB) {
            revert UniswapV4BalancerQuadStableSwapHookTarget.InvalidToken();
        }
