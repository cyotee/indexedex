...
**Name:** `UniswapV4CurveQuadStableSwapHook`  
**Date:** 2026-08-03  
...
> **DEPLOY SUPERSESSION (2026-08-04):** All CREATE3 monomorph instance deploy, `HookMinerCreate3` product mine, `UniswapV4CurveQuadStableSwapHookFactory` as primary UX, and `saltNamespace` identity law are **superseded** by  
...
| **This package** | `contracts/hooks/uniswap/v4/stable/quad/curve/` | **4-asset** StableSwap (A-amplified) on **raw ERC-20 reserves** |
...
| **Product name** | Canonical Solidity / package type: **`UniswapV4CurveQuadStableSwapHook`**. Not Balancer WeightedMath; equal units after rate scaling only. |
...
|---|----------|--------|--------|
| D1 | Product name | **`UniswapV4CurveQuadStableSwapHook`** (canonical) | **LOCKED** |
| D2 | Package location | `contracts/hooks/uniswap/v4/stable/quad/curve/` | **LOCKED** |
...
| D12 | Package shape | **Hook:** Repo + Target + Common + Math + thin wire. **Deploy libs:** FactoryService (mine/salt). **On-chain factory:** `UniswapV4CurveQuadStableSwapHookFactory` (+ interface). No Facet/DFPkg for hook or factory in v1 | **LOCKED** |
...
| D37 | Math library | Pure `UniswapV4CurveQuadStableSwapHookMath` — invariant, target reserve, scale/descale, geometric mean, fee + zap sizing helpers | **LOCKED** |
...
| D51 | On-chain factory product | **`UniswapV4CurveQuadStableSwapHookFactory`** — dedicated on-chain contract under `stable/quad/` that exposes **permissionless** deploy + pool creation (normative API §5.5) | **LOCKED** |
...
| Q21 | Factory | **On-chain permissionless** `UniswapV4CurveQuadStableSwapHookFactory`. |
...
└───────────────┬─────────────────────────────┬────────────────────┘
                ...
                ▼                             ▼
┌───────────────────────────────┐   ┌──────────────────────────────────┐
│ UniswapV4CurveQuadStableSwapHook │   │ Uniswap V4 PoolManager           │
...
Pool t0/t1  ──┐
Pool t0/t2  ──┤
Pool t0/t3  ──┼──► same UniswapV4CurveQuadStableSwapHook (shared reserves + A + D)
Pool t1/t2  ──┤
Pool t1/t3  ──┤
Pool t2/t3  ──┘
```
...
contracts/hooks/uniswap/v4/stable/quad/curve/
  ...
  interfaces/
    IUniswapV4CurveQuadStableSwapHook.sol                           # views + previews + LP surface
    IUniswapV4CurveQuadStableSwapHookFactory.sol                    # permissionless deploy + ensure pools

  UniswapV4CurveQuadStableSwapHookMath.sol                          # pure StableSwap n=4 + WAD rates + shares
  UniswapV4CurveQuadStableSwapHookRepo.sol                          # diamond-style storage slot layout
  UniswapV4CurveQuadStableSwapHookCommon.sol                        # reserve helpers, rate cache, guards
  UniswapV4CurveQuadStableSwapHookTarget.sol                        # IHooks + LP execute (pattern-copy settle)
  UniswapV4CurveQuadStableSwapHook.sol                              # single CREATE3-mined wire + ERC-20

  UniswapV4CurveQuadStableSwapHook_FactoryService.sol               # pure/internal mine + salt + isExpectedHook
  UniswapV4CurveQuadStableSwapHookFactory.sol                       # on-chain permissionless factory (D51)
...
Follow Crane **Repo** pattern: single namespaced storage slot (e.g. `keccak256("…UniswapV4CurveQuadStableSwapHook.storage")` layout struct), **not** free-floating storage variables scattered on the wire contract without a Repo library.
...
Ship **`UniswapV4CurveQuadStableSwapHookFactory`** so **any** account can:
...
```solidity
interface IUniswapV4CurveQuadStableSwapHookFactory {
    event HookDeployed(
...
2. Deploy UniswapV4CurveQuadStableSwapHookFactory via CREATE3 (script) with immutables:
...
3. **Gold TestBase** peer to dual/orbital/buffer: `TestBase_UniswapV4CurveQuadStableSwapHook`.  
...
- [ ] Package files under `contracts/hooks/uniswap/v4/stable/quad/curve/` match §5 (hook **and** factory).  
...
| **v0.4.2** | 2026-08-03 | All product identifiers renamed to **`UniswapV4CurveQuadStableSwapHook`** family (interfaces, Math/Repo/Target/Factory/FactoryService, salt `uv4-quad-stable-swap-hook-`, LP prefix `QS-`). Removed residual “Weighted” product naming. |
...
**End of PRD — UniswapV4CurveQuadStableSwapHook (Draft v0.5.2)**
