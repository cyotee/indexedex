...
import {
    UniswapV4CurveQuadStableSwapHookInitTarget
} from "contracts/hooks/uniswap/v4/stable/quad/curve/UniswapV4CurveQuadStableSwapHookInitTarget.sol";

/**
 * @title UniswapV4CurveQuadStableSwapHookInitFacet
 * @notice Source-only IFacet mixin inherited by the DFPkg (S47). Never CREATE3-deployed.
 */
abstract contract UniswapV4CurveQuadStableSwapHookInitFacet is
    UniswapV4CurveQuadStableSwapHookInitTarget,
    IFacet
{
    function facetName() public pure returns (string memory) {
        return type(UniswapV4CurveQuadStableSwapHookInitFacet).name;
    }

    function facetInterfaces() public pure virtual returns (bytes4[] memory interfaces) {
        interfaces = new bytes4[](1);
        interfaces[0] = type(IUniswapV4CurveQuadStableSwapHookInit).interfaceId;
    }
