...
} from "contracts/hooks/uniswap/v4/orbital/UniswapV4OrbitalSwapHookInitTarget.sol";

/**
 * @title UniswapV4OrbitalSwapHookInitFacet
 * @notice Source-only IFacet mixin inherited by the DFPkg (S47). Never CREATE3-deployed.
 */
abstract contract UniswapV4OrbitalSwapHookInitFacet is UniswapV4OrbitalSwapHookInitTarget, IFacet {
    function facetName() public pure returns (string memory) {
        return type(UniswapV4OrbitalSwapHookInitFacet).name;
    }

    function facetInterfaces() public pure virtual returns (bytes4[] memory interfaces) {
        interfaces = new bytes4[](1);
        interfaces[0] = type(IUniswapV4OrbitalSwapHookInit).interfaceId;
    }
