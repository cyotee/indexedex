...
import {
    UniswapV4SingleStandardExchangeBufferHookInitTarget
} from "contracts/hooks/uniswap/v4/standardExchange/single/UniswapV4SingleStandardExchangeBufferHookInitTarget.sol";

/**
 * @title UniswapV4SingleStandardExchangeBufferHookInitFacet
 * @notice Source-only IFacet mixin inherited by the DFPkg (S47). Never CREATE3-deployed.
 */
abstract contract UniswapV4SingleStandardExchangeBufferHookInitFacet is
    UniswapV4SingleStandardExchangeBufferHookInitTarget,
    IFacet
{
    function facetName() public pure returns (string memory) {
        return type(UniswapV4SingleStandardExchangeBufferHookInitFacet).name;
    }

    function facetInterfaces() public pure virtual returns (bytes4[] memory interfaces) {
        interfaces = new bytes4[](1);
        interfaces[0] = type(IUniswapV4SingleStandardExchangeBufferHookInit).interfaceId;
    }
