...
import {
    UniswapV4StandardExchangeOrbitalBufferHookInitTarget
} from "contracts/hooks/uniswap/v4/standardExchange/orbital/UniswapV4StandardExchangeOrbitalBufferHookInitTarget.sol";

/**
 * @title UniswapV4StandardExchangeOrbitalBufferHookInitFacet
 * @notice Source-only IFacet mixin inherited by the DFPkg. Never CREATE3-deployed.
 */
abstract contract UniswapV4StandardExchangeOrbitalBufferHookInitFacet is
    UniswapV4StandardExchangeOrbitalBufferHookInitTarget,
    IFacet
{
    function facetName() public pure returns (string memory) {
        return type(UniswapV4StandardExchangeOrbitalBufferHookInitFacet).name;
    }

    function facetInterfaces() public pure virtual returns (bytes4[] memory interfaces) {
        interfaces = new bytes4[](1);
        interfaces[0] = type(IUniswapV4StandardExchangeOrbitalBufferHookInit).interfaceId;
    }
