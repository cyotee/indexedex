...
abstract contract UniswapV4StandardExchangeOrbitalBufferHookDepositCore is UniswapV4StandardExchangeOrbitalBufferHookCommon {
    using SafeERC20 for IERC20;

    function _entryAddLiquidity(
        uint256 a0Max,
        uint256 a1Max,
        uint256 a2Max,
        address to,
    ...
    ) internal onlyLiquidityOwner nonReentrant returns (uint256 shares, uint256 a0, uint256 a1, uint256 a2) {
    ...
    function _entryPreviewAddLiquidity(uint256 a0Max, uint256 a1Max, uint256 a2Max)
        internal
        view
    ...
    function _entryPreviewZapSplit(address tokenIn, uint256 amountIn)
        internal
        view
    ...
    function _entryDepositFlexible(
        uint256 amount0,
        ...
        uint256 sharesMin,
        uint256 deadline
    ) internal onlyLiquidityOwner nonReentrant returns (uint256 shares, uint256 used0, uint256 used1, uint256 used2) {
    ...
    function _entryPreviewDepositFlexible(
        uint256 amount0,
        bool amount0IsSeShare,
        uint256 amount1,
        bool amount1IsSeShare,
        uint256 amount2,
        bool amount2IsSeShare
    ) internal view returns (uint256 shares, uint256 used0, uint256 used1, uint256 used2) {
        DepositFlexibleVars memory v;
        v.amount0 = amount0;
        v.amount0IsSeShare = amount0IsSeShare;
        v.amount1 = amount1;
        ...
        returns (uint256 shares)
    {
        ...
        if (kind_ == UniswapV4SeBufferHookLegLib.LegKind.StandardExchange) {
            ...
            return ExitQuoteLib.previewShareDeposit(tokenIn, amountIn);
        }
        return _previewDepositSingle(tokenIn, amountIn);
    }
