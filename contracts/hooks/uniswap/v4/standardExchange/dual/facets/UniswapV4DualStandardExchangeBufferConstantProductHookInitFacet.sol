...
import {
    UniswapV4DualStandardExchangeBufferConstantProductHookInitTarget
} from "contracts/hooks/uniswap/v4/standardExchange/dual/UniswapV4DualStandardExchangeBufferConstantProductHookInitTarget.sol";

/**
 * @title UniswapV4DualStandardExchangeBufferConstantProductHookInitFacet
 * @notice Source-only IFacet mixin inherited by the DFPkg (S47). Never CREATE3-deployed.
 */
abstract contract UniswapV4DualStandardExchangeBufferConstantProductHookInitFacet is
    UniswapV4DualStandardExchangeBufferConstantProductHookInitTarget,
    IFacet
{
    function facetName() public pure returns (string memory) {
        return type(UniswapV4DualStandardExchangeBufferConstantProductHookInitFacet).name;
    }

    function facetInterfaces() public pure virtual returns (bytes4[] memory interfaces) {
        interfaces = new bytes4[](1);
        interfaces[0] = type(IUniswapV4DualStandardExchangeBufferConstantProductHookInit).interfaceId;
    }
