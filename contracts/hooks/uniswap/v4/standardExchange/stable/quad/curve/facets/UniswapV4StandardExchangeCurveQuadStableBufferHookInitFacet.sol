...
import {
    UniswapV4StandardExchangeCurveQuadStableBufferHookInitTarget
} from "contracts/hooks/uniswap/v4/standardExchange/stable/quad/curve/UniswapV4StandardExchangeCurveQuadStableBufferHookInitTarget.sol";

/**
 * @title UniswapV4StandardExchangeCurveQuadStableBufferHookInitFacet
 * @notice Source-only IFacet mixin inherited by the DFPkg. Never CREATE3-deployed.
 */
abstract contract UniswapV4StandardExchangeCurveQuadStableBufferHookInitFacet is
    UniswapV4StandardExchangeCurveQuadStableBufferHookInitTarget,
    IFacet
{
    function facetName() public pure returns (string memory) {
        return type(UniswapV4StandardExchangeCurveQuadStableBufferHookInitFacet).name;
    }

    function facetInterfaces() public pure virtual returns (bytes4[] memory interfaces) {
        interfaces = new bytes4[](1);
        interfaces[0] = type(IUniswapV4StandardExchangeCurveQuadStableBufferHookInit).interfaceId;
    }
