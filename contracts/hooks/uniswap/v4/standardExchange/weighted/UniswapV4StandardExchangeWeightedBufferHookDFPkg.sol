 ...
 * @notice Hook diamond package: ERC20Permit LP + MultiAsset vault + SE weighted product facets.
 * @dev deployVault → registry.deployHookVault → hook CREATE2 factory. Salt excludes package address.
 *      postDeploy ensures all binom(n,2) pair doors with DYNAMIC_FEE_FLAG.
 */
    ...
    IUniswapV4StandardExchangeWeightedBufferHookPackage
{
    ...
    function calcSalt(bytes memory pkgArgs) public pure returns (bytes32 salt) {
        ...
        return keccak256(
            abi.encode(
                PRODUCT_ID, a.n, a.tokens, a.weights, a.standardExchanges, a.rateProviders
            )
        );
    }
    ...
    function postDeploy(address proxy) public returns (bool) {
        IUniswapV4StandardExchangeWeightedBufferHook h =
            IUniswapV4StandardExchangeWeightedBufferHook(proxy);
