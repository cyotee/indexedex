...
import {
    UniswapV4SingleStandardExchangeBufferConstantProductHookInitTarget
} from "contracts/hooks/uniswap/v4/standardExchange/constantProduct/single/UniswapV4SingleStandardExchangeBufferConstantProductHookInitTarget.sol";

/**
 * @title UniswapV4SingleStandardExchangeBufferConstantProductHookInitFacet
 * @notice Source-only IFacet mixin inherited by the DFPkg (S47). Never CREATE3-deployed.
 */
abstract contract UniswapV4SingleStandardExchangeBufferConstantProductHookInitFacet is
    UniswapV4SingleStandardExchangeBufferConstantProductHookInitTarget,
    IFacet
{
    function facetName() public pure returns (string memory) {
        return type(UniswapV4SingleStandardExchangeBufferConstantProductHookInitFacet).name;
    }

    function facetInterfaces() public pure virtual returns (bytes4[] memory interfaces) {
        interfaces = new bytes4[](1);
        interfaces[0] = type(IUniswapV4SingleStandardExchangeBufferConstantProductHookInit).interfaceId;
    }
