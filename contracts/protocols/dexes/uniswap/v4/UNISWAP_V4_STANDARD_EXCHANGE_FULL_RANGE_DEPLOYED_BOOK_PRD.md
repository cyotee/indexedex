...
| **D41** | In Multi = join | `exchangeInManyToOne` / preview: `tokenOut` **must** be vault shares (`address(this)`). `tokenIn[]` is **exactly the two pool currencies in PoolKey order**, with native ETH represented by WETH as in `vaultTokens()`. Ordinary ERC20 pools remain strictly ascending; the WETH face of native currency0 need not sort before currency1. `amountsIn.length == 2`, every `amountsIn[i] > 0`. Length 1, extra tokens, reversed pool order, duplicates, or `tokenOut != shares` → `ExchangeInNotAvailable` (or equivalent). |
| **D42** | Out Multi = exit | `exchangeOutOneToMany` / preview: `tokenIn` **must** be vault shares. `tokensOut[]` follows the same exact PoolKey/ERC20-face order as D41, `amountsOut.length == 2`, every `amountsOut[i] > 0`. Length 1, reversed order or any other set → `ExchangeOutNotAvailable` (or equivalent). |
