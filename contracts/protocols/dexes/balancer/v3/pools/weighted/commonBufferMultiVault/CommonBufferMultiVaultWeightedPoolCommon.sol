...
/**
 * @title CommonBufferMultiVaultWeightedPoolCommon
 * @notice Math balances + derived share depth + need scores (L7/L20).
 */
abstract contract CommonBufferMultiVaultWeightedPoolCommon {
        ...
        returns (uint256 rate, uint256 scalingFactor)
    {
        ...
        if (rate == 0) revert ICommonBufferMultiVaultWeightedPool.RateProviderZero();
    }
    ...
    function _mathBalanceAt(uint256 tokenIndex, uint256[] memory balancesLiveScaled18)
        ...
        returns (uint256)
    {
        (ICommonBufferMultiVaultWeightedPool.TokenKind kind, uint256 leg) = Repo._resolveTokenIndex(tokenIndex);
    ...
    /// @dev Score s_i = d_i * 1e18 / w_i (L7).
    function _score(uint256 vaultIndex, uint256[] memory balancesLiveScaled18) internal view returns (uint256) {
        uint256 d = _derivedShareDepth(vaultIndex, balancesLiveScaled18);
        uint256 w = Repo._weight(Repo._shareIndex(vaultIndex));
        return Math.mulDiv(d, 1e18, w);
    }

    /**
     * @dev L20 tie-break when comparing two vaults for "better deposit target" (lower score wins):
     *      equal score → larger weight wins (preferred for deposit), then lower index.
     * @return true if `a` is a better deposit target than `b` (should come first in deposit rank).
     */
        ...
        returns (bool)
    {
        ...
        uint256 wa = Repo._weight(Repo._shareIndex(a));
        uint256 wb = Repo._weight(Repo._shareIndex(b));
        if (wa > wb) return true;
        if (wa < wb) return false;
        return a < b;
    }

    /// @return true if `a` is better redeem source than `b` (higher score first; L20 same ties).
    ...
    function _rankDeposit(uint256[] memory balancesLiveScaled18) internal view returns (uint8[] memory order) {
        ...
        // Insertion sort by deposit preference.
