...
/**
 * @title CommonBufferMultiVaultWeightedPoolHookTarget
 * @notice Always-route pre-seat/reconcile with walk-next-vault (L6/L21); L23 buffer-only remove ban.
 */
abstract contract CommonBufferMultiVaultWeightedPoolHookTarget is CommonBufferMultiVaultWeightedPoolCommon, IHooks {
        ...
        returns (bool)
    {
        ...
        for (uint256 i; i < u; ++i) {
            if (exactAmountsIn[Repo._unpairedIndex(i)] == 0) {
                revert ICommonBufferMultiVaultWeightedPool.InitialInvariantTooSmall();
            }
        }
        uint256 virtualInit = exactAmountsIn[Repo._bufferIndex()];
        if (virtualInit == 0) revert ICommonBufferMultiVaultWeightedPool.InitialInvariantTooSmall();
    ...
    function onBeforeRemoveLiquidity(
        ...
        if (
            ...
            if (!anyOther) revert ICommonBufferMultiVaultWeightedPool.BufferOnlyRemoveDisallowed();
        }
        return true;
    }
    ...
    function onBeforeSwap(PoolSwapParams calldata params, address pool) public virtual override returns (bool) {
        ...
        (ICommonBufferMultiVaultWeightedPool.TokenKind kindOut,) = Repo._resolveTokenIndex(params.indexOut);
        if (kindOut != ICommonBufferMultiVaultWeightedPool.TokenKind.Buffer) {
            return true;
        }
        ...
        returns (bool, uint256)
    {
        ...
        (ICommonBufferMultiVaultWeightedPool.TokenKind kindOut,) = Repo._resolveToken(address(params.tokenOut));
        if (kindOut == ICommonBufferMultiVaultWeightedPool.TokenKind.Buffer) {
            uint256 actualOut = params.amountOutScaled18;
            Repo._clearPendingPreSeat();
            uint256 vtNow = Repo._virtualBuffer();
            if (actualOut > vtNow) {
                revert ICommonBufferMultiVaultWeightedPool.VirtualBufferUnderflow(vtNow, actualOut);
            }
    ...
    function _preSeatBufferWalk(PoolSwapParams calldata params, address pool) internal {
        ...
        if (x == 0) revert ICommonBufferMultiVaultWeightedPool.PoolBufferSideExhausted();
        ...
        if (balOut == 0) revert ICommonBufferMultiVaultWeightedPool.PoolBufferSideExhausted();
        if (balIn == 0) {
            (ICommonBufferMultiVaultWeightedPool.TokenKind kIn, uint256 pIn) =
                Repo._resolveTokenIndex(params.indexIn);
            if (kIn == ICommonBufferMultiVaultWeightedPool.TokenKind.Buffer) {
                revert ICommonBufferMultiVaultWeightedPool.PoolBufferSideExhausted();
            }
            if (kIn == ICommonBufferMultiVaultWeightedPool.TokenKind.Share) {
                revert ICommonBufferMultiVaultWeightedPool.PoolShareSideExhausted(pIn);
            }
            revert ICommonBufferMultiVaultWeightedPool.PoolUnpairedSideExhausted(pIn);
        }
        ...
        revert ICommonBufferMultiVaultWeightedPool.AllVaultsExhausted();
    }
    ...
    function _preSeatDonateAndDelta(
        uint8 vaultIdx,
        uint256 yBufferRaw,
        uint256 drainAmount,
        ...
        internal
    {
        ...
        if (bumpVirtual) revert ICommonBufferMultiVaultWeightedPool.AllVaultsExhausted();
    }
    ...
    function _reconcileExchangeIn(address seRouter, uint8 vaultIdx, uint256 xRaw)
        internal
