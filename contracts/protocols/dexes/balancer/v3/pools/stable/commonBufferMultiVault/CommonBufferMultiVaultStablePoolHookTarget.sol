...
/**
 * @title CommonBufferMultiVaultStablePoolHookTarget
 * @notice Always-route pre-seat/reconcile with walk-next-vault; StableMath pre-seat quote.
 */
abstract contract CommonBufferMultiVaultStablePoolHookTarget is CommonBufferMultiVaultStablePoolCommon, IHooks {
        ...
        returns (bool)
    {
        ...
        if (virtualInit == 0) revert ICommonBufferMultiVaultStablePool.InitialInvariantTooSmall();
        Repo._setVirtualBuffer(virtualInit);
        uint8 n = Repo._vaultCount();
        for (uint256 i; i < n; ++i) {
            if (exactAmountsIn[Repo._shareIndex(i)] == 0) {
                revert ICommonBufferMultiVaultStablePool.InitialInvariantTooSmall();
            }
    ...
    function onBeforeRemoveLiquidity(
        ...
        if (kind == RemoveLiquidityKind.SINGLE_TOKEN_EXACT_IN || kind == RemoveLiquidityKind.SINGLE_TOKEN_EXACT_OUT) {
            ...
            if (!anyOther) revert ICommonBufferMultiVaultStablePool.BufferOnlyRemoveDisallowed();
        }
        return true;
    }
    ...
    function onBeforeSwap(PoolSwapParams calldata params, address pool) public virtual override returns (bool) {
        ...
        (ICommonBufferMultiVaultStablePool.TokenKind kindOut,) = Repo._resolveTokenIndex(params.indexOut);
        if (kindOut != ICommonBufferMultiVaultStablePool.TokenKind.Buffer) {
            return true;
        }
    ...
    function onAfterSwap(AfterSwapParams calldata params) public virtual override returns (bool, uint256) {
        ...
        (ICommonBufferMultiVaultStablePool.TokenKind kindOut,) = Repo._resolveToken(address(params.tokenOut));
        if (kindOut == ICommonBufferMultiVaultStablePool.TokenKind.Buffer) {
            uint256 actualOut = params.amountOutScaled18;
            Repo._clearPendingPreSeat();
            uint256 vtNow = Repo._virtualBuffer();
            if (actualOut > vtNow) {
                revert ICommonBufferMultiVaultStablePool.VirtualBufferUnderflow(vtNow, actualOut);
            }
            Repo._setVirtualBuffer(vtNow - actualOut);
        }

        (ICommonBufferMultiVaultStablePool.TokenKind kindIn,) = Repo._resolveToken(address(params.tokenIn));
        if (kindIn == ICommonBufferMultiVaultStablePool.TokenKind.Buffer) {
    ...
    /// @dev Pre-seat quote via StableMath (not WeightedMath).
    ...
    function _preSeatBufferWalk(PoolSwapParams calldata params, address pool) internal {
        ...
        if (x == 0) revert ICommonBufferMultiVaultStablePool.PoolBufferSideExhausted();
        ...
        if (balOut == 0) revert ICommonBufferMultiVaultStablePool.PoolBufferSideExhausted();
        if (balIn == 0) {
            (ICommonBufferMultiVaultStablePool.TokenKind kIn, uint256 pIn) = Repo._resolveTokenIndex(params.indexIn);
            if (kIn == ICommonBufferMultiVaultStablePool.TokenKind.Buffer) {
                revert ICommonBufferMultiVaultStablePool.PoolBufferSideExhausted();
            }
            revert ICommonBufferMultiVaultStablePool.PoolShareSideExhausted(pIn);
        }

        IVault vault = IVault(_balancerV3Vault());
        uint256 yBufferScaled18 = _quoteBufferOut(params, pool, vault);
        if (yBufferScaled18 > x) {
            revert ICommonBufferMultiVaultStablePool.VirtualBufferUnderflow(x, yBufferScaled18);
        }
        uint256 yBufferRaw = _bufferToRaw(yBufferScaled18);
        if (yBufferRaw == 0) revert ICommonBufferMultiVaultStablePool.PoolBufferSideExhausted();
        ...
        revert ICommonBufferMultiVaultStablePool.AllVaultsExhausted();
    }
    ...
    function _depositPhysicalBuffer(uint256 amount, address seRouter, bool bumpVirtual, uint256 virtualBump) internal {
        ...
        if (bumpVirtual) revert ICommonBufferMultiVaultStablePool.AllVaultsExhausted();
    }
