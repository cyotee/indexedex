...
/**
 * @title CommonBufferMultiVaultStablePoolCommon
 * @notice Math balances + derived share depth + depth-only ranking (S2/S12).
 */
abstract contract CommonBufferMultiVaultStablePoolCommon {
    function _vaultTokenRateAndScale(uint256 tokenIndex) internal view returns (uint256 rate, uint256 scalingFactor) {
        ...
        if (rate == 0) revert ICommonBufferMultiVaultStablePool.RateProviderZero();
    }
    ...
    function _mathBalanceAt(uint256 tokenIndex, uint256[] memory balancesLiveScaled18) internal view returns (uint256) {
        (ICommonBufferMultiVaultStablePool.TokenKind kind, uint256 leg) = Repo._resolveTokenIndex(tokenIndex);
    ...
    /// @dev Score = derived depth only (S2). No weights.
    ...
    /**
     * @dev Deposit preference: lower depth first; ties → lower vault index (S12).
     * @return true if `a` is a better deposit target than `b`.
     */
    ...
    /// @return true if `a` is better redeem source than `b` (higher depth first; ties → lower index).
