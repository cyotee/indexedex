    ...
    IMixedBufferMultiVaultStablePoolPkg
{
    ...
    MixedBufferMultiVaultStablePoolStandardVaultPkg public immutable SELF;
    ...
    function vaultFeeTypeIds() public pure returns (bytes32 vaultFeeTypeIds_) {
        return VaultTypeUtils._insertFeeTypeId(
            vaultFeeTypeIds_, VaultFeeType.DEX, type(IMixedBufferMultiVaultStablePoolPkg).interfaceId
        );
    }
    ...
    function initAccount(bytes memory initArgs) public {
        ...
        MixedBufferMultiVaultStablePoolRepo.InitParams memory init = _buildInitParams(a);
        ...
        StandardVaultRepo._initialize(
            VAULT_FEE_ORACLE, vaultFeeTypeIds(), vaultTypes(), abi.encode(tokens, a.unpairedCount, a.vaultCount)._hash()
        );

        string memory name_ = "BV3MixedBufferMultiVaultStable";
        ERC20Repo._initialize(name_, "mbmvsBPT", 18);
        ...
        MixedBufferMultiVaultStablePoolRepo._initialize(init);
    }

    function _buildInitParams(PkgArgs memory a)
        internal
        view
        returns (MixedBufferMultiVaultStablePoolRepo.InitParams memory init)
    {
        (
            ,
        ...
        init = MixedBufferMultiVaultStablePoolRepo.InitParams({
    ...
    function _tokensFromInit(MixedBufferMultiVaultStablePoolRepo.InitParams memory init)
        internal
        pure
    ...
    function _validatePkgArgs(PkgArgs memory a) internal view {
        if (a.unpairedCount < 1 || a.vaultCount < 1 || a.vaultCount > 3) {
            revert IMixedBufferMultiVaultStablePool.InvalidTokenLayout(a.unpairedCount, a.vaultCount);
        }
        uint256 n = uint256(a.unpairedCount) + 1 + uint256(a.vaultCount);
        if (n < 3 || n > 5 || a.unpairedCount > 3) {
            revert IMixedBufferMultiVaultStablePool.InvalidTokenLayout(a.unpairedCount, a.vaultCount);
        }
        ...
        if (
            a.unpairedTokens.length != a.unpairedCount || a.unpairedRateProviders.length != a.unpairedCount
                || a.standardExchangeVaults.length != a.vaultCount || a.vaultShareRateProviders.length != a.vaultCount
        ) {
            revert IMixedBufferMultiVaultStablePool.ArrayLengthMismatch();
        }

        // M9: each vault must accept+produce bufferToken.
        for (uint256 i; i < a.vaultCount; ++i) {
            if (!_vaultAcceptsAndProducesBuffer(a.standardExchangeVaults[i], a.bufferToken)) {
                revert IMixedBufferMultiVaultStablePool.BufferTokenNotInVault(
    ...
    function _sortAndCheckDuplicates(LayoutWork memory w) internal pure {
        ...
        for (uint256 i = 1; i < n; ++i) {
            if (w.all[i] == w.all[i - 1]) {
                revert IMixedBufferMultiVaultStablePool.DuplicatePoolToken(w.all[i]);
            }
        }
    }
