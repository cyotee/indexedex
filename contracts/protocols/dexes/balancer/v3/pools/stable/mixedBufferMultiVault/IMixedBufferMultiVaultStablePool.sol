...
/**
 * @title IMixedBufferMultiVaultStablePool
 * @notice U unpaired free legs + 1 bufferToken → N SE vaults under StableMath (T=U+1+N, U≥1, 1≤N≤3, 3≤T≤5).
 * @dev Routing by derived share depth only (no weights). Unpaired never SE-buffered.
 */
interface IMixedBufferMultiVaultStablePool {
    enum TokenKind {
        Unpaired,
        Buffer,
        Share
    ...
    /// @notice Derived share depth d_i for vault i.
