...
} from "contracts/protocols/dexes/camelot/v2/CamelotV2StandardExchangeCommon.sol";

contract CamelotV2StandardExchangeOutTarget is
    CamelotV2StandardExchangeCommon,
    ...
    IStandardExchangeOut
{
        ...
        returns (uint256 amountIn)
    {
        ...
        if (
            ...
            // return amountIn;
            // Load pool reserves.
            // (indexSource.knownReserve, indexSource.opposingReserve, ) = indexSource.pool.getReserves();
            // // Sort reserves to match tokenIn/tokenOut order.
            // (indexSource.knownReserve, indexSource.opposingReserve) = ConstProdUtils._sortReserves(
            //     // address knownToken,
            //     address(tokenIn),
            //     // address token0,
            ...
            ) = CamelotV2Service._sortReserves(indexSource.pool, tokenIn);
            // Calculate the amount in required to purchase the requested amount out.
        ...
        if (address(tokenIn) == address(indexSource.pool) && address(tokenOut) == address(this)) {
            ...
            CamelotV2StrategyVault memory vault;
    ...
    function exchangeOut(
        ...
        if (
            ...
            ICamelotV2Router camelotV2Router = CamelotV2RouterAwareRepo._camelotV2Router();
            ...
            // amountIn = camelotV2Router.getAmountIn(amountOut, indexSource.knownReserve, indexSource.opposingReserve);
        ...
        if (
            ...
            // Load the router.
            // IUniswapV2Router router_ = _camelotV2Router();
            amountOut = indexSource.pool
                ._withdrawSwapDirect(
                    // IUniswapV2Pair pool,
                    // IUniswapV2Router router,
                    CamelotV2RouterAwareRepo._camelotV2Router(),
                    // uint256 amt,
                    amountIn,
                    // IERC20 tokenOut,
                    tokenOut,
    ...
    // function _binarySearchZapInForExactLP(
    //     IERC20 tokenIn,
    //     /// forge-lint: disable-next-line(mixed-case-variable)
    //     uint256 targetLPOut
    // ) internal view returns (uint256 requiredInput) {
    //     uint256 low = 1;
    //     uint256 high = type(uint128).max; // Reasonable upper bound

    //     while (low < high) {
    //         uint256 mid = (low + high) / 2;
    //         uint256 lpOut = _simulateZapInToLP(tokenIn, mid);

    //         if (lpOut < targetLPOut) {
    //             low = mid + 1;
    //         } else {
    //             high = mid;
    //         }
    ...
    // function _binarySearchZapInForExactShares(
    //     IERC20 tokenIn,
    //     uint256 targetSharesOut
    // ) internal view returns (uint256 requiredInput) {
    //     uint256 low = 1;
    //     uint256 high = type(uint128).max;

    //     while (low < high) {
    //         uint256 mid = (low + high) / 2;
    //         uint256 sharesOut = _simulateZapInToShares(tokenIn, mid);

    //         if (sharesOut < targetSharesOut) {
    //             low = mid + 1;
    //         } else {
    //             high = mid;
    //         }
    ...
    // function _binarySearchZapOutForExactToken(
    //     IERC20 tokenOut,
    //     uint256 targetTokenOut
    // ) internal view
    // /// forge-lint: disable-next-line(mixed-case-variable)
    // returns (uint256 requiredLPInput) {
    //     uint256 low = 1;
    //     uint256 high = type(uint128).max;

    //     while (low < high) {
    //         uint256 mid = (low + high) / 2;
    //         uint256 tokenOutAmount = _simulateZapOutFromLP(tokenOut, mid);

    //         if (tokenOutAmount < targetTokenOut) {
    //             low = mid + 1;
    //         } else {
    //             high = mid;
    //         }
    ...
    // function _binarySearchZapOutFromVaultForExactToken(
    //     IERC20 tokenOut,
    //     uint256 targetTokenOut
    // ) internal view returns (uint256 requiredSharesInput) {
    //     uint256 low = 1;
    //     uint256 high = type(uint128).max;

    //     while (low < high) {
    //         uint256 mid = (low + high) / 2;
    //         uint256 tokenOutAmount = _simulateZapOutFromVault(tokenOut, mid);
    ...
    // /// forge-lint: disable-next-line(mixed-case-function)
    // function _simulateZapInToLP(
    //     IERC20 tokenIn,
    //     uint256 amountIn
    // ) internal view returns (uint256 lpOut) {
    //     ICamelotPair pool_ = _pool();
    //     CamelotV2Service.ReserveInfo memory reserves = CamelotV2Service._sortReserves(pool_, tokenIn);
    ...
    //     uint256 swapOutput = ConstProdUtils._saleQuote(
    //         swapAmount,
    //         reserves.reserveIn,
    //         reserves.reserveOut,
    //         reserves.feePercent
    ...
    //     uint256 remainingTokenIn = amountIn - swapAmount;
    //     return ConstProdUtils._depositQuote(
    //         remainingTokenIn,
    //         swapOutput,
    //         pool_.totalSupply(),
    //         reserves.reserveIn + swapAmount,
    //         reserves.reserveOut - swapOutput
    //     );
    // }

    // function _simulateZapInToShares(
    //     IERC20 tokenIn,
    //     uint256 amountIn
    // ) internal view returns (uint256 sharesOut) {
    //     uint256 lpTokens = _simulateZapInToLP(tokenIn, amountIn);

    //     uint256 reserve_ = _reserveOfToken(IERC20(address(_pool())));
    //     uint256 totalShares_ = _totalSupply();

    //     return _convertToShares(lpTokens, reserve_, totalShares_);
    // }

    // /// forge-lint: disable-next-line(mixed-case-function)
    // function _simulateZapOutFromLP(
    //     IERC20 tokenOut,
    //     uint256 lpAmountIn
    // ) internal view returns (uint256 tokenOutAmount) {
    //     ICamelotPair pool_ = _pool();
    //     (uint112 reserve0, uint112 reserve1, , ) = pool_.getReserves();
    //     uint256 totalSupply = pool_.totalSupply();

    //     (uint256 amount0, uint256 amount1) = ConstProdUtils._withdrawQuote(
    //         lpAmountIn,
    //         totalSupply,
    //         uint256(reserve0),
    //         uint256(reserve1)
    ...
    //     CamelotV2Service.ReserveInfo memory reserves = CamelotV2Service._sortReserves(pool_, tokenOut);

    //     uint256 amountToSwap = address(tokenOut) == pool_.token0() ? amount1 : amount0;
    //     uint256 directAmount = address(tokenOut) == pool_.token0() ? amount0 : amount1;

    //     uint256 swapOutput = ConstProdUtils._saleQuote(
    //         amountToSwap,
    //         reserves.reserveOut - amountToSwap,
    //         reserves.reserveIn - directAmount,
    //         reserves.feePercent
    //     );

    //     return directAmount + swapOutput;
    // }

    // function _simulateZapOutFromVault(
    //     IERC20 tokenOut,
    //     uint256 sharesIn
    // ) internal view returns (uint256 tokenOutAmount) {
    //     uint256 reserve_ = _reserveOfToken(IERC20(address(_pool())));
    //     uint256 totalShares_ = _totalSupply();
    //     uint256 lpTokens = _convertToAssets(sharesIn, reserve_, totalShares_);

    //     return _simulateZapOutFromLP(tokenOut, lpTokens);
    // }
}
