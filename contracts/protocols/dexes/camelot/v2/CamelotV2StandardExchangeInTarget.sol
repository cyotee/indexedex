...
contract CamelotV2StandardExchangeInTarget is
    CamelotV2StandardExchangeCommon,
    ...
    IStandardExchangeIn
{
        ...
        returns (uint256 amountOut)
    {
        ...
        if (
            ...
            // (uint256 knownReserve, uint256 opposingReserve, uint256 knownFeePercent, /*uint25 opposingFeePercent*/) = CamelotV2Service._sortReserves(pool, tokenIn);
    ...
    function exchangeIn(
        ...
        // Determine actual token route from provided tokens.
        // Intended supported routes.
        // 1. Swap of token contained in the underlying pool for the opposing token contained in the underlying pool.
        //    Implemented in first branch.
        // 2. Deposit as ZapIn of token contained in the underlying pool for the underlying pool token.
        //    Implemented in second branch.
        // 3. Deposit of the underlying pool token into the vault.
        //    Implemented in third branch.
        // 4. Deposit as ZapIn of the token contained in the underlying pool for the underlying pool token into the vault.
        //    Implemented in fourth branch.
        // 5. Withdraw of the underlying pool token from the vault.
        //    Implemented in fifth branch.
        // 6. Withdraw as ZapOut of token contained in the underlying pool from the underlying pool as tokenIn.
        //    Implemented in sixth branch.
        // 7. Withdraw as ZapOut of token contained in the underlying pool from the underlying pool token from the vault.
        ...
        if (
            ...
            amountOut = CamelotV2RouterAwareRepo._camelotV2Router()
                ._swap(
