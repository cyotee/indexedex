...
import {HermeticWETH} from "contracts/protocols/staking/rocket-pool/test/hermetic/HermeticRocketPoolPorts.sol";

/**
 * @title HostileWETH
 * @notice WETH double that reenters a target mid transfer/transferFrom (adversarial harness only).
 * @dev Records nested call result so tests can assert IReentrancyLock.IsLocked.
 */
