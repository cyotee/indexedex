...
contract LidoWstETHStandardExchangeDFPkg is ILidoWstETHStandardExchangeDFPkg {
    using BetterEfficientHashLib for bytes;

    LidoWstETHStandardExchangeDFPkg public immutable SELF;
    ...
    function deployVault(address stETH_, address wstETH_, address weth_, address withdrawalQueue_)
        public
        returns (address vault)
    {
        if (stETH_ == address(0) || wstETH_ == address(0) || weth_ == address(0)) revert ZeroAddress();
        vault = VAULT_REGISTRY_DEPLOYMENT.deployVault(
            SELF,
            abi.encode(
                PkgArgs({stETH: stETH_, wstETH: wstETH_, weth: weth_, withdrawalQueue: withdrawalQueue_})
            )
        );
    }

    function name() public pure returns (string memory) {
        return type(LidoWstETHStandardExchangeDFPkg).name;
    }

    function vaultFeeTypeIds() public pure returns (bytes32 vaultFeeTypeIds_) {
        vaultFeeTypeIds_ = VaultTypeUtils._insertFeeTypeId(
            vaultFeeTypeIds_, VaultFeeType.USAGE, type(ILidoWstETHStandardVault).interfaceId
        );
    }
    ...
    function initAccount(bytes memory initArgs) public {
        ...
        string memory name_ = "IndexedEx Lido wstETH SE";
        ERC20Repo._initialize(name_, "ixLidoWst", 18);
        EIP712Repo._initialize(name_, "1");
        // ERC-4626 asset = wstETH (primary locked reserve).
        // decimalOffset=3 enables BetterMath virtual shares inflation defense (OZ-style).
        ERC4626Repo._initialize(IERC20(args.wstETH), 18, 3);
        ERC4626Repo._setLastTotalAssets(0);

        address[] memory vaultTokens = new address[](3);
        vaultTokens[0] = args.wstETH;
        vaultTokens[1] = args.weth;
        vaultTokens[2] = args.stETH;
        ...
        LidoWstETHStandardExchangeRepo._initialize(args.stETH, args.wstETH, args.weth, args.withdrawalQueue);
    }
    ...
    {
        name_ = "LidoWstETHStandardExchangeDFPkg";
