...
/**
 * @title TestBase_BaseBaseDualSelfCommonDETF
 ...
 *      - Seigniorage capture
 */
contract TestBase_BaseDualSelfCommonDETF is TestBase_AerodromeStandardExchange {
    ...
    function _deployProtocolFacets() internal virtual {
        ...
        protocolDETFExchangeInFacet = create3Factory.deployBaseDualSelfCommonDETFExchangeInFacet();
        protocolDETFExchangeOutFacet = create3Factory.deployBaseDualSelfCommonDETFExchangeOutFacet();
        protocolDETFBondingFacet = create3Factory.deployBaseDualSelfCommonDETFBondingFacet();
        protocolDETFBridgeFacet = create3Factory.deployBaseDualSelfCommonDETFBridgeFacet();
    ...
    function _deployBaseDualSelfCommonDETF() internal virtual {
    ...
    function _bondWithWeth(address user, uint256 wethAmount, uint256 lockDuration) internal returns (uint256 tokenId) {
        vm.startPrank(user);
        IERC20(address(weth)).approve(address(protocolDETF), wethAmount);
        (tokenId,) = IBaseDualSelfCommonDETFBonding(address(protocolDETF))
    ...
    function _sellNFT(address user, uint256 tokenId) internal returns (uint256 richirAmount) {
        vm.startPrank(user);
        richirAmount = IBaseDualSelfCommonDETFBonding(address(protocolDETF)).sellNFT(tokenId, user);
        vm.stopPrank();
    }
