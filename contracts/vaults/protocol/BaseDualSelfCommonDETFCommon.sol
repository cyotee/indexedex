...
/**
 * @title BaseDualSelfCommonDETFCommon
 ...
 */
abstract contract BaseDualSelfCommonDETFCommon is AerodromeDualEmbeddedDETFCommon {
     ...
     */
    function _loadPoolReserves(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, PoolReserves memory reserves_)
        internal
        view
        virtual
    {
    ...
    function _addSingleSidedVaultSharesToReservePool(
        ...
        if (tokenIndexIn_ == layoutStruct_.chirWethVaultIndex) {
            ...
            revert BaseDualSelfCommonDETFRepo.TokenNotSupported();
        }
    ...
    function _previewBridgeRichirQuote(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 targetChainId_, uint256 richirAmount_)
        internal
        view
    ...
    function _previewClaimLiquidityLpOut(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 lpAmount_)
        internal
        view
    ...
    function _currentSyntheticPrice(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_)
        internal
        view
    ...
    function _currentMintingAllowed(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_)
        internal
        view
    ...
    function _currentBurningAllowed(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_)
        internal
        view
        returns (bool allowed_)
    {
    ...
    function _previewReservePoolExitForBridge(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 bptIn_)
        internal
        view
    ...
    function _previewLocalRichirCompensation(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 chirWethVaultShares_)
        internal
        view
     ...
     */
    function _isMintingAllowed(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 syntheticPrice_)
        internal
        view
     ...
     */
    function _isBurningAllowed(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 syntheticPrice_)
        internal
        view
        returns (bool allowed)
    {
        ...
        returns (uint256 vaultShares_)
    {
        ...
        if (sim.compoundLP > 0) {
            protocolFeeLP = BetterMath._percentageOfWAD(
                sim.compoundLP, BaseDualSelfCommonDETFRepo._feeOracle().usageFeeOfVault(address(vault_))
            );
        }
     ...
     */
    function _calcSeigniorage(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, SeigniorageCalc memory calc_, uint256 amountIn_)
        internal
        view
    {
     ...
     */
    function _isWethToken(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, IERC20 token_) internal view returns (bool) {
        return address(token_) == address(layoutStruct_.wethToken);
    }

    function _isWethToken(IERC20 token_) internal view returns (bool) {
        return _isWethToken(BaseDualSelfCommonDETFRepo._layoutStruct(), token_);
    }
     ...
     */
    function _isRichToken(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, IERC20 token_) internal view returns (bool) {
        return address(token_) == address(layoutStruct_.richToken);
    }

    function _isRichToken(IERC20 token_) internal view returns (bool) {
        return _isRichToken(BaseDualSelfCommonDETFRepo._layoutStruct(), token_);
    }
     ...
     */
    function _isRichirToken(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, IERC20 token_) internal view returns (bool) {
        return address(token_) == address(layoutStruct_.richirToken);
    }

    function _isRichirToken(IERC20 token_) internal view returns (bool) {
        return _isRichirToken(BaseDualSelfCommonDETFRepo._layoutStruct(), token_);
    }
    ...
    function _isInitialized() internal view returns (bool) {
        return BaseDualSelfCommonDETFRepo._isReservePoolInitialized();
    }
        ...
        returns (uint256 actualIn_)
    {
        if (pretransferred_) {
            require(
                tokenIn_.balanceOf(address(this)) >= amount_, "BaseDualSelfCommonDETFCommon: insufficient pretransferred balance"
            );
            return amount_;
        }
    ...
    function _loadChirWethLiquidityQuote(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_)
        internal
        view
    ...
    function _quoteBalancedChirForWethDeposit(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 wethIn_)
        internal
        view
    ...
    function _quoteBalancedChirWethDepositAmounts(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 wethIn_)
        internal
        view
    ...
    function _previewBalancedChirWethLpOut(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 wethIn_)
        internal
        view
    ...
    function _previewBalancedChirWethVaultShares(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 wethIn_)
        internal
        view
        returns (uint256 vaultShares_)
    {
