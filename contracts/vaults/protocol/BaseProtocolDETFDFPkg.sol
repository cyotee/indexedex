...
/**
 * @title IBaseProtocolDETFDFPkg
 * @notice Interface for Protocol DETF Diamond Factory Package.
 */
interface IBaseProtocolDETFDFPkg is IDiamondFactoryPackage {
    struct PkgInit {
        IFacet erc20Facet;
        IFacet erc5267Facet;
        IFacet erc2612Facet;
...
/**
 * @title BaseProtocolDETFDFPkg
 * @author cyotee doge <not_cyotee@proton.me>
 * @notice Diamond Factory Package for deploying Protocol DETF (CHIR) vaults.
 */
contract BaseProtocolDETFDFPkg is IBaseProtocolDETFDFPkg, IStandardVaultPkg {
    ...
    function packageName() public pure returns (string memory name_) {
        return type(BaseProtocolDETFDFPkg).name;
    }

    function facetAddresses() public view returns (address[] memory facetAddresses_) {
        facetAddresses_ = new address[](13);
        ...
        facetAddresses_[10] = address(MULTI_STEP_OWNABLE_FACET);
        facetAddresses_[11] = address(OPERABLE_FACET);
        facetAddresses_[12] = address(PROTOCOL_DETF_RICHIR_REDEEM_FACET);
    }
    ...
    function facetInterfaces() public pure returns (bytes4[] memory interfaces) {
        ...
        interfaces[13] = type(IBaseProtocolDETFRichirRedeem).interfaceId;
    }
    ...
    function facetCuts() public view returns (IDiamond.FacetCut[] memory facetCuts_) {
        facetCuts_ = new IDiamond.FacetCut[](13);
    ...
    function updatePkg(address expectedProxy, bytes memory pkgArgs) public virtual returns (bool) {
        ...
        if (args.funder == address(0)) {
            revert("BaseProtocolDETFDFPkg: no funder");
        }
