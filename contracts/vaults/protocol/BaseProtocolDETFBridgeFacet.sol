...
import {BaseProtocolDETFBridgeTarget} from "contracts/vaults/protocol/BaseProtocolDETFBridgeTarget.sol";

contract BaseProtocolDETFBridgeFacet is BaseProtocolDETFBridgeTarget, IFacet {
    function facetName() external pure returns (string memory name) {
        return type(BaseProtocolDETFBridgeFacet).name;
    }
    ...
    {
        name = type(BaseProtocolDETFBridgeFacet).name;
