...
/**
 * @title BaseProtocolDETFBondingQueryTarget
 * @author cyotee doge <not_cyotee@proton.me>
 * @notice View-only query functions for Protocol DETF bonding operations.
 * @dev Split from BaseProtocolDETFBondingTarget to meet EIP-170 contract size limit.
...
contract BaseProtocolDETFBondingQueryTarget is BaseProtocolDETFCommon {
    ...
    function previewClaimLiquidity(uint256 lpAmount) external view returns (uint256 wethOut) {
        ...
        wethOut = _previewWethOutFromAerodromeLp(poolAddr, lpOut, address(layout.wethToken));
    }
