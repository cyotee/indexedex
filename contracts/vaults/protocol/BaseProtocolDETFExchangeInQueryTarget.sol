...
/**
 * @title BaseProtocolDETFExchangeInQueryTarget
 * @author cyotee doge <not_cyotee@proton.me>
 * @notice View-only query functions for Protocol DETF exchange-in previews.
 * @dev Split from BaseProtocolDETFExchangeInTarget to meet EIP-170 contract size limit.
 *      Contains all preview/query functions while the execute target retains
 *      state-changing functions.
 */
contract BaseProtocolDETFExchangeInQueryTarget is BaseProtocolDETFCommon {
     ...
     *      6. WETH → RICHIR (convert WETH to RICHIR)
        ...
        returns (uint256 amountOut)
    {
        ...
        if (_isWethToken(layout, tokenIn) && _isChirToken(tokenOut)) {
            ...
            amountOut = _previewMintChirFromWeth(layout, amountIn);
            return amountOut;
        }
        ...
        if (_isRichToken(layout, tokenIn) && _isChirToken(tokenOut)) {
            ...
            amountOut = _previewMintChirFromWeth(layout, wethOut);
            return amountOut;
        }
    ...
    function _previewChirRedemptionUnwind(
        ...
        uint256 wethFromChirWeth = layout_.chirWethVault
            .previewExchangeIn(IERC20(address(layout_.chirWethVault)), chirWethVaultSharesOut_, layout_.wethToken);

        uint256 chirFromRichChir = layout_.richChirVault
        ...
        returns (uint256 richirOut_)
    {
        ...
        if (_buildCompoundSim(layout_.chirWethVault).compoundLP == 0) {
            richirOut_ =
                richirOut_ - ((richirOut_ * DIRECT_PREVIEW_EXTRA_BUFFER_BPS) / PREVIEW_BUFFER_DENOMINATOR);
        }
    }
    ...
    function _previewReservePoolBptOut(
        ...
        // Convert raw balances and amounts using the same conservative rounding
        // helpers used throughout ProtocolDETF.
