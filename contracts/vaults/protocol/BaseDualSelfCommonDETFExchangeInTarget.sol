...
/**
 * @title BaseDualSelfCommonDETFExchangeInTarget
 ...
 */
contract BaseDualSelfCommonDETFExchangeInTarget is BaseDualSelfCommonDETFCommon, ReentrancyLockModifiers {
     ...
     */
    function _exitAndUnwindToWeth(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, uint256 bptIn_, uint256 deadline_)
        internal
        returns (uint256 wethOut_)
    {
        (uint256 chirWethVaultSharesOut, uint256 richChirVaultSharesOut) = _exitReservePoolProportional(layoutStruct_, bptIn_);
    ...
    *      Unlike bonding through the unified WETH route, this path does not mint CHIR or create balanced LP.
     ...
     */
    function _executeRichirToRich(BaseDualSelfCommonDETFRepo.Storage storage layoutStruct_, ExchangeInParams memory p_)
        internal
        returns (uint256 amountOut_)
    {
        ...
        if (!layoutStruct_.allowedRichirRedeemAddresses._contains(msg.sender)) {
            revert BaseDualSelfCommonDETFRepo.NotAllowedRichirRedeem(msg.sender);
        }
        ...
        if (p_.pretransferred) {
            ...
            require(
                ...
                "BaseDualSelfCommonDETFCommon: insufficient pretransferred balance"
            );
