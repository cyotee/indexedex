...
/**
 * @title IBaseDualSelfCommonDETFDFPkg
 * @notice Interface for Protocol DETF Diamond Factory Package.
 */
interface IBaseDualSelfCommonDETFDFPkg is IDiamondFactoryPackage {
    struct PkgInit {
        IFacet erc20Facet;
        IFacet erc5267Facet;
        IFacet erc2612Facet;
        ...
        DualSelfCommonDETFSuperchainBridgeRepo.BridgeConfig bridgeConfig;
    }

    struct PkgArgs {
        string name;
        string symbol;
        BaseDualSelfCommonDETFRepo.ProtocolConfig protocolConfig;

        /// @notice Address to pull initial deposit funds from (Permit2 owner)
        /// @dev This is required because `updatePkg()` is called by the Diamond factory.
        address funder;

        /// @notice Owner address for MultiStepOwnable (initial owner of the diamond proxy)
        address owner;
    }
...
/**
 * @title BaseDualSelfCommonDETFDFPkg
 * @author cyotee doge <not_cyotee@proton.me>
 * @notice Diamond Factory Package for deploying Protocol DETF (CHIR) vaults.
 */
contract BaseDualSelfCommonDETFDFPkg is IBaseDualSelfCommonDETFDFPkg, IStandardVaultPkg {
    ...
    constructor(PkgInit memory pkgInit) {
        ...
        AERODROME_STANDARD_EXCHANGE_DFPKG = pkgInit.aerodromeStandardExchangeDFPkg;
    ...
    function packageName() public pure returns (string memory name_) {
        return type(BaseDualSelfCommonDETFDFPkg).name;
    }

    function facetAddresses() public view returns (address[] memory facetAddresses_) {
        facetAddresses_ = new address[](14);
        ...
        facetAddresses_[13] = address(PROTOCOL_DETF_RICHIR_REDEEM_FACET);
    }
    ...
    function facetInterfaces() public pure returns (bytes4[] memory interfaces) {
        ...
        interfaces[8] = type(IBaseDualSelfCommonDETFBonding).interfaceId;
        interfaces[9] = type(IBasicVault).interfaceId;
        interfaces[10] = type(IStandardVault).interfaceId;
        interfaces[11] = type(IMultiStepOwnable).interfaceId;
        interfaces[12] = type(IOperable).interfaceId;
        interfaces[13] = type(IBaseDualSelfCommonDETFRichirRedeem).interfaceId;
    }
    ...
    function facetCuts() public view returns (IDiamond.FacetCut[] memory facetCuts_) {
        facetCuts_ = new IDiamond.FacetCut[](14);
    ...
    function updatePkg(address expectedProxy, bytes memory pkgArgs) public virtual returns (bool) {
        ...
        if (args.funder == address(0)) {
            revert("BaseDualSelfCommonDETFDFPkg: no funder");
        }
    ...
    function initAccount(bytes memory initArgs) public {
        ...
        BaseDualSelfCommonDETFRepo._initialize(
        ...
        // Initialize MultiStepOwnable with owner from PkgArgs (1 day ownership buffer period)
    ...
    function _postDeployProxyContext(address expectedProxy) internal {
        BaseDualSelfCommonDETFRepo.Storage storage detfStorage = BaseDualSelfCommonDETFRepo._layoutStruct();
        BaseDualSelfCommonDETFRepo.ProtocolConfig memory cfg = BaseDualSelfCommonDETFRepo._protocolConfig(detfStorage);
    ...
    function _fundProxyAndDeployExchangeVaults(
        BaseDualSelfCommonDETFRepo.Storage storage detfStorage,
        address expectedProxy,
        BaseDualSelfCommonDETFRepo.ProtocolConfig memory cfg
    ) internal {
        ...
        // Approve Aerodrome DFPkg to move underlying tokens + CHIR out of this proxy.
        if (cfg.wethInitialDepositAmount > 0) {
            IERC20(cfg.wethToken).safeApprove(address(AERODROME_STANDARD_EXCHANGE_DFPKG), 0);
            IERC20(cfg.wethToken).safeApprove(address(AERODROME_STANDARD_EXCHANGE_DFPKG), cfg.wethInitialDepositAmount);
        }
        if (cfg.richInitialDepositAmount > 0) {
            IERC20(cfg.richToken).safeApprove(address(AERODROME_STANDARD_EXCHANGE_DFPKG), 0);
            IERC20(cfg.richToken).safeApprove(address(AERODROME_STANDARD_EXCHANGE_DFPKG), cfg.richInitialDepositAmount);
        }
        if (chirForWeth + chirForRich > 0) {
            ERC20Repo._approve(address(this), address(AERODROME_STANDARD_EXCHANGE_DFPKG), chirForWeth + chirForRich);
        }

        address chirWethVaultAddr = AERODROME_STANDARD_EXCHANGE_DFPKG.deployVault(
            IERC20(cfg.wethToken), cfg.wethInitialDepositAmount, IERC20(address(this)), chirForWeth, expectedProxy
        );

        address richChirVaultAddr = AERODROME_STANDARD_EXCHANGE_DFPKG.deployVault(
            IERC20(cfg.richToken), cfg.richInitialDepositAmount, IERC20(address(this)), chirForRich, expectedProxy
        );

        BaseDualSelfCommonDETFRepo._initializeExchangeVaults(
            detfStorage,
    ...
    function _deployReservePoolAndFinalize(BaseDualSelfCommonDETFRepo.Storage storage detfStorage) internal {
        address reservePool;
        address nftVault;
        address richir;
...

        {
            ...
            IRateProvider chirWethRateProvider = RATE_PROVIDER_PKG.deployRateProvider(
                BaseDualSelfCommonDETFRepo._chirWethVault(detfStorage), BaseDualSelfCommonDETFRepo._chirWethRateTarget(detfStorage)
            );

            IRateProvider richChirRateProvider = RATE_PROVIDER_PKG.deployRateProvider(
                BaseDualSelfCommonDETFRepo._richChirVault(detfStorage), BaseDualSelfCommonDETFRepo._richChirRateTarget(detfStorage)
            );

            TokenConfig memory highWeightTokenConfig = TokenConfig({
                token: OZIERC20(address(BaseDualSelfCommonDETFRepo._chirWethVault(detfStorage))),
            ...
            TokenConfig memory lowWeightTokenConfig = TokenConfig({
                token: OZIERC20(address(BaseDualSelfCommonDETFRepo._richChirVault(detfStorage))),
                tokenType: TokenType.WITH_RATE,
            ...
            uint256 swapFeePercentage =
                VAULT_FEE_ORACLE_QUERY.defaultDexSwapFeeOfTypeId(type(IBaseDualSelfCommonDETFBonding).interfaceId);
...

        {
            ...
            BaseDualSelfCommonDETFRepo._initializeReservePool(
                detfStorage,
                chirWethVaultIndex_,
                _EIGHTY,
                richChirVaultIndex_,
     ...
     */
    function _initializeReservePoolLiquidity(BaseDualSelfCommonDETFRepo.Storage storage detfStorage, address reservePool)
        internal
    {
