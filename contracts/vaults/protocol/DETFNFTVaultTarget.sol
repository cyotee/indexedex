...
import {DETFNFTVaultRepo} from "contracts/vaults/protocol/DETFNFTVaultRepo.sol";
import {DETFNFTVaultCommon} from "contracts/vaults/protocol/DETFNFTVaultCommon.sol";
import {DETFNFTVaultService} from "contracts/vaults/protocol/DETFNFTVaultService.sol";
...
contract DETFNFTVaultTarget is DETFNFTVaultCommon, ReentrancyLockModifiers, MultiStepOwnableModifiers {
    ...
    function protocolDETF() external view returns (IProtocolDETF) {
        return DETFNFTVaultRepo._protocolDETF();
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes calldata data) public virtual {
        if (to == address(DETFNFTVaultRepo._protocolDETF())) {
