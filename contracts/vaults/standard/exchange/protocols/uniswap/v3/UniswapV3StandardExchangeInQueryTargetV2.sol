...
/**
 * @title UniswapV3StandardExchangeInQueryTargetV2
 * @notice Sequential inventory-transition quotes; standard previews reside on InMultiQueryFacet.
 */
abstract contract UniswapV3StandardExchangeInQueryTargetV2 is UniswapV3StandardExchangeInBaseV2 {
    ...
    function _inventorySharesIn(InventoryQuote memory q, uint256 amount) private view returns (uint256) {
        ...
        uint256 below;
        uint256 above = _inventoryAssets(q, high);
        if (above < amount) return q.supply;
        uint256 probes;
        while (low < high) {
            uint256 mid = low + (high - low) / 2;
            // Keep projected exact-out quotes on the same bounded search as
            // the live withdrawal path.
            if (above > below && probes < 8) {
                mid = low - 1 + Math.mulDiv(amount - below, high - low + 1, above - below);
