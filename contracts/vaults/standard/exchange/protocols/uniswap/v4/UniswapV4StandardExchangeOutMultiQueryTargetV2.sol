...
contract UniswapV4StandardExchangeOutMultiQueryTargetV2 is UniswapV4StandardExchangeOutBaseV2, NativeStandardYieldTarget {
    ...
    function previewExchangeOutOneToMany(
        ...
        if (!canOpenPoolManagerUnlock()) {
            ...
            if (free0 < amount0) {
                revert UniswapV4Exchange_InsufficientLocalReserve(_token0(), amount0, free0);
            }
            if (free1 < amount1) {
                revert UniswapV4Exchange_InsufficientLocalReserve(_token1(), amount1, free1);
            }
        }
        return s0;
    }
