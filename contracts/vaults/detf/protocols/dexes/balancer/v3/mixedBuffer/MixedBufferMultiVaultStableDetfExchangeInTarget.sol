...
import {MixedBufferMultiVaultStableDetfExchangeOutTarget} from "./MixedBufferMultiVaultStableDetfExchangeOutTarget.sol";
...
abstract contract MixedBufferMultiVaultStableDetfExchangeInTarget is MixedBufferMultiVaultStableDetfExchangeOutTarget {
    ...
    function _acquireDetf(IERC20 in_, uint256 amount_, bool prepaid_) internal returns (uint256) {
        ...
        Repo.Storage storage s_ = Repo._layoutStruct();
        if (!_isMintingAllowed()) return _reserveSwap(s_.reservePool, in_, IERC20(address(this)), actual_, 0);
        MintSplit memory split_ = _splitMintedDetf(_quoteOrdinaryDetf(in_, actual_));
        _custodyBptOnNft(_joinPayment(in_, actual_));
