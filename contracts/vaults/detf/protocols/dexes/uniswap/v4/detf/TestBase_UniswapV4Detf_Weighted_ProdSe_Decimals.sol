...
/**
 * @title TestBase_UniswapV4Detf_Weighted_ProdSe_Decimals
 ...
 */
abstract contract TestBase_UniswapV4Detf_Weighted_ProdSe_Decimals is TestBase_UniswapV4Detf_Decimals {
    ...
    function _finishWeightedProdSe() internal {
        ...
        se = se0;
        if (pairA != address(0)) pairToken = MintableERC20Decimals(pairA);
        _syncPairDecimals();
    }
    ...
    function _mintMintablePairs(uint256 amount) internal {
        MintableERC20Decimals(pairA).mint(detfUser, amount * (10 ** uint256(MintableERC20Decimals(pairA).decimals())));
        MintableERC20Decimals(pairB).mint(detfUser, amount * (10 ** uint256(MintableERC20Decimals(pairB).decimals())));
    }
