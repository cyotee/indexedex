...
import {
    UniswapV4StandardExchangeCurveQuadStableDETFExchangeOutTarget
} from "contracts/vaults/detf/protocols/dexes/uniswap/v4/standardExchange/stable/quad/curve/UniswapV4StandardExchangeCurveQuadStableDETFExchangeOutTarget.sol";
import {
...
/// @title UniswapV4StandardExchangeCurveQuadStableDETFExchangeInTarget
/// @notice Exact-in mint (pair/share/SE token), burn when tokenIn is DETF.
/// @dev Primary mint does NOT realize expansion. VaultShare uses hook depositSingleFlexible.
abstract contract UniswapV4StandardExchangeCurveQuadStableDETFExchangeInTarget is
    UniswapV4StandardExchangeCurveQuadStableDETFExchangeOutTarget
{
    ...
    function mintExactDetfOut(IERC20 tokenIn, uint256, uint256, address, bool, uint256)
        public
        virtual
        returns (uint256)
    {
        revert Repo.InvalidRoute(tokenIn, IERC20(address(this)));
    }

    function burnExactTokenOut(IERC20 tokenOut, uint256, uint256, address, bool, uint256)
        public
        virtual
        returns (uint256)
    {
        revert Repo.InvalidRoute(IERC20(address(this)), tokenOut);
    }
