...
/// @title UniswapV4StandardExchangeCurveQuadStableDETDFPkg
/// @notice Immutable/unowned DETF package: Curve Quad Stable SE Buffer Hook reserve (n=4, ≥1 SE).
/// @dev PRODUCT_ID salt identity is this type name. Hook PRODUCT_ID remains the hook type name.
contract UniswapV4StandardExchangeCurveQuadStableDETDFPkg is IUniswapV4StandardExchangeCurveQuadStableDETDFPkg {
    using BetterEfficientHashLib for bytes;

    bytes32 private constant _DEPLOY_CONFIG_SLOT = keccak256(
        abi.encode(uint256(keccak256("vault.detf.uniswap.v4.se.curve.quad.stable.pkg.deploy-config")) - 1)
    ) & ~bytes32(uint256(0xff));

    uint8 private constant M = 3;
    uint8 private constant N = 4;
    uint256 private constant MAX_AMP = 1_000_000;
    ...
    IUniswapV4StandardExchangeCurveQuadStableBufferHookPackage immutable HOOK_PKG;
    ...
    function packageName() public pure returns (string memory) {
        return type(UniswapV4StandardExchangeCurveQuadStableDETDFPkg).name;
    }
    ...
    function facetInterfaces() public pure returns (bytes4[] memory interfaces_) {
        ...
        interfaces_[7] = type(IUniswapV4StandardExchangeCurveQuadStableDETF).interfaceId;
        interfaces_[8] = bytes4(keccak256("UniswapV4StandardExchangeCurveQuadStableDETF"));
    }
    ...
    function initAccount(bytes memory initArgs) public {
        ...
        if (
            args.standardExchanges.length != M || args.vaultShares.length != M
                || args.rateProviders.length != M || args.creationPairPerDetfWad.length != M
        ) {
            revert ArrayLengthMismatch();
        }
        if (args.openingPairPerDetfWad.length != 0 && args.openingPairPerDetfWad.length != M) {
            revert ArrayLengthMismatch();
        }
        if (args.baseAmp == 0 || args.baseAmp >= MAX_AMP) revert InvalidBaseAmp();

        uint256 seCount_;
        for (uint256 i; i < M; ++i) {
            if (address(args.pairTokens[i]) == address(0)) revert ZeroAddress();
            if (address(args.pairTokens[i]) == address(this)) revert SamePairTokens();
            if (args.creationPairPerDetfWad[i] == 0) revert InvalidCreationRate();
            uint8 dec_ = IERC20Metadata(address(args.pairTokens[i])).decimals();
            if (dec_ < 6 || dec_ > 18) revert InvalidCreationRate();
        ...
        address[] memory contents_ = new address[](M);
        for (uint256 i; i < M; ++i) {
    ...
    function _resolveBinding(DeployConfig storage cfg) private {
        address[] memory tokens_ = new address[](N);
    ...
    function _initRepo(
        ...
        IERC20[] memory shares_ = new IERC20[](M);
        address[] memory rps_ = new address[](M);
        uint256[] memory rates_ = new uint256[](M);
        uint256[] memory openings_ = new uint256[](M);
        uint8[] memory pairBind_ = new uint8[](M);
        for (uint8 i; i < M; ++i) {
        ...
        emit IUniswapV4StandardExchangeCurveQuadStableDETF.ThresholdModeSet(
