...
abstract contract UniswapV4StandardExchangeOrbitalDETFCommon is ReentrancyLockModifiers {
    ...
    function _protocolLp() internal view returns (uint256) {
        return _protocolOriginalLp();
    }

    /// @notice Physical hook LP held by the bond NFT vault (D13).
    function _nftLp() internal view returns (uint256) {
        Repo.Storage storage s = Repo._layoutStruct();
        if (address(s.bondNftVault) == address(0) || s.reserveHook == address(0)) return 0;
        return IERC20(s.reserveHook).balanceOf(address(s.bondNftVault));
    }
    ...
    /// @notice Id 0 extractable LP (4626 on NFT). Unbooked live-mint LP changes NAV.
    function _protocolOriginalLp() internal view returns (uint256) {
        Repo.Storage storage s = Repo._layoutStruct();
        if (address(s.bondNftVault) == address(0)) return 0;
        uint256 orig_ = s.bondNftVault.originalSharesOf(s.bondNftVault.detfNFTId());
        if (orig_ == 0) return 0;
        return s.bondNftVault.convertToAssets(orig_);
    }
    ...
    function _topUpFeeCreatorShares() internal {
        ...
        address oracle_ = address(s.feeOracle);
        if (oracle_ == address(0)) oracle_ = address(StandardVaultRepo._feeOracle());
        if (oracle_ == address(0)) return;
        (, uint256 f_, uint256 c_) = IVaultFeeOracleQuery(oracle_).seigniorageSplitOfVault(address(this));
        DETFBondLifecycleLib._topUpFeeCreatorShares(
            IDetfSelfNftInventoryPolicy(address(s.bondNftVault)), f_, c_
        );
    }

    /// @notice Unboosted matching DETF for a bond join (D24). Empty book uses creation rates.
    function _quoteBondJoinDetf(uint8 fundedPairLeg_, uint256 pairNative_)
        internal
        view
    ...
    function _creditBondPot(MintSplit memory split_) internal {
