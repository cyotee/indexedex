...
|----------|---------------|----------------|
| `DETFProtocolCompoundLib.sol` (new) | `contracts/vaults/detf/core/` | Pure helpers: dust check; optional event topic docs; **no** Balancer calls if family-specific |
...
| File | Action |
|------|--------|
| `contracts/vaults/detf/core/DETFProtocolCompoundLib.sol` | **Create** — dust + any pure helpers |
| `contracts/vaults/detf/core/DETFBondLifecycleLib.sol` | **Optional extend** — helper that documents harvest→caller owns join |
...
| `contracts/vaults/detf/bondNft/*` | **Only if** needed for harvest consistency (e.g. view `pendingRewards(detfNFTId)` already exists — prefer **no** behavior change for users) |
| `test/foundry/spec/vaults/detf/core/DETFProtocolCompoundLib.t.sol` | **Create** pure unit tests |
...
| T0.4 | Existing bond NFT tests still green if bond NFT touched | `forge test --match-path test/foundry/spec/vaults/detf/bondNft/**` |

```bash
forge test --match-path test/foundry/spec/vaults/detf/core/DETFProtocolCompoundLib.t.sol -vv
# if bondNft edited:
