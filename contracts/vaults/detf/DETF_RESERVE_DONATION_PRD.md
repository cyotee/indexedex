...
| **N6** | Token set | Packages **with I/O tables** (unified Uni V4 DETF): resolved `donateRoutes` ⊆ mint ∪ bond, plus implicit DETF and `lpToken` ([`DETF_INSTANCE_IO_ROUTING_PRD.md`](./DETF_INSTANCE_IO_ROUTING_PRD.md) R12). Other families: live-mint / bond capital, SE `tokens()` already settled, DETF, `lpToken`. |
| **N7** | Join shape | I/O-table packages: seating vault is the donate **route row** (then Uni V4 `joinSingleAssetExactIn(share)`). Other families: matching reserve leg as today. **DETF** → self-leg join. `lpToken` donate is a pull only, then still **N4** on the **common** Bond NFT. Uni V4 unified Bond NFT books donate as **R12a** (unassigned LP when `totalOriginalShares > 0`). |
