...
| **Empty-pool / first-bond quote** | Family | Pool is empty: creation rate, weight-peg, or family bootstrap. **D8 does not run here.** D8’s live curve + amountIn bonus applies **after** live, and only to free mint/burn. **Accepted as shipped:** Uni V4 `PkgArgs` `creationPairPerDetfWad` (per pair). Balancer Single SE / MVW: weights (`detfWeight` / `vaultWeights`). Mixed-buffer: amp + first-bond amounts. Composed: existing reserve + first join. Do **not** add a Uni-style launch-rate field to Balancer families. |
