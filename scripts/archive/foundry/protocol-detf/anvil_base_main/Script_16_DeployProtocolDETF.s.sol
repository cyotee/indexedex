...
/// @notice Deploys the single-vault DETF behind the existing Stage 16 entrypoint.
contract Script_16_DeployProtocolDETF is DeploymentBase {
	...
	function _deployWeightedPool8020FactoryIfNeeded() internal {
		...
		vm.label(address(weightedPool8020Factory), "SingleVaultDetfWeightedPool8020Factory");
	}
	...
	function _deployFacets() internal {
		...
		vm.label(address(erc721Facet), "SingleVaultDetf_ERC721Facet");
	}
	...
	function _deployRichToken() internal {
		...
		vm.label(address(pairTokenPkg), "ERC20PermitDFPkg(RICH)");
	...
	function _deployUniswapV4PoolInfra() internal {
		...
		vm.label(address(poolManager), "SingleVaultDetfPoolManager");
		...
		vm.label(address(liquiditySeeder), "SingleVaultDetfLiquiditySeeder");
	}
	...
	function _deployPkgs() internal {
		...
		detfNFTVaultPkg = DetfPkgFactoryService.deployDETFNFTVaultDFPkg(vaultRegistry, nftPkgInit);

		IRebasingClaimTokenDFPkg.PkgInit memory rebasingClaimTokenPkgInit = DetfComponentFactoryService.buildRICHIRPkgInit(
			erc20Facet,
			erc5267Facet,
			erc2612Facet,
		...
		rebasingClaimTokenPkg = DetfPkgFactoryService.deployRebasingClaimTokenDFPkg(create3Factory, rebasingClaimTokenPkgInit);
	...
	function _deployProtocolDetf() internal {
		...
		inventoryDetf = vaultRegistry.deployVault(IStandardVaultPkg(address(inventoryDetfPkg)), abi.encode(args));
		vm.label(inventoryDetf, "SingleVaultDetf(CHIR)");
		...
		vm.label(protocolNftVault, "DETFNFTVault");
		vm.label(rebasingClaimToken, "RICHIR");
		vm.label(reservePool, "CHIR Reserve Pool");
		vm.label(underlyingVault, "WETH/RICH Vault");
	}
	...
	function _exportJson() internal {
		...
		json = vm.serializeAddress("", "inventoryDetfPkg", address(inventoryDetfPkg));
		...
		json = vm.serializeAddress("", "inventoryDetf", inventoryDetf);
	...
	function _logResults() internal view {
		...
		_logAddress("Single Vault DETF (CHIR):", inventoryDetf);
