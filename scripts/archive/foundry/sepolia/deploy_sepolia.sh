...
# Default mode simulates the deployment. Pass --broadcast to send transactions.
...
#
# Optional env:
#   ACCOUNT      Foundry/cast keystore account name used for broadcasting
#   SENDER       Explicit sender address override; defaults to ACCOUNT address
...
# - Sepolia-safe DEX and vault stack: the reduced deployment included in `Script_00_DeploySepoliaDemo.s.sol`.
# - Tokenlist export: emits the Sepolia tokenlists generated by the demo deployment flow.
# - Frontend artifacts: syncs tokenlists, merged platform deployment addresses, and a factories contractlist placeholder.
# - Intentional omissions: skips Stages 16-23, including Protocol DETF and all WETH/TTC pool, vault, and liquidity stages.
#
# =============================================================================
...
Options:
    ...
    --dry-run  Simulate the full Sepolia demo deployment in one forge script session (default)
...
resolve_account_address() {
    ...
    resolved_address="$(cast wallet address --account "$ACCOUNT" 2>/dev/null | tail -n 1 | tr -d '[:space:]')"
    if [[ ! "$resolved_address" =~ ^0x[0-9a-fA-F]{40}$ ]]; then
        log_error "Could not resolve address for ACCOUNT=$ACCOUNT"
        echo ""
        echo "ACCOUNT must be the name of a cast/foundry keystore entry."
        echo "If you want to override the sender address explicitly, set SENDER=0x..."
        echo ""
        exit 1
    fi
...
require_signer() {
    ...
    if [[ -z "${OWNER:-}" ]]; then
        if [[ -n "$SENDER" ]]; then
            ...
            echo ""
            echo "When using ACCOUNT, ensure the keystore alias resolves to an address or set OWNER explicitly."
            echo "Example:"
            echo "  export ACCOUNT=my-sepolia"
            echo "  export OWNER=0xYourOwnerAddress"
            echo ""
            exit 1
        fi
    fi
    ...
    if [[ -n "$SENDER" ]]; then
        ...
        export DEPLOYER="$ACCOUNT_ADDRESS"
    elif [[ -n "$PRIVATE_KEY" ]]; then
