...
/// @dev Balancer pools are deployed in Stage 12, so this only seeds UniV2 + Aerodrome.
...
contract Script_10_DepositBaseLiquidity is DeploymentBase {
    ...
    function _loadPreviousDeployments() internal {
        ...
        aeroAbPool = IAerodromePool(_readAddress("08_pools.json", "aeroAbPool"));
        aeroAcPool = IAerodromePool(_readAddress("08_pools.json", "aeroAcPool"));
        aeroBcPool = IAerodromePool(_readAddress("08_pools.json", "aeroBcPool"));
        ...
        // Load router addresses from previous deployments
        ...
        // Set routers via base class setters
    ...
    function _approveRoutersAndVault() internal {
        ...
        // Balancer Router + Permit2 approvals (Router uses Permit2 for token transfers)
        _approvePermit2(ttA);
        _approvePermit2(ttB);
        _approvePermit2(ttC);
    }
    ...
    function _approvePermit2(address token) internal {
        ...
        // Approve Permit2 to spend token for Router
