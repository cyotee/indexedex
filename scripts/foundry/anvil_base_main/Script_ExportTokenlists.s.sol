...
contract Script_ExportTokenlists is DeploymentBase {
    ...
    function _exportBalancerPools(string memory chainIdStr) internal {
        ...
        {
            (address balUniWethcWithWeth, bool ok1) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balUniWethcWithWeth");
            (address balUniWethcWithC, bool ok2) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balUniWethcWithC");
            (address balAeroWethcWithWeth, bool ok3) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balAeroWethcWithWeth");
            (address balAeroWethcWithC, bool ok4) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balAeroWethcWithC");
            bool hasWethcVaultTokenPools = ok1 && ok2 && ok3 && ok4 && balUniWethcWithWeth != address(0) && balUniWethcWithC != address(0)
                && balAeroWethcWithWeth != address(0) && balAeroWethcWithC != address(0);

            string[] memory entries = new string[](hasWethcVaultTokenPools ? 16 : 12);
            entries[0] = _tokenlistEntry(
                chainIdStr,
...

        {
            ...
            (address reservePool, bool okReservePool) = _readAddressSafe("16_protocol_detf.json", "reservePool");
            bool hasReservePool = okReservePool && reservePool != address(0);

            (address balancerWethcPool, bool okWethcPool) = _readAddressSafe("17_weth_ttc_pools.json", "balancerWethcPool");
            bool hasWethcPool = okWethcPool && balancerWethcPool != address(0);

            bool hasWethcVaultTokenPools;
            {
                (address tmp, bool ok) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balUniWethcWithWeth");
                hasWethcVaultTokenPools = ok && tmp != address(0);
                if (hasWethcVaultTokenPools) {
                    (tmp, ok) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balUniWethcWithC");
                    hasWethcVaultTokenPools = hasWethcVaultTokenPools && ok && tmp != address(0);
                    (tmp, ok) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balAeroWethcWithWeth");
                    hasWethcVaultTokenPools = hasWethcVaultTokenPools && ok && tmp != address(0);
                    (tmp, ok) = _readAddressSafe("20_weth_ttc_balancer_vault_token_pools.json", "balAeroWethcWithC");
                    hasWethcVaultTokenPools = hasWethcVaultTokenPools && ok && tmp != address(0);
                }
            }

            bool hasVaultVaultPools;
            {
                (address tmp, bool ok) = _readAddressSafe("22_weth_ttc_vault_vault_pool.json", "balancerAbVaultVaultPool");
                hasVaultVaultPools = ok && tmp != address(0);
                if (hasVaultVaultPools) {
                    (tmp, ok) = _readAddressSafe("22_weth_ttc_vault_vault_pool.json", "balancerAcVaultVaultPool");
                    hasVaultVaultPools = hasVaultVaultPools && ok && tmp != address(0);
                    (tmp, ok) = _readAddressSafe("22_weth_ttc_vault_vault_pool.json", "balancerBcVaultVaultPool");
                    hasVaultVaultPools = hasVaultVaultPools && ok && tmp != address(0);
                    (tmp, ok) = _readAddressSafe("22_weth_ttc_vault_vault_pool.json", "balancerWethcVaultVaultPool");
                    hasVaultVaultPools = hasVaultVaultPools && ok && tmp != address(0);
                }
            }

            uint256 len = (hasReservePool ? 16 : 15)
                + (hasWethcPool ? 1 : 0)
                + (hasWethcVaultTokenPools ? 4 : 0)
                + (hasVaultVaultPools ? 4 : 0);
            string[] memory entries = new string[](len);

            uint256 i = 0;
            entries[i++] = _tokenlistEntry(
                chainIdStr,
                _readAddress("12_balancer_const_prod_vault_token_pools.json", "balancerAbPool"),
                "ab BalancerV3 ConstProd Pool",
                "abBalancerV3ConstProdPool"
            ...
            if (hasWethcVaultTokenPools) {
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("20_weth_ttc_balancer_vault_token_pools.json", "balUniWethcWithWeth"),
                    "UniV2 WETH/TTC Vault + WETH BalancerV3 Pool",
                    "uniWethcVault_weth_BalancerPool"
                );
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("20_weth_ttc_balancer_vault_token_pools.json", "balUniWethcWithC"),
                    "UniV2 WETH/TTC Vault + TTC BalancerV3 Pool",
                    "uniWethcVault_ttc_BalancerPool"
                );
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("20_weth_ttc_balancer_vault_token_pools.json", "balAeroWethcWithWeth"),
                    "Aerodrome WETH/TTC Vault + WETH BalancerV3 Pool",
                    "aeroWethcVault_weth_BalancerPool"
                );
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("20_weth_ttc_balancer_vault_token_pools.json", "balAeroWethcWithC"),
                    "Aerodrome WETH/TTC Vault + TTC BalancerV3 Pool",
                    "aeroWethcVault_ttc_BalancerPool"
                );
            }
            ...
            if (hasVaultVaultPools) {
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("22_weth_ttc_vault_vault_pool.json", "balancerAbVaultVaultPool"),
                    "AB Vault + AB Vault BalancerV3 Pool",
                    "abVault_abVault_BalancerPool"
                );
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("22_weth_ttc_vault_vault_pool.json", "balancerAcVaultVaultPool"),
                    "AC Vault + AC Vault BalancerV3 Pool",
                    "acVault_acVault_BalancerPool"
                );
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("22_weth_ttc_vault_vault_pool.json", "balancerBcVaultVaultPool"),
                    "BC Vault + BC Vault BalancerV3 Pool",
                    "bcVault_bcVault_BalancerPool"
                );
                entries[i++] = _tokenlistEntry(
                    chainIdStr,
                    _readAddress("22_weth_ttc_vault_vault_pool.json", "balancerWethcVaultVaultPool"),
                    "WETH/TTC Vault + WETH/TTC Vault BalancerV3 Pool",
                    "wethcVault_wethcVault_BalancerPool"
                );
            }
