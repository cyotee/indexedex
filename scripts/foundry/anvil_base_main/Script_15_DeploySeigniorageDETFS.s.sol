...
/// @dev Run: forge script scripts/foundry/anvil_base_main/Script_15_DeploySeigniorageDETFS.s.sol --rpc-url http://127.0.0.1:8545 --broadcast --unlocked --sender <DEV_ADDRESS>
contract Script_15_DeploySeigniorageDETFS is DeploymentBase {
    ...
    function _deployPkgs() internal {
        ...
        // Seigniorage DETF package (registered via vault registry)
        {
            ...
            params.balancerV3Router = IRouter(address(balancerV3Router));
    ...
    function _deployDetfs() internal {
        IStandardExchangeProxy abVault = IStandardExchangeProxy(_readAddress("07_strategy_vaults.json", "abVault"));
        IStandardExchangeProxy acVault = IStandardExchangeProxy(_readAddress("07_strategy_vaults.json", "acVault"));
        IStandardExchangeProxy bcVault = IStandardExchangeProxy(_readAddress("07_strategy_vaults.json", "bcVault"));

        IStandardExchangeProxy aeroAbVault = IStandardExchangeProxy(_readAddress("08_aerodrome_strategy_vaults.json", "aeroAbVault"));
        IStandardExchangeProxy aeroAcVault = IStandardExchangeProxy(_readAddress("08_aerodrome_strategy_vaults.json", "aeroAcVault"));
        IStandardExchangeProxy aeroBcVault = IStandardExchangeProxy(_readAddress("08_aerodrome_strategy_vaults.json", "aeroBcVault"));
        ...
        detf_aeroAbVault = _deployDetfForReserveVault(aeroAbVault);
        detf_aeroAcVault = _deployDetfForReserveVault(aeroAcVault);
        detf_aeroBcVault = _deployDetfForReserveVault(aeroBcVault);
    }
