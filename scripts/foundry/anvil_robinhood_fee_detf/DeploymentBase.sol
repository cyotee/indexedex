...
/// @notice Shared base for Anvil Robinhood fee-DETF launch staged deploys (chain id 4663).
/// @dev Separate family from anvil_robinhood_main - do not merge.
abstract contract DeploymentBase is Script {
    string internal constant OUT_DIR = "deployments/anvil_robinhood_fee_detf";
    uint256 internal constant EXPECTED_CHAIN_ID = 4663;

    /// @dev Anvil account(0) - deployer / owner / SENDER / bootstrap actor.
    ...
    function _networkProfile() internal view returns (string memory profile) {
        ...
        return "anvil_robinhood_fee_detf";
    }
    ...
    function _requireRobinhoodChain() internal view {
        require(block.chainid == EXPECTED_CHAIN_ID, "anvil_robinhood_fee_detf: chainId must be 4663");
        require(block.chainid == ROBINHOOD_MAIN.CHAIN_ID, "anvil_robinhood_fee_detf: ROBINHOOD_MAIN.CHAIN_ID mismatch");
    }
