...
/// @notice FeeCollector + IndexedexManager via CREATE3 / diamond package factory.
contract Script_02_DeployIndexedexCore is DeploymentBase {
    ...
    function _loadExistingCore() internal returns (bool) {
        ...
        if (!hasFeeCollector || !hasIndexedexManager) return false;
        if (feeCollectorAddr.code.length == 0 || indexedexManagerAddr.code.length == 0) return false;
