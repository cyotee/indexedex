#!/usr/bin/env bash
# =============================================================================
# IndexedEx Full Deployment Script for Anvil Ethereum Sepolia Fork
# =============================================================================
#
# PREREQUISITES:
# 1. Start Anvil with Ethereum Sepolia fork in a separate terminal:
#
#    anvil --fork-url sepolia_alchemy --chain-id 31337 --fork-block-number <LATEST>
#
#    Or with explicit Alchemy URL:
#
#    anvil --fork-url https://eth-sepolia.g.alchemy.com/v2/YOUR_API_KEY --chain-id 31337 --fork-block-number <LATEST>
#
...
#
# OPTIONAL ENV:
#    FORGE_SLOW        Defaults to 1. When enabled, passes `--slow` to `forge script`
#                      so Foundry waits for each receipt before sending the next tx.
#                      This avoids Anvil receipt-polling hangs on CREATE2-heavy stages.
#
# NOTES:
...
ANVIL_FORK_URL="${ANVIL_FORK_URL:-sepolia_alchemy}"
...
usage() {
        cat <<EOF
Usage:
    scripts/foundry/anvil_sepolia/deploy_sepolia.sh [options]
...
Optional env:
    RPC_URL, ANVIL_FORK_URL, ANVIL_FORK_BLOCK_NUMBER, ANVIL_CHAIN_ID, ANVIL_HOST, ANVIL_PORT, FORGE_SLOW

NOTES:
    - This deployment forks from Ethereum Sepolia
    - We deploy our own Uniswap V2 and Aerodrome instances using Balancer V3's WETH
    - Balancer V3 WETH: 0x7b79995e5f793A07Bc00c21412e50Ecae098E7f9
    - Our Uniswap V2 will use this WETH instead of the default 0xfFf9976782d46CC05630D1f6eBAb18b2324d6B14
    - Aerodrome will be deployed fresh since it doesn't exist on Sepolia
    - FORGE_SLOW=1 serializes broadcasts and prevents Anvil from hanging on later receipt polling
EOF
}
...
sync_frontend_artifacts() {
    ...
    # Preferred: segmented tokenlists generated by Stage 24.
    ...
    shopt -s nullglob
    tokenlists=("$deployments_dir"/anvil_sepolia-*.tokenlist.json)
    shopt -u nullglob
    ...
    if [[ "$copied_any" -ne 1 ]]; then
        ...
        log_info "Expected Stage 24 outputs at: $deployments_dir/anvil_sepolia-*.tokenlist.json"
    fi
...
# Balancer V3 addresses on Sepolia
base["balancerV3Vault"] = "0xbA1333333333a1BA1108E8412f11850A5C319bA9"
base["balancerV3Router"] = "0x5e315f96389C1aaF9324D97d3512ae1e0Bf3C21a"

# Our deployed WETH (using Balancer V3's WETH on Sepolia)
base["weth9"] = "0x7b79995e5f793A07Bc00c21412e50Ecae098E7f9"

# Our deployed Uniswap V2 (using Balancer's WETH)
# These will be populated by Stage 5

# Our deployed Aerodrome (deployed fresh since not on Sepolia)
...
verify_frontend_router_keys() {
    ...
    local balancer_vault
    local balancer_router
    local balancer_vault_lc
    local balancer_router_lc

    balancer_vault="$(json_get_address "$frontend_file" "balancerV3Vault" || true)"
    balancer_router="$(json_get_address "$frontend_file" "balancerV3Router" || true)"
    balancer_vault_lc="$(printf '%s' "$balancer_vault" | tr '[:upper:]' '[:lower:]')"
    balancer_router_lc="$(printf '%s' "$balancer_router" | tr '[:upper:]' '[:lower:]')"

    if [[ -z "$balancer_vault" ]]; then
        log_error "Missing key 'balancerV3Vault' in $frontend_file"
        exit 1
    fi
    ...
    if ! has_code "$balancer_vault"; then
        log_error "No bytecode at balancerV3Vault: $balancer_vault"
        exit 1
    fi
    ...
    log_success "Verified router keys: balancerV3Vault=$balancer_vault, balancerV3Router=$balancer_router"
}
...
check_anvil() {
    ...
    if ! curl -s -X POST -H "Content-Type: application/json" \
        ...
        echo "  anvil --fork-url sepolia_alchemy --chain-id 31337"
        echo ""
        exit 1
    fi
...
clear_dev_account_bytecode() {
    # When forking L2s like Ethereum Sepolia, some default Anvil dev accounts may have
...
start_anvil_nohup() {
    ...
    # Build Anvil command
    local anvil_cmd="anvil --fork-url $ANVIL_FORK_URL --chain-id $ANVIL_CHAIN_ID"
    
    if [[ -n "$ANVIL_FORK_BLOCK_NUMBER" ]]; then
        anvil_cmd="$anvil_cmd --fork-block-number $ANVIL_FORK_BLOCK_NUMBER"
    fi
    
    anvil_cmd="$anvil_cmd --compute-units-per-second $ANVIL_COMPUTE_UNITS_PER_SECOND --fork-retry-backoff $ANVIL_FORK_RETRY_BACKOFF --host $ANVIL_HOST --port $ANVIL_PORT"
...
main() {
    log_header "IndexedEx Deployment to Anvil Ethereum Sepolia Fork"
    ...
    # On L2 forks (e.g. Ethereum Sepolia), default Anvil dev accounts may have EIP-7702
    ...
    # ================================================================================
    # STAGE 0: Preflight scripts
    # ================================================================================
    # (Already done above - sweep and bytecode clear)

    # ================================================================================
    # STAGE 1: Deploy Factories
    # ================================================================================
    ...
    # ================================================================================
    # STAGE 4: Deploy DEX Packages - Balancer V3 (use existing)
    # ================================================================================
    run_stage "Script_04_DeployDEXPackages_BalancerV3.s.sol" \
        "Stage 4: Deploy Balancer V3 Integration (use existing Sepolia)" \
        "$DEPLOYMENTS_DIR/04_balancer_v3.json"

    # ================================================================================
    # STAGE 5: Deploy OUR OWN Uniswap V2 with Balancer's WETH
    # ================================================================================
    run_stage "Script_05_DeployUniswapV2.s.sol" \
        "Stage 5: Deploy OUR OWN Uniswap V2 (using Balancer V3 WETH)" \
        "$DEPLOYMENTS_DIR/05_uniswap_v2.json"

    # ================================================================================
    # STAGE 6: Deploy OUR OWN Aerodrome (not available on Sepolia)
    # ================================================================================
    run_stage "Script_06_DeployAerodrome.s.sol" \
        "Stage 6: Deploy OUR OWN Aerodrome (deployed fresh for Sepolia)" \
        "$DEPLOYMENTS_DIR/06_aerodrome.json"

    # ================================================================================
    # STAGE 7: Deploy Test Tokens
    # ================================================================================
    run_stage "Script_07_DeployTestTokens.s.sol" \
        "Stage 7: Deploy Test Tokens (TTA, TTB, TTC)" \
        "$DEPLOYMENTS_DIR/07_test_tokens.json"

    # ================================================================================
    # STAGE 8: Deploy Pools
    # ================================================================================
    run_stage "Script_08_DeployPools.s.sol" \
        "Stage 8: Deploy Pools (UniV2, Aerodrome, Balancer)" \
        "$DEPLOYMENTS_DIR/08_pools.json"

    # ================================================================================
    # STAGE 9: Deploy Strategy Vaults
    # ================================================================================
    run_stage "Script_09_DeployStrategyVaults.s.sol" \
        "Stage 9: Deploy Strategy Vaults" \
        "$DEPLOYMENTS_DIR/09_strategy_vaults.json"
    ...
    # ================================================================================
    # STAGE 12: Deploy Balancer ConstProd Vault-Token Pools
    # ================================================================================
    ...
    # ================================================================================
    # STAGE 14: Deploy ERC4626 Permit Vaults
    # ================================================================================
    ...
    # ================================================================================
    # STAGE 18: Deploy WETH/TTC Strategy Vaults
    # ================================================================================
    ...
    # ================================================================================
    # STAGE 24: Export Tokenlists + Sync Frontend
    # ================================================================================
