#!/usr/bin/env bash
# =============================================================================
# Robinhood testnet (46630) Anvil launch: Phase 00 then 01–09.
# Fork 46630, signer Anvil Dev 0, --unlocked. Simulate each Stage then broadcast.
# Never --skip-simulation. --dry-run stops after sim. FORCE=1 / --force re-runs.
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
RH_FOUNDRY_DIR="$REPO_ROOT/scripts/foundry/anvil_robinhood_testnet"
# shellcheck disable=SC1091
source "$REPO_ROOT/scripts/shell/lib/rh_46630_stages.sh"
...
Anvil path for Robinhood Chain Testnet (46630). Always forks 46630
(robinhood_testnet_alchemy, fallback public). Signer is Anvil Dev 0 with
--unlocked. --chain-id 46630 --disable-code-size-limit.

Public 46630 (DEPLOYER_ADDRESS, no Phase 00): scripts/shell/robinhood_testnet.sh

Commands:
  all           Phase 00 Stage 01, then every Stage of Phases 01–09 in catalog order.
                No Morpho or Uni V3 SE instances (create those in the UI).
  export        Phase 09 Stage 01 (frontend JSON)
  detach-fork   Anvil-only: dump overlay, restart without a fork, reload state.
...
Options:
  ...
  --force           Re-run Stages (purge stage JSON first when restarting). FORCE=1.
...
purge_stage_artifacts() {
  ...
  mkdir -p "$dir"
  local files=("$dir"/phase*.json "$dir"/[0-9]*.json)
  if [[ -e "${files[0]:-}" ]]; then
...
print(json.loads(p.read_text()).get("create3Factory",""))' "$REPO_ROOT/$DEPLOYMENTS_DIR/phase02_stage01_create3_factory.json" 2>/dev/null || true)"
  if [[ -z "$factory" ]]; then
    log_error "Cannot verify overlay: $DEPLOYMENTS_DIR/phase02_stage01_create3_factory.json missing create3Factory"
    return 1
  fi
  local size
...
# Anvil shell always signs as Dev 0 + --unlocked.
require_deployer() {
  SENDER="$ANVIL_DEV0"
  DEV_ADDRESS="$ANVIL_DEV0"
  DEPLOYER_ADDRESS="$ANVIL_DEV0"
  OWNER="${OWNER:-$ANVIL_DEV0}"
  UI_WALLET="${UI_WALLET:-$ANVIL_DEV1}"
  log_info "Using Anvil Dev 0 as deployer"
...
forge_script_base() {
  ...
  if is_localhost_rpc && [[ "$LIVE_BROADCAST" -eq 0 ]]; then
    cmd+=(--unlocked --disable-code-size-limit --non-interactive)
  fi
  printf '%s\n' "${cmd[@]}"
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --from-phase)
      ...
      ;;
    --from-phase=*)
      FROM_PHASE="$(printf '%02d' "$((10#${1#--from-phase=}))")"
      if [[ -z "$FROM_STAGE" ]]; then
        FROM_STAGE="01"
      fi
      shift
    ...
    --from-stage)
      ...
      ;;
    --from-stage=*)
      FROM_STAGE="$(printf '%02d' "$((10#${1#--from-stage=}))")"
      shift
      ;;
    --fork-alias)
      if [[ $# -lt 2 || "$2" == -* ]]; then
...
if [[ "$LIVE_BROADCAST" -eq 1 ]]; then
  log_error "This Anvil shell does not take --live. Use scripts/shell/robinhood_testnet.sh"
  exit 1
fi
...
if [[ "$COMMAND" != "export" ]]; then
  ...
  log_info "Skipping wrapper chain-id check for $COMMAND (Phase 09 is JSON export)"
fi

log_header "Robinhood testnet (46630) Anvil deploy: $COMMAND"
log_info "SENDER=$SENDER UI_WALLET=$UI_WALLET OUT_DIR=$OUT_DIR_OVERRIDE FORCE=$FORCE"

case "$COMMAND" in
  all)
    rh_run_catalog 1 "$FROM_PHASE" "$FROM_STAGE"
    ;;
  export)
    run_stage "Phase 09 Stage 01" "$(rh_stage_script 9 1)"
    ;;
  detach-fork)
    if ! is_localhost_rpc; then
