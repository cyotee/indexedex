#!/usr/bin/env bash
# =============================================================================
# Public Robinhood Chain Testnet (46630) launch.
# No Phase 00. --sender $DEPLOYER_ADDRESS (cast wallet). Currently Phases 01–09.
# Public subset is not locked; change only after Anvil UI testing + PRD patch.
...
usage() {
  cat <<EOF
Usage:
  scripts/shell/robinhood_testnet.sh [command] [options]

Public 46630 path. Does not run Phase 00. Requires DEPLOYER_ADDRESS.
Currently runs Phases 01–09. The public subset is not locked; change only
after Anvil UI testing and a PRD patch.

Commands:
  all     Phases 01–09 in catalog order
  export  Phase 09 Stage 01

Options:
  --dry-run         Simulate each Stage, do not broadcast
  --rpc-url URL     Broadcast RPC (default Foundry alias robinhood_testnet)
...
if [[ -z "${DEPLOYER_ADDRESS:-}" ]]; then
  log_error "robinhood_testnet.sh requires DEPLOYER_ADDRESS"
...
log_header "Public Robinhood testnet (46630): $COMMAND"
log_info "SENDER=$DEPLOYER_ADDRESS OUT_DIR=$OUT_DIR_OVERRIDE (no Phase 00; public subset is not locked)"
