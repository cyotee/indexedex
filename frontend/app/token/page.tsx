...
import { loadFeaturedEarnProducts } from '../lib/earn/loadEarnProducts'
...
export default function TokenPage() {
  ...
  return (
    <div className="max-w-3xl">
      <PageHeader
        title="Token"
        subtitle="Buy (when configured), then put capital to work in Earn."
      />
      ...
      <Card className="mb-6">
        ...
        <p className="mt-2 text-sm text-[var(--text-muted,#9aa3b2)]">
          After acquiring the token (or any base asset), deposit into a featured strategy.
        </p>
        {featured[0] ? (
          <Link href={`/earn/${featured[0].address}`} className="mt-4 inline-block">
            <Button variant="secondary" size="sm">
              Deposit into {featured[0].symbol}
            </Button>
          </Link>
        ) : (
      ...
      <Card>
        ...
        <ul className="mt-2 list-disc pl-5 text-sm text-[var(--text-muted,#9aa3b2)] space-y-1">
          <li>Access to protocol products through the app (Earn, Swap, DETF workflows).</li>
