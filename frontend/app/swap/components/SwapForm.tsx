...
interface SwapFormProps {
  ...
  selectedVaultIn: string
  setSelectedVaultIn: (vault: string) => void
  selectedVaultOut: string
  setSelectedVaultOut: (vault: string) => void
  ...
  accurateQuote: bigint | null
  accurateQuoteLoading: boolean
  accurateQuoteError: string
  
  // Approval UI state
  needsTokenApproval: boolean
  needsPermit2Approval: boolean
  permit2AllowanceAmount: bigint | undefined
  tokenAllowanceAmount: bigint | undefined
  permit2SpendingLimit: string
  setPermit2SpendingLimit: (value: string) => void
  routerSpendingLimit: string
  setRouterSpendingLimit: (value: string) => void
  handleSetPermit2SpendingLimit: (amount: bigint) => void
  handleSetRouterSpendingLimit: (amount: bigint) => void
  ...
  handlePreview: () => void
  handleApproval: () => void
  handleIssuePermit2Approval: () => void
  handleIssueRouterApproval: () => void
  handleGetAccurateQuote: () => void
  handleSwap: () => void
...
export function SwapForm({
  ...
  setPermit2SpendingLimit,
  routerSpendingLimit,
  setRouterSpendingLimit,
  handleSetPermit2SpendingLimit,
  handleSetRouterSpendingLimit,
  poolOptions,
  tokenOptions,
  ...
  return (
    <div className="container mx-auto px-4 max-w-2xl">
          ...
          <div className="mt-2 p-4 bg-slate-700/30 rounded-lg border border-slate-600">
            ...
            >
              <input
                ...
                onChange={() => setApprovalMode('explicit')}
                className="mt-1"
              />
              <div>
            ...
            >
              <input
                ...
                onChange={() => setApprovalMode('signed')}
                className="mt-1"
              />
              <div>
      ...
      <div className="mb-4 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
        {showInVaultWarning && (
          <div className="mb-3 p-2 bg-amber-600/20 border border-amber-500/50 rounded-lg">
        ...
        {useTokenInVault && filteredVaultOptions.length > 0 && (
          ...
          >
            ...
            {filteredVaultOptions.map((vault) => (
        ...
        <div className="flex gap-2">
          ...
          />
          {amountInDisplay && (
            <span className="text-xs text-gray-400 self-center">{amountInDisplay}</span>
          )}
        </div>
      </div>
      ...
      <div className="mb-4 p-4 bg-slate-700/50 rounded-lg border border-slate-600 -mt-3">
        ...
        <div className="flex gap-2">
          ...
          />
          {amountOutDisplay && (
            <span className="text-xs text-gray-400 self-center">{amountOutDisplay}</span>
          )}
        </div>
      </div>
      ...
      {routePattern && (
        <div className="mb-4 p-3 bg-slate-700/30 rounded-lg border border-slate-600">
          ...
          {lastEditedField === 'in' && minOut && (
            <div className="text-xs text-gray-400 mt-1">
              Min receive: {minOut.toString()} wei
            </div>
          )}
          {lastEditedField === 'out' && maxIn && (
            <div className="text-xs text-gray-400 mt-1">
      ...
      {approvalMode === 'explicit' && ready && !useEthIn && (
        <div className="mb-4 p-3 bg-slate-700/30 rounded-lg border border-slate-600">
              ...
              <div>
                ...
                <div className="mt-2">
                  <div className="flex gap-2">
                    ...
                    />
                    <button
                      onClick={() => handleSetPermit2SpendingLimit(BigInt(2 ** 160 - 1))}
                      className="px-3 py-2 bg-slate-500 text-white rounded-lg text-sm hover:bg-slate-400"
                    >
                      Max
                    </button>
                  </div>
                  {permit2AllowanceAmount !== undefined && (
                    <div className="text-xs text-gray-400 mt-1">
              ...
              <div>
                ...
                <div className="mt-2">
                  <div className="flex gap-2">
                    ...
                    />
                    <button
                      onClick={() => handleSetRouterSpendingLimit(BigInt(2 ** 160 - 1))}
                      className="px-3 py-2 bg-slate-500 text-white rounded-lg text-sm hover:bg-slate-400"
                    >
                      Max
                    </button>
                  </div>
                  {tokenAllowanceAmount !== undefined && (
                    <div className="text-xs text-gray-400 mt-1">
      ...
      <div className="space-y-3">
        ...
        {approvalMode === 'signed' && (
          <div>
            <button
              ...
              disabled={!ready || accurateQuoteLoading || useEthIn}
              className="w-full px-4 py-3 bg-purple-600 text-white rounded-lg font-medium hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {approvalMode === 'explicit' && !needsTokenApproval && !needsPermit2Approval && (
          <button
        ...
        {approvalError && (
          <div className="text-red-400 text-sm">{approvalError}</div>
        )}
      ...
      {accurateQuote && !accurateQuoteLoading && (
        <div className="mt-4 p-3 bg-green-600/20 rounded-lg border border-green-600/50">
      ...
      {accurateQuoteError && (
        <div className="mt-4 p-3 bg-red-600/20 rounded-lg border border-red-600/50">
