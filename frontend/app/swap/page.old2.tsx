...
//   const getTokenName = useCallback((address: string | undefined) => {
//     if (!address) return 'None'
    
//     // Check if it's a known token address
//     if (address === weth9Addr) return 'WETH9'
//     if (address === ttaAddr) return 'TestTokenA'
//     if (address === ttbAddr) return 'TestTokenB'
//     if (address === ttcAddr) return 'TestTokenC'
    
//     // Check if it's a strategy vault
//     const vaultOption = vaultOptions.find(opt => opt.value === address)
//     if (vaultOption) return vaultOption.label
    
//     // Check if it's a pool
//     const poolOption = poolOptions.find(opt => opt.value === address)
//     if (poolOption) return poolOption.label
    
//     // Check if it's an ERC4626 wrapper
//     const erc4626Options = Object.entries(sepoliaAddresses)
//       .filter(([key]) => /4626$/.test(key))
//       .map(([key, value]) => ({ label: key, value: value as `0x${string}` }))
//     const erc4626Option = erc4626Options.find(opt => opt.value === address)
//     if (erc4626Option) return erc4626Option.label
    
//     // If no match found, return shortened address
//     return `${address.slice(0, 6)}...${address.slice(-4)}`
//   }, [weth9Addr, ttaAddr, ttbAddr, ttcAddr, vaultOptions, poolOptions, sepoliaAddresses])
...
//                 <div>Pool: {getTokenName(finalPool)} ({finalPool})</div>
//                 <div>TokenIn: {getTokenName(tokenInAddress)} ({tokenInAddress})</div>
//                 <div>TokenInVault: {getTokenName(tokenInVaultAddress)} ({tokenInVaultAddress})</div>
//                 <div>TokenOut: {getTokenName(tokenOutAddress)} ({tokenOutAddress})</div>
//                 <div>TokenOutVault: {getTokenName(tokenOutVaultAddress)} ({tokenOutVaultAddress})</div>
...
//               {/* Contract Arguments Debug */}
//               <div className="mt-2 pt-2 border-t border-slate-600">
//                 <div className="text-xs text-gray-400 font-medium mb-1">Contract Arguments:</div>
//                 <div className="text-xs text-gray-400">
//                   <div>Pool: {getTokenName(finalPool)} ({finalPool})</div>
//                   <div>TokenIn: {getTokenName(tokenInAddress)} ({tokenInAddress})</div>
//                   <div>TokenInVault: {getTokenName(tokenInVaultAddress)} ({tokenInVaultAddress})</div>
//                   <div>TokenOut: {getTokenName(tokenOutAddress)} ({tokenOutAddress})</div>
//                   <div>TokenOutVault: {getTokenName(tokenOutVaultAddress)} ({tokenOutVaultAddress})</div>
//                   <div>AmountIn: {exactAmountIn ? `${exactAmountIn.toString()} wei` : 'Not set'}</div>
//                   <div>MinAmountOut: {minOut ? `${minOut.toString()} wei` : 'Not set'}</div>
//                   <div>Deadline: {deadline ? new Date(Number(deadline) * 1000).toLocaleString() : 'Not set'}</div>
//                   <div>WethIsEth: {(useEthIn || useEthOut) ? 'true' : 'false'}</div>
//                   <div>UserData: 0x (empty)</div>
//                   <div>Value: {useEthIn ? `${exactAmountIn ? exactAmountIn.toString() : '0'} wei` : '0 wei'}</div>
//                 </div>
//               </div>
              
//               {/* Route Pattern Analysis */}
//               <div className="mt-2 pt-2 border-t border-slate-600">
//                 <div className="text-xs text-gray-400 font-medium mb-1">Route Pattern Analysis:</div>
//                 <div className="text-xs text-gray-400">
//                   <div>Vault Pass-Through: {isVaultPassThrough ? '✅ Yes' : '❌ No'}</div>
//                   <div>Vault Deposit: {isVaultDeposit ? '✅ Yes' : '❌ No'}</div>
//                   <div>Vault Withdrawal: {isVaultWithdrawal ? '✅ Yes' : '❌ No'}</div>
//                   <div>Direct Pool Swap: {(!specifyVaultIn && !specifyVaultOut) ? '✅ Yes' : '❌ No'}</div>
//                   <div>Route Valid: {routeValid ? '✅ Yes' : '❌ No'}</div>
//                   <div>ETH Input: {useEthIn ? '✅ Yes' : '❌ No'}</div>
//                   <div>ETH Output: {useEthOut ? '✅ Yes' : '❌ No'}</div>
//                 </div>
//               </div>
              
//               {/* Function Signature */}
//               <div className="mt-2 pt-2 border-t border-slate-600">
//                 <div className="text-xs text-gray-400 font-medium mb-1">Function Call:</div>
//                 <div className="text-xs text-gray-400 font-mono">
//                   <div>swapSingleTokenExactIn(</div>
//                   <div className="ml-4">pool: {getTokenName(finalPool)}</div>
//                   <div className="ml-4">tokenIn: {getTokenName(tokenInAddress)}</div>
//                   <div className="ml-4">tokenInVault: {getTokenName(tokenInVaultAddress)}</div>
//                   <div className="ml-4">tokenOut: {getTokenName(tokenOutAddress)}</div>
//                   <div className="ml-4">tokenOutVault: {getTokenName(tokenOutVaultAddress)}</div>
//                   <div className="ml-4">exactAmountIn: {exactAmountIn ? exactAmountIn.toString() : 'undefined'}</div>
//                   <div className="ml-4">minAmountOut: {minOut ? minOut.toString() : 'undefined'}</div>
//                   <div className="ml-4">deadline: {deadline ? deadline.toString() : 'undefined'}</div>
//                   <div className="ml-4">wethIsEth: {(useEthIn || useEthOut) ? 'true' : 'false'}</div>
//                   <div className="ml-4">userData: 0x</div>
//                   <div>)</div>
//                   <div className="ml-4">value: {useEthIn ? (exactAmountIn ? exactAmountIn.toString() : '0') : '0'} wei</div>
//                 </div>
//               </div>
              
//               {/* Debug Info */}
//               <div className="mt-2 pt-2 border-t border-slate-600">
//                 <div className="text-xs text-gray-400 font-medium mb-1">Additional Debug Info:</div>
//                 <div className="text-xs text-gray-400">
//                   <div>📝 Check browser console for detailed swap execution logs</div>
//                   <div>🔍 Console shows exact arguments sent to contract</div>
//                   <div>📊 Includes route pattern analysis and ETH wrapping details</div>
//                 </div>
//               </div>
//             </div>
//           </div>
