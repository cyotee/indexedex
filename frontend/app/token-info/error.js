...
function TokenInfoError({ error, reset, }) {
    (0, react_1.useEffect)(() => {
        (0, debug_1.debugError)('[token-info] route error:', error);
    }, [error]);
    return (<div className="container mx-auto px-4 max-w-3xl">
      ...
      <div className="mt-4 flex gap-2 justify-center">
        <button onClick={() => reset()} className="px-3 py-1 text-xs rounded-md border border-gray-600 bg-gray-700 text-gray-200 hover:bg-gray-600">
          Retry
        </button>
        <button onClick={() => window.location.reload()} className="px-3 py-1 text-xs rounded-md border border-gray-600 bg-gray-700 text-gray-200 hover:bg-gray-600">
          Reload
        </button>
      </div>
    </div>);
}
exports.default = TokenInfoError;
