...
export function SeVaultSlot({
  ...
  const [v4Entry, setV4Entry] = useState<'tokens' | 'poolkey' | 'poolid'>(
    seHost === 'uniswap-v4' && !hostPerSlot ? 'poolid' : 'tokens',
  )
        ...
        : tokenB && !checksumAddress(tokenB)
          ...
          : hooks.trim() && !checksumAddress(hooks)
            ...
            : tokenA && tokenB && same(tokenA, tokenB)
              ...
              : v4Key && poolLookupDone && !poolExists
    ...
    try {
      ...
      if ('error' in looked) {
        if (platform.stateView) {
          const live = await readV4PoolInitialized(readClient, platform.stateView, parsed)
          if (live) {
            setV4PoolIdError(
