...
/** True when this process is pointing at a local Anvil RPC (wallet RPC still wins when connected). */
export function isLocalRobinhoodTestnet(): boolean {
  return (
    deploymentEnv === 'anvil_robinhood_testnet'
    || deploymentEnv === 'anvil_robinhood_main'
    || Boolean(localRpcEnv && localRpcEnv.trim())
  )
}
