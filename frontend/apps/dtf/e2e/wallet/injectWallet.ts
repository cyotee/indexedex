/**
 * Injected EIP-1193 provider for Playwright (DTF).
 *
 * Signs with Anvil account #0 by default; RPC + chain id are env-driven
 * (default Robinhood Anvil: chain 4663 @ http://127.0.0.1:8545).
 */
...
/** Anvil account #1 — UI wallet in Robinhood deploy scripts */
...
/** Default: Robinhood Anvil fork (fee_detf / main lab). Override with E2E_CHAIN_ID. */
export const DEFAULT_E2E_CHAIN_ID = Number(process.env.E2E_CHAIN_ID ?? 4663)
...
export async function installInjectedWallet(
  ...
  const chain = {
    ...sepolia,
    id: chainId,
    name: chainId === 4663 ? 'Robinhood (Anvil)' : sepolia.name,
  ...
  await page.exposeFunction(bridgeName, async (payload: RpcRequest) => {
    ...
    switch (method) {
      ...
      case 'personal_sign': {
        ...
        void from
        const msg = message.startsWith('0x') ? ({ raw: message as Hex } as const) : message
