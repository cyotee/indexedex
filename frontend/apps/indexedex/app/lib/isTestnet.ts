...
function isAnvilLabEnvironment(): boolean {
  const env = process.env.NEXT_PUBLIC_DEFAULT_DEPLOYMENT_ENVIRONMENT ?? ''
  return (
    env.startsWith('anvil_')
    || env === 'local_testing'
    || env === 'supersim_sepolia'
  )
}

export function isTestnetChainId(chainId: number | undefined): boolean {
  if (isAnvilLabEnvironment()) return true
