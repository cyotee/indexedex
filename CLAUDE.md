...
6. **Weird-token law (universal — do not re-ask):** FoT forbidden. Rebasing underlyings forbidden (`rebasingClaimToken` is a protocol product, not an underlying). Non-18 decimals allowed (scale to 18). Pause / blacklist accepted. No `PkgArgs` allowlist. Full text: [`docs/agent/INDEXEDEX_AGENT_LAW.md`](docs/agent/INDEXEDEX_AGENT_LAW.md) § Token policy.
