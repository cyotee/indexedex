...
10. **`forge build` before `forge test` (non-negotiable).** IndexedEx Foundry deployment helpers load creation bytecode from `out/` through `ArtifactCreationCode`, which reads artifact JSON directly and can link external libraries through CREATE3. Keep implementation imports out of helpers that only need artifact bytecode. After a production edit, use **`python3 scripts/forge-artifacts.py test <edited-source.sol> --test-root <test-file-or-directory>`**: it first builds concrete dependents/runtime artifacts, then excludes unrelated compilation roots with `--skip`. Repeat `--test-root` for several suites. Keep configured source/test paths stable: changing `FOUNDRY_TEST` invalidates the shared compiler cache. `build` is available for artifact refresh before `forge script`; add `--consumer '<script-path>'` when preparing its runtime artifacts. `forge test` alone can deploy **stale** `out/` bytecode; missing, empty, or unlinked bytecode must fail. Do not delete `contracts/utils/foundry/CraneFactoryArtifactSeed.sol`. Full builds remain supported. See the [artifact build workflow](docs/testing/ARTIFACT_BUILDS.md) and [`INDEXEDEX_AGENT_LAW.md`](docs/agent/INDEXEDEX_AGENT_LAW.md) § FactoryService creation bytecode.
